...
- [ ] T014 [US2] facade `rust-api/server/src/model/facade/sys_user.rs` 加 `pub async fn update_user(db, id: i64, data: UpdateUserData, operator: i64) -> Result<bool, DbErr>`(data-model §3:`mutate_in_txn`:load active by id〔無→Ok(false)、payload_before=audit_json〕→ update business 欄 + **`updated_at=Set(now())` 與 `updated_by=Set(Some(operator))` 成對**〔§I.6 成對紀律;updated_at **無 DB default/trigger** → 須顯式 set,**非**靠 DB〕〔**不動 user_name/password**,Q1=A〕→ `replace_roles_in_txn` → audit `Update`〔before+after〕)。依賴 T003/T004/T008。**live-DB `#[ignore]`**(update + role replace + audit + 驗 updated_at/by 皆非 null)。守 009 lint。**同檔序列(接 T009)**。
...
- [ ] T017 [US2] **acceptance**(contracts §4,curl;涵蓋 **SC-009 userName 不可改面**):Super updateUser(帶 id)→ 0000 + 暱稱/角色更新、**userName 仍舊**、審計 UPDATE(updated_at/by 非 null);**含一個無效 + 一個有效 roleCode → 只採有效(US2 AS-2/C3,不崩潰)**;Admin→403。依賴 T016。
...
- [ ] T023 **acceptance**(contracts §6,curl;Phase 6 cross-cutting、無 story label;涵蓋 **SC-009 停用無法登入面**):建一 user→啟用時登入 OK→updateUser 設 status=2→該帳號登入回 **1000 + 無 token**;對照啟用帳號(Admin)登入 0000。依賴 T022 + T015(updateUser 設停用)。
