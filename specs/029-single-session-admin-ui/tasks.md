...
- [ ] T007 **3 handlers + getUserList 加欄**:`server/src/handler/system_manage.rs` 加 `get_system_settings`(Res<列設定>)/ `update_system_setting`(`{key,value}` → **依該設定 `value_type` 驗 `value`、非法值→`Res::err` 不持久〔FR-013 write-side 拒絕、原值不變;與 typed accessor read-side fallback 分職〕** → facade update → **`publish_settings_invalidate`**)/ `update_user_session_policy`(`{userId,policy}` → `sys_user::update_session_policy` → **`DEL sess:{userId}`**〔`redis::cmd("DEL").arg(format!("sess:{user_id}"))`,使 028 is_current 下個請求 lazy-rehydrate 新 policy〕);+ `UserItem`(:149-164)加 `session_policy: String`、`user_item()`(:171-184)加參、`get_user_list` mapping 傳 `m.session_policy`(camelCase wire `sessionPolicy`)。
