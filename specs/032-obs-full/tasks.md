...
- [ ] T016 [P] [US3] 建 `deploy/grafana-provisioning/alerting/rules.yml`：`apiVersion:1` + `groups:[{orgId:1, name:rust-api-availability, folder:obs-full, interval:1m, rules:[{uid, title:"rust-api target down", condition:C, data:[A=`up{job="rust-api"}` instant @`datasourceUid:prometheus`, C=threshold `lt 1`], for:2m, noDataState:Alerting, execErrState:Alerting, labels.severity:critical}]}]`。**datasourceUid 用顯式 `prometheus`**（對 T005 datasource uid）;**不** provision notification-policies/contactpoints（內建 default backstop = channel deferred、避 dangling-receiver boot error、R6/D7）per [research R6](research.md) / [data-model §5](data-model.md)
- [ ] T017 [US3] Acceptance **C7**（grafana `/api/v1/provisioning/alert-rules` 非空、含 baseline rule;可選：停 rust-api → rule 進 Alerting）per [verification-commands](contracts/verification-commands.md)（依賴 T016 + US1 T006 grafana profile/datasource + T009 pipeline）
...
- [ ] T021 [P] backfill：CLAUDE §8.2（prometheus `23090`「已進 compose」+ pushgateway `29091`「已進 compose」+ 加 postgres_exporter/redis_exporter 列〔內網無 host port〕+ `prometheus_data` volume + grafana profile→`["obs","metrics"]`;exporter reuse postgres/redis password〔無新 secret〕註記）
