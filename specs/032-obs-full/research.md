...
- **baseline alert rule（D7）**: rule group `rust-api-availability`(folder `obs-full`、auto-create)含 `rust-api target down`(query A `up{job="rust-api"}` instant + expr C threshold `lt 1`、`condition:C`、`for:2m`、`noDataState:Alerting`/`execErrState:Alerting`、`labels.severity:critical`)。可擴 5xx 比率 / exporter down(plan/tasks)。
