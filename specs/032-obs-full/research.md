...
- **Decision**: **`oliver006/redis_exporter:v1.85.0-alpine`**(2026-06-04 stable;quay.io/ghcr.io mirror)。port **9121**(內網)。`REDIS_ADDR=redis://redis-stack:6379`(service short-name、容器內 6379 非 host 26379)。`profiles:[metrics]`。**★ 修正(實作期經驗證)**:**必用 `-alpine` 變體** —— bare `v1.85.0` tag 是 `FROM scratch`、**無 `/bin/sh`**,本 feature sh-wrapper(下方 entrypoint 讀 `redis_password` secret)在 scratch 上起不來;`v1.85.0-alpine`(同版、含 shell)才可。
...
- **Decision**: **`prom/pushgateway:v1.11.3`**(2026-05-27 stable、無 v1.12)。port **9091**(host 29091 §8.2 保留、prod internal-only)。`profiles:[metrics]`。**cleanup-job(獨立 crate `rust-api/cleanup-job/src/main.rs`)加 push**:`metrics-exporter-prometheus` `PrometheusBuilder::new().install_recorder()` → `metrics::gauge!` 記 → `handle.render()` 取 text → **blocking HTTP PUT `http://pushgateway:9091/metrics/job/cleanup_job`**(用 `ureq`、~5 行、deterministic exit)。**★ 修正(實作期)二則**:(a) `metrics-exporter-prometheus` 用 **`default-features = false`** —— 預設 features(`push-gateway`/`http-listener`)會拉進 `hyper-rustls`/`aws-lc-rs`/`cmake` 一整套重 transitive stack,本 feature 只用 `install_recorder()`+`render()`(皆 core/無條件)+ 自寫 `ureq` blocking PUT、**從不用 `with_push_gateway`** → 關預設 features 去掉無謂重依賴;(b) HTTP client 用 **`ureq` 非 `reqwest::blocking`** —— cleanup-job 是 `#[tokio::main]` binary,reqwest blocking client 在 tokio runtime 內建構會 panic(nested runtime),`ureq`(無 runtime 依賴)才安全。
