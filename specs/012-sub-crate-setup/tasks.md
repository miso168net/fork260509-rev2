...
- [ ] T005 在 `migration/src/m20260529_000005_create_casbin_rule.rs` 新增建表 migration：`up()` 呼 `sea_orm_adapter::up(manager.get_connection())`、`down()` 呼 `sea_orm_adapter::down(...)`（單一 schema 來源、避免手寫 DDL 與 adapter drift，[research R3](./research.md)）；`migration/Cargo.toml` 加 dep `sea-orm-adapter`（path）；`migration/src/lib.rs` 加 `mod` + `Box::new(...005...::Migration)`。並給拷貝進來的 `sea-orm-adapter/src/migration.rs` 的 `create_table` 補 `.if_not_exists()`（若原碼未帶）→ 使 adapter `new()` 的自動建表成無害 no-op（[research R3](./research.md)）
...
- [ ] T007 [US1] Acceptance：[verification §1](./contracts/verification-commands.md) —— dev stack up（postgres + `run --rm migrate` 套 001..005）；跑 T006 smoke（`docker run --network rev2-admin_rev2_net -e DATABASE_URL=... ... test -- --ignored`）→ 綠；psql 斷言 `casbin_rule` 有 `p,alice,data1,read` 列。對應 spec US1 + SC-001/SC-002
