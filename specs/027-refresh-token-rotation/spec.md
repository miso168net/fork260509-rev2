...
- **SC-002**: 重放一張已被換掉的舊憑證,在正常路徑(單實例、無極端並發)下 **100%** 使該登入族系全部憑證作廢、真實使用者下次換新被乾淨登出(影響遏制在一次重新登入);**極端並發**下(合法換新的新憑證 insert 與盜用偵測的整族系作廢同毫秒交錯)可能殘留一張 active,於該使用者**下次任一換新**觸發的偵測時收斂作廢(data-model §4 競態註,殘餘風險可接受)。
