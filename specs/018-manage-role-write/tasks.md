...
**Tests**:純函式邏輯 test-first(`is_seed_role_code` / `find_active_enabled` SQL / facade SQL-build / enum);**wiring + 形狀對映 + base-web 接線無單元測試 → 由 `contracts/verification-commands.md` C-V(CDP + curl + psql)覆蓋**(理由:wiring/接線無可獨立測的純邏輯,沿 016/017 慣例)。
...
- [ ] T005 [US-shared] facade `find_active_by_id(db,id)->Result<Option<Model>,DbErr>`(**供 T017/T022 種子 guard 解析 id→code**、亦供 update/soft_delete 載入 active row)+ `find_active_enabled()->Select<Entity>`(`find_active().filter(Column::Status.eq(1))`)+ **單測**(斷言 find_active_enabled SQL 含 `deleted_at IS NULL` AND `status` = 1)in `rust-api/server/src/model/facade/sys_role.rs`(依賴 T002)
...
- [ ] T010 [P] 純函式 `fn is_seed_role_code(code:&str)->bool { matches!(code, "R_SUPER"|"R_ADMIN"|"R_USER_COMMON") }`(種子身分**以 roleCode 識別**,對齊 spec FR-005「code 為穩定鍵」、非 id 1-3〔analyze I1 親決〕)+ **單測**(三碼=true、其他=false)in `rust-api/server/src/handler/system_manage.rs`
...
- [ ] T017 [US2] handler `update_role` + `RoleUpdateReq{id:String,roleName,roleDesc?,status?}`(**省 roleCode**,D2 immutable;serde 丟棄 wire 帶的 roleCode)+ **種子停用 guard(code-based)**(若 `status` 解析為停用(2):`find_active_by_id(id)` 取 row → `is_seed_role_code(&row.code)`→2222「不可停用系统内置角色」)+ status 非法→2222「状态取值无效」(複用 T013 `enum_str_to_i16`)+ not-found(Ok(false))→2222「角色不存在」+ id parse 失敗→2222 + route `POST /systemManage/updateRole`(enforce_mw)in `rust-api/server/src/handler/system_manage.rs` + `main.rs`(依賴 T016/T010/T005)
...
- [ ] T022 [US3] handler `delete_role`(DeleteReq{id:String})+ `batch_delete_roles`(BatchDeleteReq{ids:Vec<String>})+ **種子保護 guard(code-based)**(handler `find_active_by_id` 解析每個 id→`row.code`、`is_seed_role_code`:單筆種子或批次任一種子→2222「不可删除系统内置角色」**整批拒、無部分執行**〔批次先全載入 code 檢查、通過才逐筆 soft_delete〕)+ 空 ids→no-op Ok + id parse 失敗→2222 + not-found(Ok(false))寬鬆跳過 + routes `DELETE /systemManage/deleteRole`·`/batchDeleteRole`(enforce_mw)in `rust-api/server/src/handler/system_manage.rs` + `main.rs`(依賴 T021/T010/T005)
...
- [ ] T026 回歸:013 enforce allow/deny(B 改後)+ **refresh-then-enforce(G1):停用角色→access 過期→舊 refresh 換新 access(claims.roles 仍含已停用角色)→新 access 打受保護端點 enforce 仍 403+5003(證 claims.roles 對 enforce vestigial、即時性不被 refresh 重簽復活)** + 014 menu 三階梯 + 016 list(getRoleList 顯停用·roleDesc/status 真值;getAllRoles 排除停用)+ **017 US1-3 + update_user updated_at DB-side 成對** + 守恆(server 單測 + entity_access_lint〔handler 零 `entity::`〕+ xdb + `Migrator::up` grep 0)
