...
- [ ] T010 [US1] Acceptance：[verification §1/§2/§5](./contracts/verification-commands.md) —— dev stack up（postgres + `run --rm migrate` 套 001..009）+ rust-api up;**curl 活體**（login Super→token → getUserInfo `{userId:"1",userName:"Super",roles:["R_SUPER"],buttons}`;User→`User01` alias;帳密錯 `1000`;token 缺/壞 `3333`）;**＋ CDP browser 登入 smoke**（base-web `.env` `VITE_SERVICE_BASE_URL`→rev2〔BASE-WEB-ADAPT、見 [research R8](./research.md)〕→ 登入頁 Super/123456 → 登入成功 + getUserInfo 渲染對齊）。**CDP defer 風險自覺**：若 base-web build/設定阻礙 → 記 plan Deviation + Follow-up 補測、curl 部分仍須綠。對應 spec US1 + SC-001。
