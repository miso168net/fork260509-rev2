...
- **D-002（impl 階段、menu policy row count 文件修正）**：T003 實作時發現 spec-kit 文件 prose 多處寫「8 rows」,但 [data-model §3](#) 自身的 D3 矩陣枚舉 = home(✓×3)+manage_user(✓×2)+manage_user-detail(✓×2)+manage_role(✓×1)+manage_menu(✓×1)=**9 rows**(Super 5 + Admin 3 + User 1),且 spec FR-002 明列 Admin 含「使用者明細」(manage_user-detail)、acceptance §3 亦要 Admin manage children=[manage_user,manage_user-detail]。**矩陣 + FR-002 為權威**,「8」為算術誤植。**處置**:migration seed **9 rows**(不丟列、否則破壞某角色選單);同步修正 9 處文件計數(tasks.md/data-model.md/research.md/quickstart.md/verification-commands.md);D3 可見範圍**不變**(純計數修正、非設計變更)。
