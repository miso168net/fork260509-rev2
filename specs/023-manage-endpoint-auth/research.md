...
**Decision**:023 migration seed **per-endpoint R_SUPER 列(無 wildcard)** —— R_SUPER 每 gated endpoint 一條 `(R_SUPER,path,method)`,如 009/016 以來 de-facto。**in-place 校正** `docs/INTEGRATION-DESIGN.md` 三處過時 `p,R_SUPER,*,*` 文字,並記為 **§11.21 amendment-of-record**(鏡像 §11.19/§11.20 blockquote 格式)。三處:**L500**(§4.6.3 矩陣列「全通(wildcard)」)/ **L521**(§4.6.3 seed 寫法「R_SUPER 用 wildcard `p,R_SUPER,*,*`」)/ **L749-752**(§6.3「`p,R_SUPER,*,*`(super 全通)」)→ 皆改「逐 endpoint 列、無 wildcard、無 `*` 主體」。
