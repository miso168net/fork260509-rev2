...
**as-built 行號漂移注記(回填,§2.27)**:本節與 `tasks.md` T010 引用的 `INTEGRATION-DESIGN.md` 行號(L500/L513/L515-517/L521/L523/L749-752/L750/L1104)為 023 當時值;DESIGN 後續多次編輯(至 026 auth DRY refactor)行號已位移 → **以 section anchor 為準**(§4.6.3 矩陣/seed/menu-read、§6.3 seed body、§10 Phase 4 016 as-built、§11.22 reconcile-of-record),勿信字面行號。reconcile 內容(per-endpoint 無 wildcard、menu-read 三列 Super-only)已落地、不受行號漂移影響。
