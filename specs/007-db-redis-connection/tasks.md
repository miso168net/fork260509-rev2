...
- [ ] T005 Edit `rev2-root/docker-compose.yml`:top-level `secrets:` 加 `database_url`(file `./deploy/secrets/database_url.txt`)+ `redis_url`(file `./deploy/secrets/redis_url.txt`);`rust-api` service `secrets:` 加這 2 個 + `environment:` 加 `APP_DATABASE_URL_FILE: /run/secrets/database_url` / `APP_REDIS_URL_FILE: /run/secrets/redis_url`。dev override rust-api **不重定義 `secrets:`/`environment:`**,沿用 master 服務定義的 2 secret + `APP_*_URL_FILE` env(compose 合併以 master 為準;dev 連 `postgres:5432`/`redis-stack:6379` 同 URL);由 **T010 dev stack `up --wait` 連線成功**確認繼承生效,實測若未繼承才在 dev override 顯式補(以實測為準)(對齊 [data-model](./data-model.md) + [quickstart](./quickstart.md))
...
- [ ] T014 [US2] Edit `rev2-root/rust-api/migration/src/main.rs`:stub → `cli::run_cli(migration::Migrator).await`(sea-orm-migration 內建 CLI;server 不自動 migrate)。**定稿 dev migration invocation** = `docker compose -f docker-compose.yml -f docker-compose.dev.yml run --rm --entrypoint "" rust-api cargo run --bin migration -- up`(cargo-watch image 覆寫 entrypoint);implementer 對真 image 確認可跑,dev image 若無 cargo 則改用 prod image entrypoint dispatch `migration`
