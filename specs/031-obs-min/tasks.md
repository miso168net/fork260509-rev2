...
- [ ] T013 [US2] Acceptance **C2**（log↔audit：已認證請求 → loki `{service="rust-api"} | json | fields_trace_id="<…>"`〔rust-api trace_id 巢狀於 `fields`、見 data-model §1.1〕對上 psql `sys_access_log.trace_id`）+ **C3**（經 front-nginx 請求 → `{service="front-nginx"} | json` 見 JSON access log 含 `request_id`、與 rust-api 同值）。**前置**：`dcargo build -p server && docker compose ... restart rust-api`（WSL2 inotify、memory `devstack-acceptance-restart`）+ `restart front-nginx`（依賴 T010/T011/T012 + US1 T009 pipeline 已起）
