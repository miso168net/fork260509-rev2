...
- **Rationale**: `ctx_mw` 已算好 `trace_id`（`extract_trace_id`、X-Request-Id header 或 mint uuid v4、`audit_ctx.rs:74-80`、`:109`）並寫進 `sys_access_log.trace_id`（`:136`）→ 同一 id 同時落 log 與審計 DB → log↔審計對接（FR-003）。production log = `fmt().json()`（`application.yaml:11` `format:"json"` → `main.rs:506` `_ =>` arm）。boundary event 把 `trace_id` 當 **event 欄位**(而非僅靠 span 巢狀)→ 提供穩定、非 array-index-fragile 的 LogQL 查詢鍵。
- **CRITICAL open question 解決（JSON span-field path）— ★ as-built 修正**: tracing-subscriber 0.3 `fmt().json()` 預設 `with_current_span=true`/`with_span_list=true` → span 欄位**巢狀**在 `"span"`/`"spans"` 物件下（`| json` 後變 `span_trace_id`、array 變 `spans_0_trace_id` index-fragile）。**故設計用 boundary event 把 `trace_id` 當 event 欄位**避開 array 路徑。**惟 `fmt().json()` 同時把 event 欄位巢狀在 `"fields"` 物件下**(不只 span 欄位 — 這是 rust-api 全體 log 既有慣例,如 sqlx log 的 `fields.summary`/`fields.db.statement`),故 as-built 查詢鍵為 **`fields.trace_id` → loki `fields_trace_id`**:穩定 LogQL = **`{service="rust-api"} | json | fields_trace_id="X"`**(或顯式抽取 `| json trace_id="fields.trace_id" | trace_id="X"`)。**目標仍達成**:`fields_trace_id` 正是穩定、非 array-fragile 的 path(R1 要避開的是 `spans_0_trace_id` array index);原 Rationale「top-level flat」措辭不精確 — boundary event 確實提供了穩定查詢鍵、只是落在 `fields_trace_id` 而非字面 top-level `trace_id`。**`flatten_event(true)`(把 event 欄位提到 top-level)刻意不採用** — 那會改變所有 rust-api log 的形狀、超出 031 audit_ctx.rs-only 範圍 → 無 code 改動(實作正確且與 app 既有 `fields`-nesting 慣例一致)。span 提供巢狀事件的 ambient 關聯、boundary event 提供穩定的 `fields_trace_id` 查詢鍵。
