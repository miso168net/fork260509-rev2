...
| 2026-05-28 | T013 postgres 重 init 的卷清理方式 | `docker compose ... down -v`(清全部 named volume) | `down --remove-orphans` + `docker volume rm rev2_postgres_data`(只清 postgres 卷) | `down -v` 會連 rust-api `rust_api_cargo_cache`/`rust_api_target` 與 base-web `bw_node_modules` 一起清,致冷重建超過 healthcheck 視窗(rust-api start_period 120s)→ `up --wait` 假性失敗。R9 真正需求僅「postgres data dir 為空才重 init」,只移除 postgres 卷即達成、且保留 build 快取暖啟動。user 拍板。已同步 [verification-commands §7](./contracts/verification-commands.md)。 |
