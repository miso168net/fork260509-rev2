---
source_file: "base-web/packages/scripts/package.json"
type: "code"
community: "Build/Release Scripts (web)"
location: "L17"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Build/Release_Scripts_web
---

# devDependencies

## Connections
- [[@soybeanjschangelog]] - `contains` [EXTRACTED]
- [[bumpp]] - `contains` [EXTRACTED]
- [[c12]] - `contains` [EXTRACTED]
- [[cac]] - `contains` [EXTRACTED]
- [[consola_1]] - `contains` [EXTRACTED]
- [[enquirer]] - `contains` [EXTRACTED]
- [[execa]] - `contains` [EXTRACTED]
- [[kolorist_1]] - `contains` [EXTRACTED]
- [[npm-check-updates]] - `contains` [EXTRACTED]
- [[package.json_6]] - `contains` [EXTRACTED]
- [[picomatch]] - `contains` [EXTRACTED]
- [[rimraf]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Build/Release_Scripts_web