---
source_file: "rust-api/server/src/model/facade/sys_casbin_rule.rs"
type: "code"
community: "Casbin Policy Archive & Enforcer (rust)"
location: "L1579"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Casbin_Policy_Archive__Enforcer_rust
---

# live_menu_restore_brings_policy_back()

## Connections
- [[archives_for_route()]] - `calls` [EXTRACTED]
- [[clean()]] - `calls` [EXTRACTED]
- [[connect()_1]] - `calls` [EXTRACTED]
- [[delete_menu_cascade_in_txn()]] - `calls` [INFERRED]
- [[grant_menu_vis()]] - `calls` [EXTRACTED]
- [[mutate_in_txn()]] - `calls` [INFERRED]
- [[restore_menu_cascade_in_txn()]] - `calls` [INFERRED]
- [[seed_menu()]] - `calls` [EXTRACTED]
- [[sys_casbin_rule.rs_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Casbin_Policy_Archive__Enforcer_rust