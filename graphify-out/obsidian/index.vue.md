---
source_file: "fork260509-soybean-admin-base/packages/materials/src/libs/admin-layout/index.vue"
type: "code"
community: "Layout Geometry & Scroll"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Layout_Geometry__Scroll
---

# index.vue

## Connections
- [[fixedHeaderAndTab]] - `contains` [EXTRACTED]
- [[footerLeftGapClass]] - `contains` [EXTRACTED]
- [[handleClickMask()]] - `contains` [EXTRACTED]
- [[headerLeftGapClass]] - `contains` [EXTRACTED]
- [[index.ts_9]] - `imports_from` [EXTRACTED]
- [[isContentScroll]] - `contains` [EXTRACTED]
- [[isHorizontal]] - `contains` [EXTRACTED]
- [[isVertical]] - `contains` [EXTRACTED]
- [[isWrapperScroll]] - `contains` [EXTRACTED]
- [[leftGapClass]] - `contains` [EXTRACTED]
- [[showFooter]] - `contains` [EXTRACTED]
- [[showHeader]] - `contains` [EXTRACTED]
- [[showMobileSider]] - `contains` [EXTRACTED]
- [[showSider]] - `contains` [EXTRACTED]
- [[showTab]] - `contains` [EXTRACTED]
- [[siderPaddingClass]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Layout_Geometry__Scroll