---
source_file: "base-web/src/plugins/index.ts"
type: "code"
community: "App Bootstrap (web)"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/App_Bootstrap_web
---

# index.ts

## Connections
- [[app.ts_1]] - `re_exports` [EXTRACTED]
- [[components.ts]] - `re_exports` [EXTRACTED]
- [[dayjs.ts_1]] - `re_exports` [EXTRACTED]
- [[iconify.ts]] - `re_exports` [EXTRACTED]
- [[loading.ts]] - `re_exports` [EXTRACTED]
- [[main.ts]] - `imports_from` [EXTRACTED]
- [[nprogress.ts]] - `re_exports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/App_Bootstrap_web