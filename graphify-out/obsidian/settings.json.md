---
source_file: "fork260509-soybean-admin-base/.vscode/settings.json"
type: "code"
community: "Chart Plugin Views"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Chart_Plugin_Views
---

# settings.json

## Connections
- [[editor.codeActionsOnSave]] - `contains` [EXTRACTED]
- [[editor.defaultFormatter]] - `contains` [EXTRACTED]
- [[editor.formatOnSave]] - `contains` [EXTRACTED]
- [[eslint.validate]] - `contains` [EXTRACTED]
- [[i18n-ally.displayLanguage]] - `contains` [EXTRACTED]
- [[i18n-ally.editor.preferEditor]] - `contains` [EXTRACTED]
- [[i18n-ally.enabledFrameworks]] - `contains` [EXTRACTED]
- [[i18n-ally.enabledParsers]] - `contains` [EXTRACTED]
- [[i18n-ally.keystyle]] - `contains` [EXTRACTED]
- [[i18n-ally.localesPaths]] - `contains` [EXTRACTED]
- [[i18n-ally.parsers.typescript.compilerOptions]] - `contains` [EXTRACTED]
- [[oxc.fmt.configPath]] - `contains` [EXTRACTED]
- [[unocss.root]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Chart_Plugin_Views