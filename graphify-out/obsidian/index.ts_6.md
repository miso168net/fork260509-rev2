---
source_file: "base-web/packages/color/src/types/index.ts"
type: "code"
community: "Theme & Color Config (web)"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Theme__Color_Config_web
---

# index.ts

## Connections
- [[ColorIndex]] - `contains` [EXTRACTED]
- [[ColorPalette]] - `contains` [EXTRACTED]
- [[ColorPaletteFamily]] - `contains` [EXTRACTED]
- [[ColorPaletteFamilyWithNearestPalette]] - `contains` [EXTRACTED]
- [[ColorPaletteMatch]] - `contains` [EXTRACTED]
- [[ColorPaletteNumber]] - `contains` [EXTRACTED]
- [[ColorPaletteWithDelta]] - `contains` [EXTRACTED]
- [[antd.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_2]] - `re_exports` [EXTRACTED]
- [[index.ts_4]] - `imports_from` [EXTRACTED]
- [[palette.ts]] - `imports_from` [EXTRACTED]
- [[recommend.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Theme__Color_Config_web