---
source_file: "fork260509-soybean-admin-base/src/utils/storage.ts"
type: "code"
community: "Community 46"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_46
---

# storage.ts

## Connections
- [[createLocalforage()]] - `imports` [EXTRACTED]
- [[createStorage()]] - `imports` [EXTRACTED]
- [[dayjs.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_20]] - `imports_from` [EXTRACTED]
- [[index.ts_23]] - `imports_from` [EXTRACTED]
- [[index.ts_29]] - `imports_from` [EXTRACTED]
- [[index.ts_33]] - `imports_from` [EXTRACTED]
- [[index.ts_34]] - `imports_from` [EXTRACTED]
- [[index.ts_36]] - `imports_from` [EXTRACTED]
- [[index.ts_37]] - `imports_from` [EXTRACTED]
- [[loading.ts]] - `imports_from` [EXTRACTED]
- [[localStg]] - `contains` [EXTRACTED]
- [[localforage_1]] - `contains` [EXTRACTED]
- [[route.ts]] - `imports_from` [EXTRACTED]
- [[sessionStg]] - `contains` [EXTRACTED]
- [[shared.ts_3]] - `imports_from` [EXTRACTED]
- [[shared.ts_4]] - `imports_from` [EXTRACTED]
- [[shared.ts_5]] - `imports_from` [EXTRACTED]
- [[shared.ts_8]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_46