---
source_file: "base-web/src/layouts/base-layout/index.vue"
type: "code"
community: "web: getsiderandcollapsedwidth"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/web_getsiderandcollapsedwidth
---

# index.vue

## Connections
- [[getSiderAndCollapsedWidth()]] - `contains` [EXTRACTED]
- [[headerProps]] - `contains` [EXTRACTED]
- [[imports.ts]] - `imports_from` [EXTRACTED]
- [[isTopHybridHeaderFirst]] - `contains` [EXTRACTED]
- [[isTopHybridSidebarFirst]] - `contains` [EXTRACTED]
- [[isVerticalHybridHeaderFirst]] - `contains` [EXTRACTED]
- [[isVerticalMix]] - `contains` [EXTRACTED]
- [[layoutMode]] - `contains` [EXTRACTED]
- [[siderCollapsedWidth]] - `contains` [EXTRACTED]
- [[siderVisible]] - `contains` [EXTRACTED]
- [[siderWidth]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/web_getsiderandcollapsedwidth