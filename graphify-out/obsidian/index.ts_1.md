---
source_file: "fork260509-soybean-admin-base/packages/axios/src/index.ts"
type: "code"
community: "Alova Request Adapter (+docs)"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Alova_Request_Adapter_docs
---

# index.ts

## Connections
- [[CustomAxiosRequestConfig]] - `imports` [EXTRACTED]
- [[FlatRequestInstance]] - `imports` [EXTRACTED]
- [[MappedType]] - `imports` [EXTRACTED]
- [[RequestInstance]] - `imports` [EXTRACTED]
- [[RequestOption]] - `imports` [EXTRACTED]
- [[ResponseType]] - `imports` [EXTRACTED]
- [[constant.ts_1]] - `re_exports` [EXTRACTED]
- [[createAxiosConfig()]] - `imports` [EXTRACTED]
- [[createCommonRequest()]] - `contains` [EXTRACTED]
- [[createDefaultOptions()]] - `imports` [EXTRACTED]
- [[createFlatRequest 实例创建实战]] - `contains` [EXTRACTED]
- [[createRequest()]] - `contains` [EXTRACTED]
- [[createRetryOptions()]] - `imports` [EXTRACTED]
- [[index.ts_20]] - `imports_from` [EXTRACTED]
- [[index.ts_29]] - `imports_from` [EXTRACTED]
- [[options.ts]] - `imports_from` [EXTRACTED]
- [[shared.ts]] - `imports_from` [EXTRACTED]
- [[table.ts]] - `imports_from` [EXTRACTED]
- [[transformResponse()]] - `imports` [EXTRACTED]
- [[type.ts_1]] - `re_exports` [EXTRACTED]
- [[use-request.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Alova_Request_Adapter_docs