# rev3 管理系統開發設計書 — 骨架目錄 (v2)

> **狀態**：v2 = v1 全章展開 + 結構重排（Part I～IV）+ 脊椎章 45 條驗證修正（2026-06-11；grounded on rev2 live DDL 稽核 + 實碼逐行核對）。**⚠️ = 工程決策的「資深建議預設」、待 user 覆核**；六個開放問題（待決①～⑥）全數留 user 親決——**全書決策總表見附錄 G**。
> **結構**：Part I 總綱（§0～§2，10 分鐘懂全貌）／ Part II 設計契約（§3～§7，凍結度高）／ Part III 交付計畫（§8，明示可變）／ Part IV 基線與附錄（§9～§11 + 附錄 A～G）。
> **與 v1 編號差異**：舊 §2 凍結基盤→§9、舊 §8.1/§8.1a→§1.5/§1.6、舊 §3.6+§5.10 取捨+§10.3 AES→§2 決策包、舊 §3.2 欄級字典→附錄 F（§3 子節順移：舊 3.3→3.2、3.4→3.3、3.5→3.4）、舊 §8.2–8.4 + 舊 §9 全部→§8 交付計畫；§3/§4/§5/§6/§7/§10/§11 章號不變。
> **rev2 編號慣例**：本書將移植到 rev3 repo——文中凡 `（rev2 NNN）` 指 rev2 的 feature 編號（rev2 repo `specs/<NNN>-*`，001–035）；`rev2 mNNN` 為 rev2 migration `m20260529_000NNN` 縮寫。rev3 自身的 feature／migration 編號日後不帶此前綴，兩者不可混讀。
> **素材來源**：真實命名由 rev2 三權威文件 ground —— `REVIEW-DATABASE.md`(12-table live 稽核)、`constitution.md` v1.6.0、`INTEGRATION-DESIGN.md` + 35 specs 與實碼實機核對。
...
- **§0.3 文件衛生紀律**：本書是**前瞻設計（薄、穩定）**；as-built 實錄走 append-only 的 `INTEGRATION-MILESTONES.md`。**設計書不因實作而膨脹**——rev2 的 `INTEGRATION-DESIGN.md` 膨脹到 247KB（實測 246,803 bytes）＝計畫與實錄混寫的代價。本書各章只寫設計結論與不變式；實作細節留 `specs/` 與 MILESTONES。
...
| **track（受管軌道）** | base-web「不動 inline」鐵紀律下的受控改動授權邊界（constitution §III）：**預設可動** = L1/L2 BASE-WEB-ADAPT（`.env` + typings 新檔）、L3 BASE-WEB-WRAPPER（`rev2-*` 新檔）、RUSTAPI-SOURCE-ISOLATION（rust-api 全新寫）；**★ 需 constitution 顯式授權** = L4 BASE-WEB-BUILD-CONFIG（`pageExcludePatterns` 隱藏 demo）、L4 MODAL-WIRING（views inline 5 用途）。 | §9.4・constitution §III |

> **標記慣例**：⚠️ = 工程決策的「資深建議預設」、待 user 覆核（總表附錄 G）；「待決①～⑥」指文末開放問題，user 親決前不入凍結集。
...
### §1.1 範圍宣告
- **base-web 為權威**（constitution §I.1，NON-NEGOTIABLE）：base-web example 有的功能、rust-api 都要提供對應 endpoint；**「v1 從簡」只能是 phase 實作排程、不能簡化設計範圍**（§I.1 原文）。
- 功能全集的兩層錨：**wire ground truth** = `MOCK-COVERAGE-AUDIT.md`（mock 實機 capture）→ 期望 API 全集 = rev2 DESIGN §3.1（20 端點）→ rev2 as-built 已長成 **41 條業務 route**（auth 3 + route 3 + systemManage 35，`rust-api/server/src/main.rs`）+ `/health` `/metrics`。**rev3 FR 基線 = 這 41 條 as-built 全集**，不是早期 20 端點表。
- 超出 base-web 既有功能的擴張候選 = user-facing dashboard / reporting / 靜態加密 / 合規升級——**四項集中於 §2 新能力決策包（皆待決⑥）**、不入 v1 凍結集。
...
- 替代 login 入口 4 流程（reset-pwd / code-login / register / bind-wechat）：constitution §II §11.13 拍板 (c) 全實作雙模 + v1 stub mode；as-built `main.rs` **無**任何對應 route（35 features 未排入）。
...
- **單實例 v1**（對齊 §5.6）：無 HA / failover 承諾；redis pub-sub 失效通知 v1 即啟用、為未來多實例預留收斂路。
...
- **資料成長**：三 log 表 append-only、DB 端無保留策略（as-built；`cleanup-job` 僅清 `sys_token` 過期列）→ ⚠️ v1 僅容量監控、retention 政策 defer（與 §10.4 不可竄改互看；多租戶情境見 §2 第 4 項）。

### §1.4 明確不做（防 scope creep）
mock / demo 有、rev3 延續 rev2 刻意不做（實證：rev2 DESIGN §3.6 mock-only 清單 + as-built `main.rs` route 全集）：
- **mock 不檢 Authorization**（無 Bearer 也回 success）→ 不模仿；全業務端點 Casbin enforce（§5.3）。
- **mock `getConstantRoutes` 偶發 502** → 不模仿；100% 回 200。
- **ApiFox 平台鑑權 header `apifoxToken`**（MOCK-COVERAGE-AUDIT §4.8）→ 不模仿；rust-api 忽略 unknown header（§II §11.4）。
- **`/auth/error` echo 工具端點**（消費者僅 demo 頁）：rev2 as-built 未提供 → ⚠️ rev3 維持不做（demo 頁不顯示、無呼叫者）。
- **`/mock/getLastTime`**（alova demo 專用）→ 不做。
- **demo menu 不入 `sys_menu` seed**：8 組 demo customRoutes（document/exception/multi-menu/iframe 等，constitution §I.2）在 dynamic mode 無 menu 列即不顯示；唯一例外 `function`/`function_toggle-auth` 已升真實 Casbin-enforced 選單（rev2 022、§I.2 v1.3.0 amend）；BUILD-CONFIG ★ 授權的 `pageExcludePatterns` **至今未動用**（as-built `base-web/build/plugins/router.ts` 無此欄位）。
- **真實 SMS / wechat OAuth 流程**：§11.13 雙模拍板的「真實 mode」明列 v2/v3 → v1 不做（stub 尾巴見 §1.2 註）。
...
- **as-built 校正（rev3 必須採真實命名、丟掉 rev2 DESIGN §4.2/§5.2 提案）**：① **無 `axum-casbin` crate**（enforce 在 `auth/enforce.rs`）② **無 service 層**（handler 直呼 facade）③ **router flat-in-`main.rs`**（無 `router/` 樹，受管端點逐條 `.route()`、endpoint_coverage_lint 守 main.rs==ENDPOINT_REGISTRY==seed 三源一致 @ 35；維持 flat 或重整 = 待決①）④ `model/` 拆 `entity/` crate + `server/src/model/facade/`。
...
【目的】rev3 相對 rev2 唯一真正的新 scope 集中一處：選項、成本、牽連面一表呈現、**拍板一次**；其他章只引用本章結論、不再散落重複（v1 骨架原散落於 §1.2/§3.6/§4.4/§5.10/§10.3 五處）。

| # | 能力 | rev2 現況（實證） | v1 建議形 ⚠️ | deferred 形（觸發條件） | 牽連 |
|---|---|---|---|---|---|
...
- **v1 統一結論 ⚠️**：四項 v1 建議形皆 **0 新表** → **rev3 v1 資料模型 = rev2 同 12 表（一次設計對，§3）**；deferred 形（`sys_dashboard_view` / `sys_report_job` / encrypted-col）登記為未來表、**不入 v1 凍結集**。
...
- 三個新能力（dashboard / report / AES）的 schema 取捨集中於 §2：**v1 全 0 新表、rev3 v1 = rev2 同 12 表**。
...
**關係表**（純文字環境看這張即可；CJK 對齊安全）。**FK 欄判讀**：rev2 as-built **全表零 FK**（含 `sys_user_role`，REVIEW-DATABASE 逐表確認）；下表 ✅ = **rev3 建議加 FK（⚠️ 待決④）**、❌ = 建議維持零 FK——勿把 ✅ 誤讀為現狀或已拍板：
...
【目的】系統裡「行為 > 資料」的少數模組。**先設計 states + transitions + invariants，表只是那台機器的持久化。** 判定法則：有非平凡狀態機 / 「先 X 再 Y 會怎樣」的故事 → 行為島。rev2 的痛正是把唯一真正的行為島（治理）當「又一張表」排到最後（附錄 B）。rev3 共 **3 台狀態機**（持久化於 §3 的 sys_token / casbin_rule+archive / sys_user.session 欄），各自獨立設計、各自一刀（§8.2）。
...
- 縱切時**各自一刀**（§8.2：Token/Session 一刀、Policy-governance 一刀）。
- **⚠️ 潛在第 4 台島（報表非同步，建議 defer；取捨集中 §2）**：若報表升為排程/非同步 → `sys_report_job` 帶 `queued→running→done→failed` 狀態機，屆時**升格為行為島**、用同款鏡頭設計；v1 同步匯出不觸發、不立此島。
...
> **universal 面**（不入下表逐格）：**§5.4 envelope**（每個 API；例外 `/health` plain text 與 `/metrics` Prometheus exposition）、**§5.5 single-session gate**（每受保護讀端）、**§5.10 AES at-rest**（v1 磁碟層、全庫一致，§2）。
...
| `sys_casbin_policy_archive` | —(進/全離) | ✓(restore 寫；審計記在 entity_table='casbin_rule') | ✓(回收桶 Super) | ✓(getArchived：全量、無 filter/分頁、archived_at DESC) | — | D-buffer |
...
### §5.2 統一審計（mutation）
mutation → `sys_operation_log`：`audit::mutate_in_txn`（`model/audit.rs`）泛型 wrapper **同 txn** 寫 before/after 快照 + operator + trace；`AuditSerialize` trait 對敏感欄（password）redact 成 `"<redacted>"`；`audit.rs` 本身不 import entity（守 lint）。**HTTP/region 軌另走** `sys_access_log`（`audit_ctx` 中介層 + xdb，§5.9）——**兩 sink = 縱切兩刀（§8.2 地基）**。
...
面定義：at-rest 加密全庫一致、對 schema/查詢透明。v1 建議形（磁碟/tablespace 層）與 deferred 欄位級方案的代價比較**集中於 §2 新能力決策包**；若未來採欄位級，加密欄退出 §5.8 filter/index 並逐欄標記。
...
【目的】補 rev2 §9.1 grep 人工守的痛點，升為可機器校驗的契約。base-web 為權威 → wire 由前端期望定義（§6 → §7）；與資料脊椎衝突時 **wire 優先**（§0.1 雙脊椎宣告）。

### §7.1 endpoint 全集
> 盤點來源：base-web `src/service/api/{auth,route,system-manage,rev2-system-manage}.ts` 共 **42 個 fetch 函式**，cross-check rust-api `server/src/main.rs` flat router **43 條 route**——其中 **35 條掛 `enforce_mw` route_layer**（`endpoint_coverage_lint` 鎖 `EXPECTED_ROUTE_COUNT=35`）、8 條不掛（public 或 JWT-in-handler）。對齊結果：**41 條兩端俱在；1 條 mock-only**（`/auth/error`）；`/health`/`/metrics` 為 rust-only infra、無 base-web caller。path 全集對齊 mock ground truth（`MOCK-COVERAGE-AUDIT.md`）。
> 形狀欄縮寫：`Api.*` 型別宣告於 `base-web/src/typings/api/{auth,route,system-manage,common}.d.ts`；`Api.SystemManage.` 前綴以下省略。另有 alova mock 7 endpoint 不經 `service/api/*`、不入本表（silent-fallback 風險見 rev2 DESIGN §9.5）。
...
| POST | `/systemManage/add{User,Role,Menu}` · `update{User,Role,Menu}` | 建/改（update 多帶 `id`；userName/roleCode/routeName immutable） | write-model（`rev2-system-manage.ts` inline `UserWriteModel` 等） | `null` | rev2 017/018/020 |
...
### §7.2 三端對齊機制
**三端**（rev2 真實位置）：① rust 手寫 serde DTO（`server/src/handler/{auth,route,system_manage}.rs`，`rename_all = "camelCase"`）② base-web 型別 = `typings/api/*.d.ts` 全域宣告 + `service/api/*.ts` inline write-model ③ component state（`views/manage/**`）。rev2 機制 = **人工 grep 紀律**（rev2 DESIGN §9.1；CLAUDE.md §3 Phase 0 research 紀律同款），無任何機器校驗。

**痛點實證（rev2）**：
- **id/parentId string type-lie 引爆（rev2 REVIEW-014-026 §4.1，rev2 025-I1）**：rust 把 `id`/`parent_id` 序列化為**字串**（§I.3 凍結），typings 卻宣告 `number`（`Common.CommonRecord.id`/`Menu.parentId`）且 `defaultTransform` 不轉型 →（rev2 016/017）潛伏無害，（rev2 025）`menu-operate-modal.vue` 對 wire `"0" === 0` 為 false → `effectiveLayout=''` 吃掉 component 前綴 = 可觀察 data-corruption。修法落消費端 `Number()` 正規化、wire 凍結不動。
- **grep 紀律本身會 rot**：rev2 DESIGN §9.1 的示例 grep 至今指向 `server/src/api/*.rs`，而實碼在 `server/src/handler/`——人工紀律無 CI 錨、目錄改名即靜默失準。

**待決② 選項並陳**（本書不拍板）：

| 選項 | 收益 | 代價 |
|---|---|---|
| A 維持人工 grep 紀律（rev2 as-is） | 零工具投資；紀律已成文（CLAUDE.md §3） | 無機器強制，（rev2 025-I1）類 lie 仍靠 review 抓；grep 例已實證會 rot |
...
| 7 | wire | base-web `service/api/rev2-*.ts` wrapper + `typings/api/*.d.ts`；走 §9.4 軌道（L3 WRAPPER / ★L4 MODAL-WIRING） | §7 三端對齊 |
...
| 9 | CDP 驗收 | 經 front-nginx `:21080` 真實 `/api` 路徑、isolated browser context；curl 直送 ≠ modal 對齊（C-V contract） | §6 流程閉環 |
...
〔data island 縱切〕
  User（rev2 016* 017）   Role（rev2 013* 016* 018）〔rev2 013 建 sys_role(m006)+sys_user_role(m007)+nick_name(m008)
                                                    +policy seed(rev2 m009) — Role 的 schema 起點在 rev2 013、非 016/018;
                                                    rev2 016 一 feature 兩 entity → 縱切應拆兩刀〕
  Menu（rev2 014〔runtime 讀〕 019 020 021 025）
〔行為島縱切（用 §4 鏡頭）〕
  Auth/Token/Session（rev2 013* 026 027 028 029 030）〔cleanup-job=本縱切 L8 binary〕
  Policy-governance（rev2 034 035）〔casbin_rule 治理欄(m031)+archive 表(m032)+sys_menu.protected(m034)
...
| 波 | 內容（素材 = §8.2） |
|---|---|
| 0 地基 | infra/deploy + envelope + soft-delete 基建 + audit 兩刀（op-log / access-log+xdb）——（rev2 001-012 + 015）對應 |
| 1 第一刀 | User **或** `system_settings` 打樣（**待決③、user 親決**）：migration→facade→handler→enforce→wire→frontend 全鏈 + §5 各面一次逼出 |
| 2 data islands | 其餘業務 entity 各一刀（User/Role/Menu；rev2 016 一 feature 兩 entity → rev3 拆兩刀） |
| 3 行為島 | §4 三台機器各自一刀：Auth/Token/Session・Policy-governance・Button/Endpoint policy（純 casbin policy、無新 entity） |
| 4 observability | obs-min(log) → obs-full(metrics) → dashboard，包覆全體 |
...
|---|---|---|
| policy 治理島形狀（§4.2） | 治理欄/archive 與 adapter 的互動形狀，不打樣只能猜 | rev2 把它當「又一張 casbin 表」defer 成獨立治理軌道，拖到 **rev2 034/035**（35 features 的最後兩個）才落地；原 defer 主因「soft-delete ⇒ 必 fork adapter」係未驗假設、rev2 034 親驗推翻（MILESTONES §1 `8e95fa1` row / DESIGN §10 Phase 3 #6） |
| wire 三端對齊（§7.2） | rust DTO ↔ typings ↔ component state 三端靠 grep 人工守，漂移無機器閘 | id/parentId string type-lie （rev2 016/017）一路無害接受，（rev2 025-I1）在 `menu-operate-modal.vue` 的 `parentId("0") === 0` 引爆成可觀察 data-corruption（DESIGN §9.1 / rev2 REVIEW-014-026 §4.1）→ 是否上 OpenAPI/code-gen 層 = 待決② |
...
| single-session 跨實例（§4.3） | pointer DB+Redis 快取 + pub-sub 收斂，多副本 / watcher 失聯情境單機驗不到 | rev2 已實證「DB 直改不 publish → in-memory 狀態脫鉤」：（rev2 035）live 測試須刻意不 publish 以免污染 running watcher（MILESTONES §1 `e841225` row）；watcher 韌性重訂閱 + `is_current` fail-OPEN 邊界是 rev3 多實例化前必打樣處 |
...
- **三者不可壓成一張圖**：rev2 把 Phase roadmap 同時當依賴圖 + 交付表用，治理島被排程綁架（「Phase 3 #6」編號上屬 Phase 3、實際在 Phase 4/5/6 全收完後的（rev2 034/035）才落地，DESIGN §10）——排程滑動被誤讀成架構依賴，正是三序分離存在的理由。

**縱切 vs rev2 Phase 對照**（同工作量、橫 phase → 縱 entity）：

| rev2 Phase 排序的問題點（INTEGRATION-DESIGN §10 實證） | rev3 縱切怎麼改 |
|---|---|
...
### §8.7 每刀的 SDD+TDD 工作流（rev2 CLAUDE.md §3 全文沿用；此處壓縮、rev3 不另立流程）
...
> 來源 = `.specify/memory/constitution.md` v1.6.0（Ratified 2026-05-28 / Last Amended 2026-06-06）。本章是 rev3 對 rev2 凍結權威的**承接快照**：§I 原則與 §II 拍板原則上整批 carry、僅標注 as-built 出入與 rev3 加嚴處；rev3 自己要新凍什麼（§3 schema / §5 面進不進 constitution）是「待決⑤」、本章不替 user 拍板。每個 feature 的 `/speckit-plan` 須跑 constitution §IV 的 8 題 Constitution Check，任一不過 → 回 brainstorm 或申請 amendment（§9.5）。
...
> 逐列抄自 constitution §II（2026-05-27 親決；原檔 §II 標題寫「12 項拍板摘要」、表實列 §11.1~§11.13 共 **13** 列，v1.0.0 起即如此——as-built 誠實並陳）。★=違反「不動 inline / build 配置」直覺的拍板。詳細理由見 INTEGRATION-DESIGN §11。
...
| 軌道 | 等級 | 授權範圍 | rev2 實例 |
|---|---|---|---|
| BASE-WEB-ADAPT | L1+L2 預設可動 | `.env*` + `src/typings/api/` 新檔（如 `rev2-extra.d.ts`）；新增為主、禁刪既有 type/field | `VITE_SERVICE_BASE_URL` 指 rust-api、dynamic route mode |
| BASE-WEB-WRAPPER | L3 預設可動（constitution §III.1） | `src/service/api/rev2-*.ts` 一律新檔；不改既有 `auth.ts`/`system-manage.ts`/`route.ts` | rev2 新增端點 wrapper |
| **BASE-WEB-BUILD-CONFIG ★ 已授** | L4 build infra | `build/plugins/router.ts` 加 `pageExcludePatterns`、**嚴格限隱藏 demo menu** | （已授權、as-built **未動用**——base-web 全樹無 `pageExcludePatterns`，rev2 022 plan Constitution Check 明記「本波不配」；且依 DESIGN §11.5 (b'-narrow)，`document`/`exception` 等 customRoutes 本非此機制可隱藏對象） |
...
- L 級語意（INTEGRATION-DESIGN §7）：L1=`.env` / L2=typings 新檔 / L3=service 新檔 / L4=build 配置或 view inline；★=需 constitution 顯式授權（本基線已授）。
- 注：DESIGN §7 一覽表標 WRAPPER「L3 需授權」、constitution §III.1 列其為**預設可動**——凍結權威為準，rev3 承接 constitution 版。
- ★ 軌道每改一處的 spec 義務：紀錄 file:line + 改動內容 + upstream 衝突風險評估；共用元件改動 MUST 用附加 prop + 安全預設（不變既有呼叫端行為）。

### §9.5 amendment 流程 + 版本規則
- **流程**（constitution §V.2）：提案（INTEGRATION-DESIGN §11 開新項：改哪節/為何/影響）→ **user 親決**（Claude 不主動 amend）→ 凍結（更新 constitution + bump version + 回填 DESIGN §11）→ **獨立 commit** `docs(constitution): amend <條目>...`。
...
- **JWT bearer 驗證鏈**（`auth/bearer.rs` → `auth/jwt.rs`）：`bearer_token`（case-sensitive `Bearer ` 前綴、trim、空 token=None）→ `jwt::verify`（HS256、`leeway=0` 杜絕剛過期仍過、驗 signature+exp+aud；`iss` 不驗——iss==aud=="rev2-admin" 故冗餘）。5 個 callsite 三種失敗策略（bearer.rs 文件化）：**fail-closed**（`enforce_mw`：verify 失敗→3333、但 role-lookup DB 失敗→5003 而非 3333）/ **advisory**（getUserInfo / getUserRoutes / isRouteExist：一律 3333 要求重新認證）/ **best-effort**（`audit_ctx` ctx_mw：None→`operator_id=NULL`、不擋請求）。
- **Super-only 治理端點（rev2 seed 實證，共 17 個、全部不 seed R_ADMIN/R_USER_COMMON；以下 mNNN 皆 rev2 migration）**：m021 menu-auth 4（getRoleMenu/updateRoleMenu/getRoleHome/updateRoleHome）・m022 button-auth 3（getAllButtons/getRoleButton/updateRoleButton）・m023 endpoint-auth 3（getAllEndpoints/getRoleEndpoints/updateRoleEndpoints）・m025 選單回收桶 2（getDeletedMenus/restoreMenu）・m029 settings 3（getSystemSettings/updateSystemSetting/updateUserSessionPolicy）・m035 policy 回收桶 2（getArchivedPolicies/restorePolicy）。另：user/role/menu **全部寫端點** + menu 讀端點亦 R_SUPER-only seed（m015/m017/m019/m020）。
...
- 對外（0.0.0.0）僅 front-nginx 單入口、TLS 終止於此（§11.1）；其餘 service 走內網 `rev2_net`，惟 postgres / redis-stack 於 prod 仍保留 `127.0.0.1` loopback host port（`25432`/`26379`，host 管理用）、rust-api prod 無 host port；rust-api `/metrics` 對外路徑被 nginx exact-match 擋 404（`_locations.inc`）。
...
- **AES-256 靜態加密**：rev2 無；取捨集中 **§2 新能力決策包**（待決⑥）——v1 建議磁碟/tablespace 層、欄位級 defer；面定義見 §5.10。
...
### §11.1 拓撲 / port / secret
- 單入口 front-nginx（TLS 終止 §10.2；`location /api/` 剝前綴轉 rust-api、`= /api/metrics` 擋 404，`deploy/nginx/conf.d/_locations.inc`）；其餘 service 不對外（0.0.0.0）暴露——dev 經 `127.0.0.1` loopback 直連，postgres / redis-stack 的 loopback 映射在 prod 仍保留（host 管理用）。compose project name = `rev2-admin`（卷名 auto-prefix 規約沿用）。
...
- `migration/` crate 35 個 migration、每個有對稱 `down()`；rev3 沿用此結構，且依 §3.4 紀律目標**零事後 ALTER**（建表即帶全審計欄 + 治理欄）。
...
> 實掃（`grep -rl 'rev2'`，2026-06-11；設定/部署/文件層、排除 `docs/` 歷史與 `specs/`）：外層 **16 檔**（骨架 v1 原記「13 檔」；漏計 `.gitignore`、`.dockerignore`〔源倉排除列〕、`README.md`〔stale 卷名〕，已校正。另 `tests/` 8 檔與 `.specify/wf-spec-review-006-013.js`〔硬寫 workspace 路徑〕含 rev2，屬測試/一次性腳本、刻意不入表）+ base-web worktree 5 檔 + rust-api worktree 7 檔 + `constitution.md`。`.claude/`（settings.json / hook-git-submodule-SOP.sh）經全目錄 grep **零觸點**、免改。

### A.1 外層 workspace（16 檔）

| 檔案 | rev2 觸點 | rev3 改法 |
|---|---|---|
| `.gitmodules` | `branch = rev2-admin-base-web` / `rev2-admin-rust-api`；rust-api `url = …/fork260509-rev2-anew-rust-api.git` | 分支改 `rev3-admin-*`；⚠️ rust-api 源倉名本身含 rev2 → 建議 rev3 重寫起新倉（或沿用倉、僅換分支） |
| `.gitignore` + `.graphifyignore` + `.dockerignore` | 源倉排除列 `fork260509-rev2-anew-rust-api/` + worktree 註解 | 隨源倉 / 分支命名同步 |
| `CLAUDE.md` | 68 行（branch 名、project name、port 表、§8 操作參考） | rev3 workspace 重寫此檔（非逐字替換） |
...
### A.3 短名 / 長名總表（沿 CLAUDE.md §1 命名分工）

| 層 | rev2 | rev3 |
|---|---|---|
| outer repo / default branch | `fork260509-rev2` / `rev2-admin-root` | ⚠️ 新倉名 / `rev3-admin-root` |
...
> **完整性五判準**（v1 §0.3 移此）：① 明示預設範式 ② 明示例外（行為島）③ 每部分用對鏡頭 ④ 凍結不變式 ⑤ as-built 誠實。**rev2 有 ④⑤、缺 ①②③**——這是它「能跑但設計書失準」的全部原因；下表即缺 ①②③ 的實證清單。

| 痛點 | rev2 實證（MILESTONES §1 / DESIGN §10 可查） | 對應原則（§0.2） |
|---|---|---|
| **DAG 失準 — 排程被當依賴** | 治理層 = DESIGN §10 **Phase 3 #6**，該 Phase header 自註「#6 受管 RBAC = 獨立 deferred 治理軌道」；實際以 **rev2 034（2026-06-09）/035（2026-06-09）** 落地，晚於 Phase 4 主流業務（rev2 016-025）、Phase 5（✅ 06-06）、Phase 6 觀察性（rev2 031-033）（✅ 06-08）**全部完成之後** — 「Phase 3」編號與真實依賴序 / 交付序徹底脫鉤 | ③ 三序分離（§1.5+§8）：依賴序凍結、交付序明示可變、不共用一張圖 |
| **行為島誤分類** | 治理層被當「又一張 casbin 表」混入 CRUD 排程；直到 rev2 034 落地才親驗 stock adapter column-scoped（`ptype,v0..v5`）、推翻「必 fork adapter」前提（DESIGN §11 翻案紀錄）— 行為密集模組用資料鏡頭設計，形狀最晚才定 | §0.1 行為島例外宣告 + §4 state-machine 鏡頭**第一輪**設計 + ④ 治理獨立一刀（§8.2） |
...
| **面被攤成增量 rollout** | soft-delete / audit 接線「隨各寫端增量完成」（DESIGN §10 Phase 2 餘）：soft-delete 分批（rev2 009/018/019）、audit Insert/Update 隨（rev2 017-020）補裝、Restore 隨（rev2 025） — 橫切面變成多 feature 的尾巴、覆蓋狀態靠事後盤點 | ② 面矩陣（§5）：設計一次、每 entity 繼承、§5.0 逐格打勾 |
...
|---|---|---|
| **全庫零 FK** | 12 表無任何 FOREIGN KEY（REVIEW-DATABASE 逐表 Notes 確認，連 `sys_user_role` join 表亦無；RI 全靠 application） | §3.1 ⚠️ 選擇性 FK（join 表加、log/token/casbin 維持零；**待決④**）；未加處義務集中明列 §3.3 |
...
| **code-guard → data-driven 遲到** | 種子選單保護初版 = in-code `is_seed_menu` 純函式（rev2 020 引入）+ base-web 硬寫 `SEED_MENU_ROUTE_NAMES`；**rev2 034** 才以 rev2 m033（標 16 列 casbin `protected`）+ m034（`sys_menu.protected`，標 7 列）退役 4 個 rust caller（US2）與前端硬寫清單（US5） | 保護屬性 = 資料（`protected` bool），建表即帶（附錄 F #3/#10；#11 archive 為 D-buffer 變體、無 protected 欄）；不寫 code 常數清單 |
| **migration 檔名零串易誤抄** | REVIEW-DATABASE 稽核時 6 表 8 條「指派路徑」多打一個 0 寫成 7 位數（如 `m20260529_0000028`），實檔為 6 位數（`m20260529_000028`）— 非 drift、純引用筆誤，但證明長零串檔名抄錄必錯 | 引用 migration 一律自 `migration/src/` 實檔名複製；⚠️ rev3 建議改短編號檔名（如 `mNNN_<name>`） |
...
| 本書章 | 來源（rev2 權威文件 §） |
|---|---|
| §0 前言與方法論 | 新作（由附錄 B post-mortem 反推；rev2 無對應章） |
| §1 範圍與系統大圖 | `MOCK-COVERAGE-AUDIT.md` + `INTEGRATION-DESIGN.md` §3（期望 API 全集）；§1.5/§1.6 = DESIGN §2/§4/§5 經 as-built 校正 + `Cargo.toml`/`package.json`/compose 實機 |
| §2 新能力決策包 | 新作（v1 骨架 §1.2/§3.6/§4.4/§5.10/§10.3 五處整併） |
| §3 資料模型脊椎（+附錄 F） | `REVIEW-DATABASE.md`（12 表 live DDL、零 drift）+ rust-api `migration/src/` 35 migration |
| §4 行為島模型 | rev2 `specs/026-030`（token / session）+ rev2 `specs/034-035`（治理）+ DESIGN §6、§11 拍板 / 翻案紀錄 |
| §5 面矩陣 | DESIGN §1（設計原則）+ §6；constitution §I.3（wire）/ §I.6（審計欄） |
| §6 前端 UI/UX | `MOCK-COVERAGE-AUDIT.md` + constitution §III（受管軌道）+ DESIGN §7 |
| §7 wire contract | DESIGN §3 + §9（一致性與風險紀律）+ `MOCK-COVERAGE-AUDIT.md` |
| §8 交付計畫 | DESIGN §10（實施階段）+ `INTEGRATION-MILESTONES.md` §1（實際時序）+ `CLAUDE.md` §3（工作流） |
| §9 凍結基盤 | `constitution.md` v1.6.0 全檔（§I 6 原則 / §II 拍板表〔header 記 12、實列 13〕/ §III 軌道 / §V amendment） |
| §10 安全與合規 | constitution §I.2/§I.5 + DESIGN §6（JWT/secret）+ rev2 `specs/013`、`specs/022-024` + migration seed 實掃 |
| §11 部署與運維 | DESIGN §8（部署拓撲）+ `docker-compose*.yml` / `deploy/` as-built + `CLAUDE.md` §8.2 |
| 附錄 A / B / C / F | workspace 實掃（2026-06-11）/ MILESTONES §1 + DESIGN §10-§11 / `REVIEW-DATABASE.md` + migration 源 / `REVIEW-DATABASE.md`（欄級字典） |

## 附錄 E — ✓ Google-5 覆蓋 gate

> 外部主題清單只作本附錄的一次性覆蓋檢查（借主題、不借順序），正文不再標注；「對映本書」欄已對齊 v2 章節結構。
...
> 全表零 drift（rev2 live-audit 2026-06-09，`REVIEW-DATABASE.md`）。`*` = rev3 v1 沿用 rev2、無新增表（§2 結論）。
...
## 附錄 G — 全書決策紀錄表（user 親決前不入凍結集）

| # | 決策點 | 工程預設 ⚠️ | 所在章 |
|---|---|---|---|
| 待決① | router 結構：維持 flat-in-main 還是重整 `router/` 樹 | 無強預設（as-built = flat + endpoint_coverage_lint 三源一致 @ 35，運作良好） | §1.5・§8.1 |
| 待決② | wire contract 機器化（OpenAPI / contract test / 維持 grep） | C 輕量 contract test 起步、B 視 endpoint 增速再評 | §7.2 |
| 待決③ | 縱切第一刀：User 直刀 vs `system_settings` 打樣 | 傾向 A（User 直刀） | §8.3 |
| 待決④ | 選擇性 FK | join 表（`sys_user_role`）加 FK、其餘維持零 | §3.1 |
| 待決⑤ | §3/§5 凍結邊界：哪些進 constitution、哪些留設計書 | 建議：archetype（§3.2）+ 行為島 invariants（§4）+ wire 碼表（§5.4/§7.3）入凍結；欄級字典（附錄 F）與常數值留設計書 | §3・§9 章首注 |
| 待決⑥ | 新能力 4 項（dashboard / reporting / AES / 合規） | v1 全 0 新表形（§2 表） | §2 |
| ⚠️a | 效能 / 可用性數字 | p95 300/500ms/1s；99.5%/月 | §1.3 |
| ⚠️b | 審計查詢讀端 + UI 補做 | 補（Super-only；矩陣已預標 ⚠️） | §1.2・§5.0 |
| ⚠️c | `/auth/error` | 維持不做 | §1.4 |
| ⚠️d | redis-stack image tag | 建 stack 當下即 pin 數字版 | §1.6 |
| ⚠️e | `5000` 的 HTTP status 配對 | rev3 拍板（200 信封 or 啟用 500 mapping）並入 contract test | §5.4・§7.3 |
| ⚠️f | 13 碼矩陣整組凍結（含 4 保留碼） | 沿用整組 | §7.3 |
| ⚠️g | constitution §I.5 `axum-casbin` 措辭 | rev3 重鑄為「enforce 層全新寫（in-tree）」 | §9.2 |
| ⚠️h | 排程性拍板重議（§9.3 表之拍板 §11.2/§11.8/§11.13） | 重議走 amendment、不默改 | §9.3 |
| ⚠️i | L4 授權模式 | 沿用「窄邊界 + 逐次擴邊」 | §9.5 |
| ⚠️j | rust-api 源倉 | 新倉（倉名含 rev2）；或沿用倉換分支 | 附錄 A |
| ⚠️k | migration 檔名 | 改短編號（`mNNN_<name>`） | 附錄 C |
| ⚠️l | settings 多 key 熱讀 | 需要時把單鍵 swap 推廣為 keyed map（設計變更、非預設） | §5.6 |

---

## 待討論 / 開放問題（user 親決；現況與證據錨）

1. **router 結構**（→ §1.5 校正③、§8.1 工序⑤）：維持 flat-in-main（as-built）還是復原 rev2 DESIGN §5.2 nested 意圖？
2. **wire contract 引入 OpenAPI/code-gen 嗎**（→ §7.2 選項並陳）？rev2 反覆 bug 源（rev2 025-I1 實證），值不值得這層投資。
3. **縱切第一刀**（→ §8.3 並陳）：User 還是更小的 `system_settings` 打樣？
4. **資料模型引入真 FK 嗎**（→ §3.1）？rev2 刻意零 FK 保 audit 歷史；rev3 非 audit 表是否加 FK。
5. **§3/§5 凍結邊界**（→ §3 章首、§9 章首注）：哪些進 constitution（不可動）、哪些留設計書（可動）？
6. **新增能力取捨**（→ §2 決策包）：user-facing dashboard / reporting(PDF/CSV) / 靜態加密 / 合規——rev3 要不要做？

> 全書 ⚠️ 工程預設彙整見附錄 G；任一拍板後：結論回填對應章、附錄 G 該列改「✅ 已決（日期）」。
