...
- **operator_ip INET 註記**:entity 模為 `Option<String>`、本 feature 永遠 None;`None→NotSet`(略過欄、避 PG 42804 `NULL::text` cast 錯)。真實 INET 值寫入(Phase 3 middleware)需 sea_query Expr cast 或 ipnetwork custom type。**as-built(015)**:015 已以新表 `sys_access_log`/`sys_login_attempt` 的 `client_ip` 欄證明 INET 真值寫入機制 = sea-orm `with-ipnetwork` feature + entity 欄型 `IpNetwork`(`IpNetwork::from(IpAddr)`、emit `'x.x.x.x/32'` typed INET、解 42804);`sys_operation_log.operator_ip` 本身的 retrofit(`Option<String>`→`IpNetwork`)留 Phase 4「A:資料變動補 operator」套同一 pattern(015 plan Deviation D-2、不碰 011 未寫路徑)。
...
3. **audit-middleware(015、前半)✅ 完成 (2026-05-30)**:全域 request-context middleware(`audit_ctx::audit_mw`、`from_fn_with_state` 掛全 Router、serve 改 `into_make_service_with_connect_info` 取直連 peer)建 `RequestContext{operator_id/client_ip/x_forwarded_for/region/trace_id}` 入 extensions;兩審計表 `sys_access_log`(已認證請求)+ `sys_login_attempt`(登入嘗試成敗 + 兩 lockout index)、append-only。「只記已認證」閘門(R2:middleware iff `operator_id Some` 才寫 access-log → 天然涵蓋 public/health/login/過期 token 不寫)+ 登入嘗試由 login handler 自記(D4)。來源忠實(D5):client_ip 直連 INET 真值、x_forwarded_for 原始鏈逐字(不做信任代理解析)、region xdb raw。best-effort(D6/FR-007:`best_effort_audit` wrapper、型別保證不傳播、寫失敗只 `tracing::warn!`、非 011 原子)。**§2.14 INET 機制解**(with-ipnetwork+IpNetwork、`IpNetwork::from(IpAddr)` emit `'x.x.x.x/32'` typed INET、解 42804)+ **§2.15 xdb runtime 首消費者**(`XDB_FILEPATH` env、dev `/app/xdb/resources/`/prod `/app/resources/`、prod runtime Dockerfile COPY `ip2region.xdb` ~11MB、boot `searcher_init(env XDB_FILEPATH)` 非 None、避容器 cwd panic)。守 008 envelope/013 login wire/009 lint(17)/007 FR-009。新 dep `uuid`(v4)+`with-ipnetwork`、無新 crate。三 US 活體 + server 69/3 單測 + prod build + xdb 打包。**後半(A 資料變動補 operator)/lockout/XFF-trust = 後續**。設計細節見 [`specs/015-audit-middleware/`](../specs/015-audit-middleware/);follow-up 見 [CHECKLIST §2.18](INTEGRATION-CHECKLIST.md)。
4. **Casbin redis pub-sub 啟用 feature** — `casbin:policy:invalidate` channel(即使單 instance,v1 啟用)
5. **policy seed feature** — 三 role × 主流 endpoint 的 Casbin policy migration seed（**013 已做第一刀**:migration 009 seed 示範路由 `/systemManage/getUserList` × {R_SUPER,R_ADMIN}〔不給 R_USER_COMMON 作 deny 證明〕、casbin_rule 維持 stock;**完整 3 role × 主流 endpoint 矩陣留此 feature**）
6. **axum-casbin 重寫 feature** — Casbin Axum enforce 中介層,rev2 自家 metrics / error / observability(**2026-05-29 012 brainstorm 從 Phase 2 §11.6 重定位至此**:中介層需真實受保護路由 + enforce 點 + observability 目標〔Phase 6〕才驗得了、整合得了,在 Phase 2 做會「無消費者、無法驗」)（**013 已做第一刀**:`auth/enforce.rs` rev2 自家 axum middleware〔最小機制、驗 JWT→per-role enforce→403+5003、單示範路由〕+ `Arc<RwLock<Enforcer>>` 並發無問題〔未用 CachedEnforcer〕;**本 feature = fuller rewrite**:全路由 rollout + metrics/error/observability 整合〔Phase 6〕）
7. **受管 RBAC policy 層 feature** — 在 casbin policy 上加 rev2 治理:**(a)** soft-delete 可復原 **(b)** 不可刪 protected policy(remove 守衛 + 受保護標記)**(c)** policy 變更走 [011 audit](#§64-sys_operation_log統一-audit)(`mutate_in_txn`,記 operator/何時/加或移除哪條)**(d)** 統一 CRUD facade。**需 fork 拷貝進來的 `sea-orm-adapter`**(改 `load_policy` 過濾 `deleted_at IS NULL`、改 `remove_*` 設標記)+ casbin_rule 屆時加 `deleted_at` + partial unique index + protected 標記。**動到 §11.6「adapter=拷貝」前提** → specced 時須評估 constitution Amendment。(**2026-05-29 012 brainstorm 衍生**:user 要 recoverability + protected + 變更軌跡 + 一致 CRUD;但 soft-delete 的「刪掉不再 enforce」、audit 的「誰改的」皆依賴 Phase 3 的 enforce/operator/policy 入口才驗得了,故與 axum-casbin 重寫同期)
...
1. **manage list endpoints feature** — `getRoleList` / `getAllRoles` / `getUserList` / `getMenuList/v2` / `getAllPages` / `getMenuTree`(全 read,對齊 mock)
   ...
   - **as-built(017-manage-menu-list,2026-05-31):Phase 4 第二刀 = menu 三件落地 + 建 `sys_menu` 選單表**(`getMenuList/v2` 分頁巢狀樹 / `getMenuTree` 輕量樹 `{id,label,pId,children}` / `getAllPages` **唯一 role-aware** 頁名 catalog:R_SUPER real+demo、R_ADMIN 只 real)。**建 `sys_menu` 表**(base-web `Menu` 全 21 欄、扁平 column + `route_ext`/`buttons` 兩 **JSONB**〔sea-orm `Json`/`.json_binary()`、011 先例〕、partial unique `route_name WHERE deleted_at IS NULL`、seed 6 節點對齊 014 `business_routes()`〔home + manage 父 + 4 children、parent_id subquery〕、m..016 create+seed / m..017 seed 6 endpoint policy〔getMenuList/v2·getMenuTree·getAllPages × {R_SUPER,R_ADMIN}〕)。3 條掛 `enforce_mw`、id wire=**number**、`route_name`=string〔casbin `v2='menu'` 對齊鍵、tree-prune 父 manage 不在 casbin〕。**getMenuList 非 016 DB 分頁** = 取全 active + 記憶體 `build_menu_tree` + 頂層 slice(D-A:menu 巢狀有界小);**getMenuTree** reuse 同 nesting(lite 版、無 order 欄保 menu_order 序);**getAllPages** handler 自讀 `Claims.roles` 做 demo 過濾(double-JWT-parse、§2.17 defer、D-B)。**唯讀**;**不動 014 getUserRoutes**〔仍讀 in-code、雙源並存 D-D〕;**可見性續住 casbin**〔不建 sys_role_menu〕;**demo 頁只進 getAllPages catalog 不 seed 成 menu**〔Path A、D-E〕;createBy/updateBy/buttons 回 null(operator/按鈕折後續、D-E)。`REAL_PAGES`(5)+`DEMO_PAGES`(38、對齊 base-web `routes.ts`、§I.5 未 grep rev1、D-C)。**無新 crate/dep、不動 base-web、無外層 deploy 改動**。純測 14 個(build_menu_tree 巢狀/排序/孤兒 promote/葉省略 children + route_ext 攤平 null + paginate_roots root-slice + build_menu_tree_lite + pid serde + pages_for_roles Super43/Admin5/empty5 + facade SQL-build) + 三 US 活體(分頁巢狀樹 + 輕量樹 + Super⊋Admin 差集38 + enforce 403+5003/200/3333) + server **112**/3 + lint 17 + prod build。Constitution **7+7=14 PASS、無 violation、無 Amendment、無 §I.5 例外**(比 016 乾淨)。Deviation D-A〜D-E(實作層折期、非鐵紀律偏離)。subagent-driven 13 task、各 spec+quality 雙審 + final review = READY TO MERGE。詳見 [specs/017](../specs/017-manage-menu-list/spec.md) + [MILESTONES](INTEGRATION-MILESTONES.md)。
   - **as-built(018-userroutes-sys-menu,2026-05-31):Phase 4 第三刀 = getUserRoutes 改讀 `sys_menu`(拆 D-D 單一真相源)**。純函式 `build_route_tree(Vec<MenuNode>)→Vec<MenuRoute>`(`MenuNode`→`MenuRoute` 對映 + parent_id 巢狀 + `menu_order` 升冪 NULLS LAST + None component→`""`)+ `filter_routes_for_roles` 純化為吃 `(routes, roles, enforcer)`(Casbin prune/enforce body 逐字不變、只換 source + param)+ **移除 in-code `business_routes()`**(資料移入 test `fixture_tree()`)。handler `get_user_routes`:`sys_menu::all_active`→inline map `MenuNode`(`route_ext->>props` 抽 bool)→`build_route_tree`→`filter_routes_for_roles(tree,..)`;`{routes,home:"home"}` + DbErr→3333 契約不變。**設計抉擇 Option C**:`build_route_tree` 吃 menu.rs 本地 `MenuNode`、**非 `entity::Model`** —— 沿用 017 慣例避 entity-access lint(route/handler 不寫 `entity::` token、**不動 facade**);data-model §4 原寫 `Vec<Model>`,2026-05-31 user 拍板改本地 DTO。**US2 前置**:forward-only seed 修正 migration `m..018` 補 `manage_user-detail` `component=view.manage_user-detail` + `route_ext={"props":true}`(016 seed 此列原 NULL/NULL、回 base-web 權威 R1/R2)。**唯讀**(FR-008、main.rs 未動、getUserRoutes 路徑 0 處 operator/`*_by`)、**wire 對 014 baseline(`8965c8a`)零差異**(`MenuRoute`/`RouteMeta` struct 未變、None 欄省略、三角色逐項同)、**可見性續住 casbin**(menu-visibility policy 9 rows 不動、route_name=v2 對齊鍵)。**無新 crate/dep、不動 base-web、無外層 deploy 改動**。9 個 `build_route_tree` 純測 + 6 個 filter 測改餵 `fixture_tree` + 三角色 curl 活體 + 重跑 014 CDP smoke;server **119**/3 + lint 17。Constitution **8/8 PASS、無 violation/Amendment/§I.5 例外**。rust-api `d4cb09e`(US2)+`85f95f1`(US1)。詳見 [specs/018](../specs/018-userroutes-sys-menu/spec.md) + [MILESTONES](INTEGRATION-MILESTONES.md)。
