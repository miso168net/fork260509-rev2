...
### §6.6 sys_user 單一-session(028 access 端 stateful 撤銷)

**✅ 028-single-session-enforcement 落地(2026-06-06、rust-api `d958c02`)— as-built**:
- **schema(migration `m20260529_000027_alter_sys_user_session`,ALTER 非 CREATE)**:`sys_user` 加 2 系統欄 `current_session_id VARCHAR(36) NULL`(pointer 持久真相、login 寫、未登入=NULL)+ `session_policy VARCHAR(20) NOT NULL DEFAULT 'inherit'`(三態 inherit/on/off)。entity +2 欄;§I.6 六審計欄規則前提(業務表 CREATE)不觸發、PASS-by-scope。up→down→up 可逆、既有資料 DB default 自動回填 inherit。
- **config**:`SessionMode{On,Off}` + `JwtConfig.single_session_default`(env `APP_JWT_SESSION_DEFAULT`、tolerant parse〔unset/空/未知→Off〕、**028 預設 Off = dormant 上線**;029 改 runtime store)。
- **純邏輯 seam(唯一單測 seam,FR-015)**:`resolve_policy(session_policy:&str, system_default:SessionMode)->bool`(on→true / off→false / inherit+未知→`system_default==On`;TDD red→green U1 全分支×兩系統預設)。
- **`auth/session.rs`(新)pointer 混合存取**:`PointerRec{session_policy,sid}` JSON + `sess_key(uid)` 單一 key 源 + `cache_set`(best-effort Redis SET、err 吞)+ `cache_get`(GET→deserialize、corrupt JSON→當 miss)+ `load_pointer_from_db`(sys_user fallback、DbErr→None)+ `set_pointer`(**persist-then-cache**:`sys_user.current_session_id` must-succeed→Redis best-effort)+ `is_current`(Redis→sys_user 混合 lazy rehydration、`resolve_policy` off→**true 不踢**、on→`sid==pointer`、**I/O 失敗 fail-OPEN=true**〔附加收斂、非主授權閘;主閘 enforce_mw casbin+DB-fresh roles 仍 fail-closed〕)。
- **facade**:`sys_user::set_current_session`(唯一寫管道、update_many col_expr、系統欄**無審計成對**)+ `sys_token::revoke_other_chains(user_id,keep_chain)`(沿 Reuse 整鏈 revoke、`.ne(keep) AND status=active AND user_id`、回 affected)。
- **Claims**:+`sid:String`(**required**、internal、base-web opaque);`sign`/`issue_tokens` +`session_id` 參(access+refresh 兩 token 同 sid)。
- **handler 串接(wire 中性)**:`login` 鑄獨立 `sid`(uuid、與 027 rotation_chain 正交)→ issue_tokens → create_chain_head →（resolved=on）`revoke_other_chains(keep=chain)`→ **一律** `set_pointer`（即使 off、供日後切 on;DbErr→`5000`）;`login_attempt_inner` 回 `session_policy` 第三欄（免二次查）;`refresh_token` **pointer-first** `is_current`→`7777`（在 `issue_tokens` 之前、I2）+ 繼承 `claims.sid`（027 rotate 不變）;**4 認證 gate**（`enforce_mw`/`get_user_info`/`get_user_routes`/`is_route_exist`）verify 後注入 `is_current`→false→`ModalLogout7777`（**ctx_mw 不加**）。
- **wire 中性 + 失敗碼**:login/refresh 成功回應逐字不變;踢一律 **`7777`**「账号在他处登录」（base-web `MODAL_LOGOUT_CODES` 現成→modal→乾淨登出、排 expiredToken 前無迴圈）;被踢/失效路徑**絕不** `3333/9999/9998`（refresh 被踢→`7777` pointer-first 或 `8888` 鏈已 revoke、皆合規）;pre-028 無 sid token → `Claims.sid` required → `jwt::verify` deserialize 失敗 → `3333`/`8888` → 一次性重登（FR-011、與 7777 正交）。**base-web 零改、無新對外端點**。
- **acceptance 全綠**:純單測 U1（resolve_policy 全分支）+ U2（sid round-trip）+ pre-028-reject;**live-DB L1-L5**（踢舊／off 不踢／混合回填／撤鏈／**fail-open**、in-crate #[ignore] Postgres+Redis、--test-threads=1）;**curl C1-C4**（policy=on 踢 `7777` 全 4 gate + refresh／off 多裝置並存／失敗碼紀律無 3333/9999/9998／正常 refresh `0000` 不誤踢、sid 跨換新存活）;**CDP isolated-context modal smoke**（「账号在他处登录」彈窗 + 確認→`/login` + `dialogCount`=1 不重複〔SC-002〕 + off 兩分頁不踢）;守恆（server **221** 純測〔220 + pre-028-reject〕 + 5 新 live #[ignore] + `entity_access_lint` **17** + `endpoint_coverage_lint` **30**〔無新端點〕 + `Migrator::up` **0** + migration up→down→up 可逆 throwaway DB〔既有資料不動〕）+ **prod image build 綠**。
- **無新表/crate/dep/fork**（`redis`/`uuid`/`chrono`/`serde_json` 皆既有 server dep；redis 從只 PUBLISH 加 GET/SET 用法）。**無 amendment**：access 端 stateless→可被 session 撤銷 = §11.17（018 enforce DB-fresh stateful）同軸延伸；`sys_user` 系統欄 §I.6 PASS-by-scope。Constitution **v1.5.0** §IV **8/8 PASS**（plan + post-design re-check）。subagent-driven-development T002-T021 逐單元 fresh implementer + spec compliance（對 spec.md）+ code quality 雙審 + final holistic = READY TO MERGE（FR-001..015 全 trace、8 不變式）。**設計細節見 `specs/028-single-session-enforcement/`**。
- **029-single-session-admin-ui**：028 policy 的 admin 管理 UI（系統預設 runtime store + 每帳號 policy UI）✅ **已落地(2026-06-06、rust-api `27c03de` + base-web `75f2a20`)— as-built 見 §6.7**。

### §6.7 system_settings KV + single-session admin 控制面(029、疊在 028 之上)

**✅ 029-single-session-admin-ui 落地(2026-06-06、rust-api `27c03de` + base-web `75f2a20`)— as-built**:028 把單一-session 引擎與 policy 儲存做好、但「開關」只能後端設(系統預設改 config 重部署、每帳號 policy 直接動 DB);029 讓系統操作者(Super)經 UI:(a) runtime 調整全站預設(不重部署、下個請求即生效)、(b) 使用者管理頁設每帳號 policy。**不改 028 enforcement 邏輯**(`resolve_policy`/`is_current`/4 gate/refresh 行為逐字不變、只換系統預設值的「來源」:immutable config → runtime store)。

- **schema(migration `m20260529_000028_create_system_settings`,CREATE)**:通用「系統設定」KV 表 `system_settings`(`setting_key VARCHAR(64) PK / setting_value / value_type〔如 "enum:on,off"〕/ description NULL` + §I.6 **六審計欄真適用**〔人類 admin 編輯、非 028 系統欄 PASS-by-scope〕)+ seed 首列 `('single_session_default','off','enum:on,off',…)`。entity `system_settings.rs`(String PK `auto_increment=false`)。`single_session_default` 是第一個 setting、日後系統級開關共用此表。
- **runtime 熱路徑**:`AppState +session_mode: Arc<RwLock<SessionMode>>`(獨立欄、仿 enforcer);main.rs boot 從 `system_settings::get` 載入(缺列/查詢失敗→config `jwt.single_session_default` 安全預設、空值→typed accessor fail-safe Off;FR-009 store 不可達仍啟動);`is_current`(`session.rs:190`)+ login(`handler/auth.rs:150`)由 `state.jwt.single_session_default` 改讀 `*state.session_mode.read().await`(028 邏輯不變、只換來源)。
- **settings_watcher(`auth/settings_watcher.rs`,鏡像 `policy_watcher`)**:channel `"settings:invalidate"`、`spawn_settings_watcher(client, session_mode, db)` 韌性重訂閱 loop、收訊從 DB 重讀 `single_session_default`→write-lock 更新(Ok(None)/Err 保留現值)、`publish_settings_invalidate`(PUBLISH)。多副本一致(對齊 027/021 pub-sub 範式)。
- **facade**:`system_settings`(唯一寫管道、守 009):`get_all`/`get`(濾 soft-delete)/ `update`(`mutate_in_txn` + 011 Update + §I.6 updated_at/by 成對)+ pure `parse_session_default(value)->SessionMode`(TDD U1 seam、on→On / 其他→Off fail-safe);`sys_user::update_session_policy`(只動 session_policy 欄、絕不碰 current_session_id、011 audit;sys_user `audit_json` 補 session_policy 使 before/after 見新值)。
- **3 Super-only 端點(count 30→33、4-site byte-identical 對齊)**:`GET getSystemSettings`(列設定 camelCase)/ `POST updateSystemSetting{key,value}`(**FR-013 write-side 依 value_type 驗、非法 2222 不持久** → facade update → `publish_settings_invalidate`,handler 只 publish、watcher reload)/ `POST updateUserSessionPolicy{userId,policy}`(三態驗 → facade → **DEL `sess:{uid}`** 經 `session::sess_key` 單一源,使 028 `is_current` 下個請求 lazy-rehydrate、不被 PointerRec cache 遮)。`getUserList` additive `sessionPolicy`(§I.3 ADAPT 超集、唯一既有 wire 漂移面、FR-010)。
- **casbin + sys_menu seed(migration `m20260529_000029_seed_settings_admin`)**:3 endpoint p-policy(R_SUPER)+ sys_menu `manage_system-settings`(`route_path /manage/system-settings`、`component view.manage_system-settings`、order 4)+ menu casbin(R_SUPER);選單走 §I.2 Casbin enforce、非-Super `getUserRoutes` 濾除(curl 實證 Super 見、Admin/User 不見)。
- **base-web(雙倉、MODAL-WIRING ★ (e) v1.6.0 ratified `3bd3eda`)**:新 `views/manage/system-settings/index.vue`(on/off NSwitch + 後果提示、`fetchGetSystemSettings`/`fetchUpdateSystemSetting`)+ user 頁 `sessionPolicy` 欄(status 後 operate 前 NTag)+ 抽出 `modules/user-session-policy-modal.vue`(3 態 NSelect、`fetchUpdateUserSessionPolicy`)+ typings(`SystemSetting`/`User.sessionPolicy`)+ rev2 service wrapper ×3 + i18n(en/zh + app.d.ts Schema)+ elegant-router 自動生成(route name 對齊 m029)。
- **acceptance 全綠**:pure U1(typed accessor)+ U2(SystemSetting camelCase)+ `value_in_value_type`(FR-013)單測;**live-DB L1-L4**(settings facade+audit / update_session_policy 只動 policy 欄+audit / settings_watcher pub-sub reload / boot load 兩分支、in-crate #[ignore] Postgres+Redis);**curl C1-C6+C3b**(Super 0000 / Admin·User 5003 / runtime toggle off→on 踢 `7777` 不重啟 / FR-013 `maybe`→2222 原值不變 / per-account on→`7777` + `EXISTS sess`=0 / getUserList sessionPolicy / getUserRoutes 選單 Super-only);**CDP isolated-context 雙頁 smoke**(設定頁 switch→「设置已更新」+ DB 翻轉 / 使用者頁 policy 欄+action+modal 3 態);守恆 server **224** + lint **17**/**33** + `Migrator::up` 0 + migration m028/m029 up→down→up 可逆〔manage_user 無連帶〕 + base-web typecheck 0 + prod build。
- **無新 crate/dep/workspace member**(redis/sea-orm/serde/chrono 既有)。**唯一 amendment**:MODAL-WIRING ★ (e) 同 manage 範式新管理頁(v1.5.0→v1.6.0、§11.24、ratified `3bd3eda`、user 親決;MINOR 軌道授權邊界擴展、非新軌道)。Constitution **v1.6.0 §IV 8/8 PASS**。subagent-driven T002-T021 逐單元雙審 + final holistic 三維 = READY TO MERGE。**設計細節見 `specs/029-single-session-admin-ui/`**。
...
| **Phase 6(production-ready)** | **啟 obs-full**(+ prometheus + 2 exporter〔postgres/redis〕 + pushgateway + grafana prometheus datasource/alerting;**已落地 — 見 §10 Phase 6 #2 as-built**) | 2-3 人日 | 性能監控、alerting 需求 |
...
### Phase 3 — 認證 + 動態選單(✅ 核心完成 2026-06-05;#5 axum-casbin metrics 尾 ✅ 032-obs-full、#6 受管 RBAC = 獨立 deferred 治理軌道)
...
2. **dynamic mode 路由 feature** — `/route/getConstantRoutes` + `/route/getUserRoutes`(含 `home` 欄)+ `/route/isRouteExist`（**✅ 014-dynamic-routes 已交,2026-05-30 merge `--no-ff` 回 `rev2-admin-root` / merge `9d06347` / SHA pin `8965c8a`**:base-web 翻 **dynamic auth route mode**(`.env` `VITE_AUTH_ROUTE_MODE=static→dynamic`、BASE-WEB-ADAPT)、由 rust-api 供應 3 route endpoint,**業務 menu 可見性走 Casbin enforce 過濾**(§I.2 核心原則落地、重用 013 enforcer)。**3 endpoint**:`GET /route/getConstantRoutes`〔**公開、無認證、無 middleware**、回程式內 constant 5 條、SPA reload 即取〕/ `GET /route/getUserRoutes`〔JWT、**不掛 enforce middleware、過濾在 handler 內**:`bearer→jwt::verify〔access secret+JWT_AUD〕→roles_for_user〔DB 權威〕→menu::filter_routes_for_roles〔enforcer read lock〕→{routes,home:"home"}`;token/roles 錯→`3333`〕/ `GET /route/isRouteExist?routeName=`〔JWT、`menu::route_exists_for_roles`、依角色 allow+deny〕。**route 定義程式內寫死**(`server/src/route/menu.rs`:`MenuRoute{id:string=name,name,path,component,meta:RouteMeta,children?,props?}`/`RouteMeta`/`UserRoute{routes,home}`,**serde camelCase + skip_serializing_if + 省 meta.roles**〔dynamic mode server 已過濾〕;`constant_routes()`〔403/404/500/login/iframe-page,component `layout.blank$view.*`/`layout.base$view.iframe-page`、meta.constant=true〕+ `business_routes()`〔home 葉 + manage 父〔`layout.base`〕含 4 children manage_user/role/menu/user-detail〕,**component 精確對齊 base-web `elegant/routes.ts` 的 elegant-router `$` 複合格式、§I.5 未 grep rev1**〔最高風險點、CDP smoke 抓〕)。**menu 可見性走授權引擎**(D-001 re-scope:初版 brainstorm 程式內 `role→route` map 被 plan Constitution Check item 3 抓違反 §I.2 → user 拍板改):migration `m20260529_000010_seed_menu_policy` seed menu-visibility policy `p,<role>,<route_name>,menu` 進 `casbin_rule`〔**9 rows** = D3 矩陣 Super5+Admin3+User1、`ON CONFLICT DO NOTHING`、沿 009 寫法、經 010 自動套、**父層 manage 不 seed**;`act="menu"` 與 013 endpoint policy `act=method` 共存於同表同 RBAC model、**model 不改**〕+ `filter_routes_for_roles`/`route_exists_for_roles` 用 013 enforcer `enforce((role,route_name,"menu"))`〔任一 role allow 即可見〕+ **tree-prune**〔父層 manage:子項任一可見則保留、children 只留 visible、否則整個 omit;home 葉直接判〕。**D3 三階梯**:Super 全〔home + manage〔user/role/menu/user-detail〕〕/ Admin home + manage〔僅 user + user-detail〕/ User 只 home〔manage tree-prune omit〕。**測試**:enforce 決策 + tree-prune **test-first 6 新單測**〔`MemoryAdapter` seed 同 9 條 menu policy + inline 同份 production RBAC model:Super 全集/Admin 部分/User 只 home/route_exists allow+deny/unknown false/multi-role union〕;wiring 由活體 acceptance 覆蓋。**acceptance 全綠**:三 endpoint curl 活體〔getConstantRoutes 5 條公開、getUserRoutes 三角色不同 + `home="home"` + token 缺/壞 `3333`、isRouteExist Super manage_role→true·**User manage_role→false〔deny〕**·home→true、psql `v2='menu'` 9 rows + `v2='GET'` 2 不受影響〕+ **CDP dynamic-mode 瀏覽器 smoke**〔9229 自建 tab、reload→getConstantRoutes 觸發→登入 Super〔側欄含「系统管理」〕vs User〔側欄**只「首页」**= menu deny 端到端證明〕、getUserRoutes 200 + 兩角色側欄不同〕。**dynamic mode 正面副作用:getUserRoutes 只送業務 route → demo menu(function/plugin/alova/document/multi-menu)根本不送 → §11.5/BASE-WEB-BUILD-CONFIG ★〔隱藏 demo〕在此模式 moot、本 feature 不需動**。守 007 FR-009〔server 不自動 migrate、grep 0 命中〕+ 009 entity-access lint〔route 模組/handler 不碰 `entity::`、roles 經 facade、17 passed〕+ 008 envelope〔全 `Res<T>`〕+ §I.2/§I.3。**無新 dep/表/crate**〔重用 013 enforcer/jwt/bearer/roles_for_user + 008〕;prod runtime image build sanity 驗綠;既有不破〔server 50+3→**56+3**〔+6 filter 測試〕+ xdb 9 + lint 17〕。**兩段式 commit**〔rust-api worktree 7 commits `f65105e..c4b1d7e` push fork〔含收尾移除 mod route 過時 `#[allow(dead_code)]`〕+ base-web `.env` dynamic `41f44b18` push fork + 外層 SHA pin〕;subagent-driven-development 13 task、各 unit spec+quality 雙審 + opus final holistic review = Ready to merge;Constitution 7+7=14 ✅ + Deviation Log **D-001**〔§I.2 re-scope:in-code map→Casbin policy seed+enforce〕/**D-002**〔menu policy row 文件計數 8→9 修正、D3 矩陣權威〕。**設計細節見 [`specs/014-dynamic-routes/`](../specs/014-dynamic-routes/)**;CDP harness `tests/014-dynamic-routes/`。**scope 外〔皆後續〕**:sys_menu 表/menu CRUD、全路由 endpoint enforce 矩陣〔#4〕、policy 治理〔#6〕、redis pub-sub 失效通知〔#3〕、demo menu）
   - **★ audit-middleware feature（015-audit-middleware,merge `589a553`〔rust-api worktree 9 commits `a736072..cff9785` push fork + 外層 deploy/compose + SHA-pin `cff9785`〕,2026-06-01）** — 在 013/014 認證鏈之上補**請求脈絡擷取 + 稽核留存層**:落地 **request-context middleware `ctx_mw`**〔BEFORE 段擷取脈絡置入 request extension、AFTER 段於 `next` 回程組 access-log row 寫入,best-effort 不破業務〕+ **2 張 append-only 審計表** `sys_access_log`〔已認證請求恰一筆,operator 閘門 = FR-001/FR-002:公開/未認證路由(常數路由、login、`/health`)與認證失敗請求皆不留;內容含 operator_id/method/path/status/client_ip/region/forwarded-chain/trace_id/created_at〕/ `sys_login_attempt`〔每次登入嘗試恰一筆〔成敗皆然〕、由 login 介面寫入〔唯它知業務成敗 FR-004〕、**2 index 支援「某帳號/某 IP 時窗失敗次數」查詢** FR-008,為未來帳號鎖定備料〕。兩表 append-only〔不可變事件、**無 `deleted_at`、不 impl SoftDeletable**、FR-009〕、entity `impl AuditSerialize`。
     - **首個 xdb 消費者**:`ctx_mw` 以直連對端來源 IP 解 region〔xdb `ip2region.xdb`、`XDB_FILEPATH` env、prod `deploy/Dockerfile.rust-api.txt` builder 須 `COPY ip2region.xdb`;**僅由直連 IP 解地、不解轉送鏈推「真實 client」** FR-006〕,把 012 拷入的 `xdb` crate 第一次接上 runtime path。
     - **首個真值 INET client_ip（解 011 伏筆）**:client_ip 改存**真實可查詢網路位址值**〔`ipnetwork::IpNetwork` + sea-orm `with-ipnetwork`、ipnetwork 0.20、FR-007〕,落實 011 留的「`Some(ip)→Set` INET 真值寫入」伏筆〔011 因永遠 None 而 `None→NotSet` 略過欄、避 PG 42804 INET cast 錯;015 middleware 帶真值 IP 後正式寫穿〕。原始轉送 header 逐字保存〔未帶為空 FR-005(b)〕、trace_id 入站帶請求識別 header 則沿用、否則生成 uuid。
     - **acceptance（全綠）**:subagent-driven 19 task/3 US,各 spec+quality 雙審 + opus final holistic review;server 65 + entity_access_lint 17 + xdb 9 全綠 + dev/prod acceptance 全綠〔US1/US2/US3 + SC-004 + **prod image build region 非 NULL**〕。**無新端點/表 schema 外的 crate**〔重用 012 xdb、新增 ipnetwork dep〕。Constitution v1.1.0 §IV 8/8 ✅;015 branch 保留。**設計細節見 [`specs/015-audit-middleware/`](../specs/015-audit-middleware/)**;follow-up CHECKLIST §2.19。

3. **Casbin redis pub-sub 啟用 feature** — `casbin:policy:invalidate` channel(即使單 instance,v1 啟用)（**✅ 021-manage-menu-auth 順帶交付,2026-06-03 merged `827d1e9` 回 rev2-admin-root〔--no-ff、保留 021 branch〕+ 已推 origin**:MenuAuth 的 `set_role_menu` runtime 改 casbin menu policy 後 PUBLISH `casbin:policy:invalidate`〔消費此前 dead_code 的 `AppState.redis` ConnectionManager、消 `field redis is never read` warning〕;boot `spawn_policy_watcher` 以 `client.get_async_pubsub()` **獨立 pubsub 連線** SUBSCRIBE → 收訊取 enforcer write lock `load_policy()` reload〔error-log 不靜默、5s backoff 重連、不 panic〕。**+1 direct dep `tokio-stream`**〔redis 1.2.1 async pubsub 僅給 `Stream`、逐則收訊須 `StreamExt::next()`;tokio-stream 已是 Cargo.lock transitive `0.1.18`、宣告 direct 僅暴露既有 crate、實作期親決〕。單 instance 自收冪等〔in-place 改 + 自收 reload 讀同態 DB,前提 auto_save 同步〕;publish 失敗 warn-only 不擋請求〔local 改已生效〕;**home 編輯不 publish**〔sys_role 欄、每次 getUserRoutes 從 DB 讀新值、無 enforcer cache〕。多 instance 真實 reload 留 follow-up。實現 §6.3「一致性優先、v1 即啟用」)
4. **policy seed feature** — 三 role × 主流 endpoint 的 Casbin policy migration seed（**013 已做第一刀**:migration 009 seed 示範路由 `/systemManage/getUserList` × {R_SUPER,R_ADMIN}〔不給 R_USER_COMMON 作 deny 證明〕、casbin_rule 維持 stock;**完整 3 role × 主流 endpoint 矩陣留此 feature**）（**✅ 023-manage-endpoint-auth 交付此 #4,2026-06-04 merged `ef5ebe0` 回 rev2-admin-root〔--no-ff、保留 023 branch〕+ 已推 origin**:grounding 對抗驗證揭露「**全路由 enforce 字面早已完成**」〔25/25 enforce_mw route 皆有 ≥1 seed policy、零破口〕→ 本波**重定義交付 = runtime 可編輯 + 治理 + 防呆守衛**,非「補未把關端點」。**① runtime 可編輯**:新 `auth/endpoint_auth.rs`〔1:1 鏡像 022 button_auth,唯一 divergence = endpoint 列 keyed on `v2=HTTP method`〔變值,非常數 'button'〕故 HARD REPLACE **逐 method `remove_filtered_policy(0,[role,"",m])` for m∈{GET,POST,DELETE}**、**絕不裸空-v2**〔否則誤刪 v2='menu'(021)/v2='button'(022)列、破正交〕→ `add_policies` 每 endpoint 一列 `[role,path,method]`、空集 Ok(false);6 純單測證 multi-method HARD REPLACE 對 menu/button **正交** + 空集清除 + (path,method) 排序穩定〕+ 3 Super-only handler 入既有 `handler/system_manage.rs`〔`getAllEndpoints` 回 `ENDPOINT_REGISTRY` const〔**28 = 25 既有 + 3 治理**〕**infallible**〔無 DB/無 Err arm,結構差異 vs get_all_buttons〕、`getRoleEndpoints` 預載〔**R_SUPER→全 registry**、root 全通〕、`updateRoleEndpoints` HARD REPLACE + 011 audit〔casbin_rule、operator + before/after endpoint 集〕+ 共用 021 redis pub-sub watcher 即時跨實例〕。**② root-mode**〔自鎖歸零〕:`update_role_endpoints` 拒編輯 R_SUPER〔bare literal `code=="R_SUPER"`、**非** `is_seed_role_code`〔會誤擋 R_ADMIN/R_USER〕〕→ 2222「不可编辑超级管理员」、非法 endpoint〔(method,path)∉ registry〕→ 2222「接口不存在」。**③ D1 build-time 靜態守衛**〔`server/tests/endpoint_coverage_lint.rs`、model on entity_access_lint、重用 strip_comments_and_strings;server bin-only 故 **text-parse** main.rs/migration/endpoint_auth.rs〕:斷言 main.rs enforce_mw routes ↔ migration seed policy ↔ ENDPOINT_REGISTRY **三方一致**〔每 enforce_mw (path,method) 有 ≥1 seed → 關 path-typo/漏 seed 靜默死路由破口〕+ negative fixtures 證咬合〔含 009-form down-statement `v0 IN(…) AND v2='GET'` 不 false-seed 的 regression pin〕;**靜態不發請求 → 繞過 enforce token-vs-policy 陷阱**〔R5:匿名請求先吃 3333、誤 pass〕。**④ 接口权限 modal**:新 `endpoint-auth-modal.vue`〔鏡像 button-auth-modal、NTree key=composite `"method path"`、checks:string[]、`watch(visible)` 重載、root-mode 對 R_SUPER 全勾+NTree/confirm disabled〕+ role-operate-drawer trigger〔**MODAL-WIRING ★ v1.4.0 (c) 子句授權**、§11.21〕+ 3 fetch fn〔BASE-WEB-WRAPPER〕+ i18n `endpointAuth`。**migration 023 只 seed 3 治理端點 R_SUPER 列、無新 crate/dep/fork/表**〔§I.6 gate N/A〕;up→down→up throwaway DB 可逆〔menu 15/button 10/既有 endpoint 存活、僅 3 列精準增刪〕。**⑤ 矩陣 reconcile**〔§11.22〕:DESIGN §4.6.3/§6.3 過時 `p,R_SUPER,*,*` wildcard 文字校正為**逐 endpoint**〔matcher exact-equality `enforce.rs:41`、wildcard 從未實作、016 as-built 以來 de-facto〕+ menu-read 三列〔getMenuList/v2·getAllPages·getMenuTree〕R_ADMIN ✓→✗ 對齊 **Super-only 實作**〔FR-007 單一真相;Admin 可 runtime 經 modal 獲授〕。**acceptance 全綠**:contracts §1–7 curl/psql/redis/audit〔getAllEndpoints 28+Super-only〔Admin 5003/none 3333〕·getRoleEndpoints 預載+root 全通〔Super 28〕·updateRoleEndpoints round-trip **即時**〔grant getMenuTree→Admin 200→還原 403、無重啟〕·root-mode 2222·非法〔含合法 path 錯 method〕2222·roleId number|string·Admin 5003·011 audit operator_id+before/after〕+ **CDP isolated-context smoke**〔① 非-Super modal 可編輯+endpoint tree·② root-mode Super 全勾+disabled,`tests/023-manage-endpoint-auth/`、未擾 user tab〕+ migration 可逆 + 守恆〔server **195 unit**〔含 endpoint_auth 6〕+ **D1 lint 5** + entity_access_lint 17 全綠、零 `entity::` token、`Migrator::up`=0〕+ **回歸 013/016/019/021/022**〔enforce 階梯·menu(021)·button(022) policy 不破、既有 endpoint 矩陣不變 FR-010〕。**Constitution v1.4.0 §IV 8/8 PASS**〔MODAL-WIRING ★ amendment `d700434` + §11.21〕。**兩段式 commit**〔rust-api worktree:migration 023 / endpoint_auth+單測 / 3 handler+route / D1 lint;base-web worktree:wire / modal+drawer;outer:tests/023 CDP harness + SHA pin〕;subagent-driven-development 各單元 fresh implementer + **spec compliance + code quality 雙審**。**設計細節見 [`specs/023-manage-endpoint-auth/`](../specs/023-manage-endpoint-auth/)**。**scope 外〔後續 feature〕**:endpoint 授權的復原/版本管理/受保護標記「完整受管層」、授權把關機制重寫〔萬用/前綴比對〕、in-handler-jwt 端點把關方式統一）
5. **axum-casbin 重寫 feature** — Casbin Axum enforce 中介層,rev2 自家 metrics / error / observability(**2026-05-29 012 brainstorm 從 Phase 2 §11.6 重定位至此**:中介層需真實受保護路由 + enforce 點 + observability 目標〔Phase 6〕才驗得了、整合得了,在 Phase 2 做會「無消費者、無法驗」)（**013 已做第一刀**:`auth/enforce.rs` rev2 自家 axum middleware〔最小機制、驗 JWT→per-role enforce→403+5003、單示範路由〕+ `Arc<RwLock<Enforcer>>` 並發無問題〔未用 CachedEnforcer〕;**本 feature = fuller rewrite**:全路由 rollout + metrics/error/observability 整合〔Phase 6〕）（**2026-06-05 status / 行政收口**:「全路由 rollout + error + DB-fresh + DRY」核心已由 013/018/023/026 增量吸收 → **不再當獨立 feature、視核心完成**;**唯 metrics/observability 埋點 defer 至 Phase 6 obs stack**〔無 prometheus/grafana 消費者前做 = 重工〕。Phase 3 header 隨之標「核心完成」。**（2026-06-08 更新:enforce metrics 埋點 ✅ 032-obs-full 落地 —— `enforce_mw` 自埋 `casbin_enforce_total{decision}` allow/deny counter、經 prometheus 消費、merge `a13f85f`,見下方 Phase 6 #2;Phase 3 #5 完全收。）**）

   - **★ auth 層 DRY refactor（026-auth-dry-refactor,✅ 已 merge `4553892`、已推 origin）** — #5「fuller rewrite」縮水後的**第一個落地子部分 = 純內部 DRY refactor**（metrics / observability / 全路由 rollout 仍 defer Phase 6）。收斂 auth 層兩處系統性重複:**(1) `verify_bearer`**（`auth/bearer.rs`,與 `bearer_token` 同檔）把 **5 處** `bearer_token→jwt::verify(access_secret, JWT_AUD)→claims` 前導抽成 `(headers, secret, aud) -> Option<Claims>`〔`enforce_mw` / `get_user_info` / `get_user_routes` / `is_route_exist` / `ctx_mw`〕;**(2) `issue_tokens`**（`handler/auth.rs`,近 `LoginToken` wire DTO）把 **2 處** access+refresh 簽發對抽成私有 `(user_id, roles, &JwtConfig) -> Result<LoginToken, jwt::JwtError>`〔`login_attempt_inner` / `refresh_token`,**注意 error type actual name = `jwt::JwtError` 非 brainstorm 的 `jwt::Error`**〕。**各 callsite 保留自身失敗映射** —— helper 只回中性結果、不決定碼:**§2.18 三策略**（`verify_bearer` doc-comment 文件化,FR-005）= `enforce_mw` **fail-closed**〔bearer/verify→3333,但下游 roles-DB 失敗→403/5003、server fault 不誤導重驗〕/ `get_user_info`+route handlers **advisory**〔token·roles→3333 叫重驗〕/ `ctx_mw` **best-effort**〔→operator_id None、不擾請求〕;`issue_tokens` 兩 callsite 各保留 `Err→Internal(5000)`。**roles 查詢段刻意留 inline**〔3 變體（`roles_for_user`/`_ordered`/+get_user_info 的 user-load）+ 2 錯誤策略（403 vs 3333）,強抽需 typed-error 抽象、CP 值低 → Approach A 親決;**故落地為窄 `verify_bearer`〔僅 bearer→verify→claims〕、非 follow-up 設想的較寬 `verify_and_load_*`**〕。**純 refactor、wire / 碼 / 狀態逐字不變**:既有 200 單測前後逐字不變 + 2 新 helper 單測〔`verify_bearer` 5 case〔valid→Some·缺/壞簽/過期/錯 aud→None〕·`issue_tokens` round-trip〔access 用 `jwt_secret`、refresh 用 `refresh_token_secret` 各 verify 回 user_id/roles〕〕→ server **206** + entity_access_lint 17 + endpoint_coverage_lint 30 + `Migrator::up` 0 + **curl 等價 smoke (a)-(g) 14/14 == 重構前 baseline**。**唯一可觀察差異**〔親決、wire 中性〕= ① verify-failed debug log 由 4 條 per-handler 訊息合併為 helper 內 1 條通用訊息〔`ctx_mw` 原 silent、現亦經 helper 共用 log〕;② `refresh_token` 兩條 sign-error 訊息（access / refresh）合併為 1 條通用訊息〔皆 error 級〕。**無新端點 / 表 / crate / dep / migration、不動 base-web、不碰 casbin policy、無 amendment**;Constitution v1.5.0 §IV 8/8 trivially PASS。閉合 013/014/015 各自 follow-up 登記的 auth-DRY 三項〔issue_tokens 簽發收斂 / verify_bearer 前導收斂 / ctx_mw 第 5 處〕。**設計細節見 [`specs/026-auth-dry-refactor/`](../specs/026-auth-dry-refactor/)** + Phase 0 brainstorm [`docs/superpowers/026-auth-dry-refactor.md`](superpowers/026-auth-dry-refactor.md)。
6. **受管 RBAC policy 層 feature** — 在 casbin policy 上加 rev2 治理:**(a)** soft-delete 可復原 **(b)** 不可刪 protected policy(remove 守衛 + 受保護標記)**(c)** policy 變更走 [011 audit](#§64-sys_operation_log統一-audit)(`mutate_in_txn`,記 operator/何時/加或移除哪條)**(d)** 統一 CRUD facade。**需 fork 拷貝進來的 `sea-orm-adapter`**(改 `load_policy` 過濾 `deleted_at IS NULL`、改 `remove_*` 設標記)+ casbin_rule 屆時加 `deleted_at` + partial unique index + protected 標記。**動到 §11.6「adapter=拷貝」前提** → specced 時須評估 constitution Amendment。(**2026-05-29 012 brainstorm 衍生**:user 要 recoverability + protected + 變更軌跡 + 一致 CRUD;但 soft-delete 的「刪掉不再 enforce」、audit 的「誰改的」皆依賴 Phase 3 的 enforce/operator/policy 入口才驗得了,故與 axum-casbin 重寫同期)（**2026-06-05 status**:前置〔enforce/operator/policy 入口〕已由 013/021/022/023 備齊 → 技術上隨時可做、無 blocker;成本門檻 = fork sea-orm-adapter + §11.6 amendment + 長期維護 fork。單-instance 下其解的 casbin 非原子/多-instance 債 benign → **重新定位為獨立 deferred 治理軌道、非 Phase 3 核心完成度 gate**。）

   - **★ 受管 RBAC policy 治理層（034-managed-rbac-policy,✅ 已交,2026-06-09 — rust-api worktree `87e3d8a..a2a52fb` push fork + base-web `75f2a20..a5783df` push fork + 外層 SHA pin / merge SHA TBD;rust-api + base-web 兩倉、5 US/5 migration）** — 把上面 deferred 治理軌道整個落地。**★ 翻案主因 = 架構 B（archive 表、免 fork）**:**親驗 stock `sea-orm-adapter` 嚴格 column-scoped 到 `ptype,v0..v5`**(`load_policy = Entity::find().all()` 只選 entity 定義 6 欄、`insert` 只填那 6 欄)→ 加治理欄 + archive 表對 adapter **完全隱形** → **不 fork adapter、§11.6 後端不觸、無 backend constitution amendment**(推翻原「soft-delete 須 fork adapter」的 defer rationale,詳 §11.6 補註)。核心不變式:**`casbin_rule` 永遠只裝 live policy**、軟刪列搬 `sys_casbin_policy_archive`;治理 facade DB-first(sea-orm 直寫 casbin_rule + archive + 011 audit 同 `mutate_in_txn` → commit → enforcer `load_policy()` → publish `casbin:policy:invalidate`),旁路 adapter `auto_save` 的 pool-handle(原子性死結根因)。讀熱路徑(enforce_mw / getUserRoutes)零改、純寫側(FR-014 讀決策逐項不變)。
     - **5 User Story(全交 + 雙審）**:**US1 原子治理 facade** — grant / revoke(=soft-delete 搬 archive)/ restore(從 archive 搬回 casbin_rule),每動作與 011 audit **同 txn both-or-neither**(審計寫入失敗 → policy 變更一併回滾、消滅「已生效卻無審計」);governance service 集中 commit→reload→publish。**US2 data-driven `protected`** — m033 seed **16 條 casbin protected 列** + m034 `sys_menu.protected` **7 列**,revoke / hard-replace 省略 protected → 業務碼拒;**退役 `is_seed_menu` 4 caller**(改吃 data-driven protected)、**修 D13**(補掉今天 manage_role / manage_system-settings 頁未受保護的 self-lockout 漏洞)。**US3 menu↔policy 同步** — 選單軟刪 / 還原連帶其可見性 policy cascade revoke / restore(同 txn);**修 DRIFT-2/3/4**(孤兒可見性 policy / 同名 route 重建靜默繼承 / menu CRUD 不碰 policy);**同名 route 重建 = 零繼承**(新選單對所有角色起始皆不可見);**restore 以 `transaction_timestamp` scope 到自身那一代 soft-delete generation**(避免誤還原他批歸檔列,US3 review fix 收口)。**US4 收斂三胞胎** — 把 021/022/023 三條寫路徑(`set_role_menu` / `set_role_button` / `set_role_endpoint`)收斂為單一 DB-first **`set_role_dimension`**,使 audit(US1)/ protected(US2)一致套用三維度;**退役 menu self-lock guard**(→ 改由 protected 承載);**三介面對外 wire 逐位元組不變**(既有 acceptance 全綠、SC-006)。**US5 回收桶** — 2 治理端點 `getArchivedPolicies` / `restorePolicy`(Super-only、`Res<T>` envelope、重用既有 BizCode)+ base-web 回收桶頁(`views/manage/policy-archive/`,鏡像 manage 範式)+ **D10 menu.protected**;選單軟刪連帶歸檔項排除於 policy 回收桶之外(clarify Q2,由還原選單帶回)。
     - **schema:5 新 migration m031-m035**(無新業務表 retrofit 債、無新 crate / dep):**m031** `casbin_rule += protected / created_at / created_by`(alter adapter 標準表)/ **m032** 新 `sys_casbin_policy_archive`(restore buffer、§I.6 append-only 例外變體、帶 `archived_at` / `archived_by` + grant 出處欄)/ **m033** seed casbin protected(16 列)/ **m034** `sys_menu += protected` + seed(7 列)/ **m035** US5 回收桶頁 seed(sys_menu 列 + R_SUPER role-menu policy、§I.2 可見性)。**casbin live set 69→72**(US5 +3 新治理端點 / 頁列)、protected 19(casbin 16 + 後續 +3)、sys_menu protected 8。詳見 [REVIEW-DATABASE §casbin/schema](REVIEW-DATABASE.md)。
     - **acceptance(全綠)**:subagent-driven-development 逐 US fresh implementer + 兩階段 review(spec compliance 對 spec.md + code quality)+ final holistic review。**server 245 unit / 0 failed** + entity_access_lint 17 + endpoint_coverage_lint 5 @ `EXPECTED_ROUTE_COUNT=35` + **live governance 整合測 17**(`#[ignore]` live-DB)+ **prod image build GREEN** + **讀決策不變(FR-014)** + base-web typecheck CLEAN + **CDP browser smoke 過**。**兩段式 commit**〔rust-api worktree push fork `rev2-admin-rust-api`:Foundational `6e84d9e` → US1 `6c9308f` → US2 `e1c202a` → US3 `42eb293`+`dc92175` → US4a `fe36fd2` → US4b `6f92364` → US5-backend `09e0609` → doc polish `a2a52fb`;base-web worktree push fork `rev2-admin-base-web`:US5 `c4eab9e` → doc polish `a5783df`;外層 SHA pin〕。**Constitution US1-US4 全 PASS、無 amendment;US5 回收桶頁經 2026-06-09 user 親決認定落 MODAL-WIRING use (e) v1.6.0 既有界、不需 v1.7.0 amendment**(詳 §11.25)。**設計細節見 [`specs/034-managed-rbac-policy/`](../specs/034-managed-rbac-policy/)**(spec/plan/research/data-model/contracts)+ Phase 0 brainstorm [`docs/superpowers/034-managed-rbac-policy.md`](superpowers/034-managed-rbac-policy.md)。

   - **★ 治理層收尾硬化（035-policy-governance-hardening,✅ 已交,2026-06-09 — rust-api worktree `1dca5b6`(US1)+`1fa2ebd`(US2)push fork `rev2-admin-rust-api` + 外層 SHA pin a2a52fb→1fa2ebd → merge --no-ff 回 rev2-admin-root,merge SHA 見 [MILESTONES §1](INTEGRATION-MILESTONES.md)）** — 收 034 治理 facade 兩個「正確但不夠好」的內部債,**純 rust-api 4 檔(199+/18-)、對外 wire/錯誤碼/讀(enforce)決策逐項不變、無 migration/端點/crate/dep、base-web 零改**。**US1 restore 審計豐富化** — `RestoreOutcome::Restored` 由 unit variant → 帶 `{v0,v1,v2}`(role/obj/act);`restore_policy` 的 [011 Restore 審計](#§64-sys_operation_log統一-audit) `payload_after` 從懸空 `{archive_id}`(archive 列同 txn 即刪 → id dangling、forensic 無價值)→ 改記實際 policy 身分 `{role, target, dimension}`(`dimension_from_v2` 既有 + 新增 `pub(crate) fn restore_audit_payload` 純函式 seam、零額外 DB round-trip;身分由 `Restored` 變體在還原同 txn 內、archive 刪除前即帶出 → FR-002 還原後仍可解析);menu restore cascade match 臂同步改 `Restored { .. }`、其 summary 審計 `{route_name, roles}` 負載**不改**(已 meaningful)。**US2 no-op reload 跳過** — 新 trait `PolicyMutated { fn mutated(&self)->bool }`(3 impl:`SetRoleOutcome`=`matches!(Applied(_))` / `RestoreOutcome`=`matches!(Restored{..})` / `bool`=`*self`);`mutate_and_reload` commit 後由無條件 `reload_and_publish` 改為條件化 `if result.mutated() { reload_and_publish }`、**6 caller 零改**(只多 trait bound、型已 impl);`batch_delete_menus`(走直呼 `reload_and_publish`、非 `mutate_and_reload`)改 loop 內 `any_mutated |= deleted` 追蹤、整批全 no-op 才跳末次 reload。**範圍邊界(FR-006)= 只跳「結構性零變更」**(`SetRoleOutcome::Rejected` / `RestoreOutcome::{NoOp,NotFound}` / menu cascade `bool=false`〔查無〕);**空-diff `Applied`(更新成同集)與 menu-found-無-policy 仍 reload**(刻意、不優化)。`reload_and_publish` 本體、enforce 讀熱路徑、NoOp/NotFound 不寫審計皆零改。**acceptance(全綠)**:subagent-driven-development 逐 US fresh implementer + spec compliance + code quality 雙審 + final holistic review(**SC-001~004 全 MET**、逐 governance 寫路徑驗 SC-004 無 stale-enforce 破口)。**server 225 unit / 0 failed**(+ T002 restore payload 純測〔menu/button/method 三維度〕+ T008 `PolicyMutated` 真值表〔8 case〕)+ entity_access_lint **17** + endpoint_coverage_lint **5 @ 35** + **live `#[ignore]` restore 審計 payload roundtrip**(== `{role,target,dimension}`、存活於 archive 刪除後、不污染 watcher〔不 publish〕)+ **prod runtime image build GREEN** + **讀決策不變(FR-005/SC-003、live getUserRoutes Super/Admin/User + getRoleList enforce 逐項同 034、restorePolicy 假 id→2222 无審計、getArchivedPolicies wire 不變)** + base-web 零改。reload-skip 無 spy → 真值表 + `mutate_and_reload` conditional code review + no-op 後 DB 狀態不變間接守(research R5 誠實註)。**Constitution v1.6.0 §IV 8/8 PASS、無 amendment**。**兩段式 commit**〔rust-api worktree `1dca5b6`(US1)→`1fa2ebd`(US2、含 `policy_governance.rs` module doc 校正 reload 條件化)push fork;外層 SHA pin bump a2a52fb→1fa2ebd〕。**設計細節見 [`specs/035-policy-governance-hardening/`](../specs/035-policy-governance-hardening/)**(spec/plan/research/data-model/contracts/verification-commands)+ Phase 0 brainstorm [`docs/superpowers/035-policy-governance-hardening.md`](superpowers/035-policy-governance-hardening.md)。

### Phase 4 — 主流業務(✅ 全完成 — 016-025 + 跨切項〔wire mapping / alova-only / 菜單樹 / 審計欄 retrofit〕全交;status/gender 等 wire 細節全功能巡檢 ✅ 2026-06-09 經 CDP 完成、見下方 Phase 7 #1)
...
1. **manage list endpoints feature** — `getRoleList` / `getAllRoles` / `getUserList` / `getMenuList/v2` / `getAllPages` / `getMenuTree`(全 read,對齊 mock)（**✅ 第一刀 016-manage-role-user-list 已交,2026-06-01**:落地 **user/role 三條**〔`getUserList` 分頁取代 013 stub / `getRoleList` 分頁 / `getAllRoles` 全量〕,**menu 三條〔getMenuList/v2、getAllPages、getMenuTree〕→ ✅ 019 補齊**〔見下方「menu DB-driven feature(019)」〕。**不動業務表**:base-web 想顯示但 entity 無的欄(status/email/phone/gender/roleDesc/時間/operator)一律 DTO 回 `null`〔D2〕、真正補欄綁 #5 審計欄 retrofit + write 那一波。**realized wire 決策**:① 對外 **id=string**〔i64 於 DTO 邊界 `.to_string()`,**採 constitution §I.3 凍結 string,凌駕本檔 §1.5/§3.6/§5.1/L88 殘留的「`Role.id:number`」舊敘述** — 那些屬待掃清矛盾、未走 amendment〕;② pagination 外殼 `PageRes<T>{current,size,total,records}` 放 `envelope.rs` 與 `Res<T>` 同層〔4 欄、**無 pages/success**、current/size/total 為 JSON number〕;③ roles **批次** `roles_for_users(&[i64])→HashMap`〔避 N+1,整頁固定 count1+page1+roles1=3 query,SC-006〕;④ size clamp `[1,100]`、`current` 0-based 轉換**只**在 handler `normalize_page` 一次〔facade 收 page_idx 直接 `fetch_page`,不雙重 -1〕;⑤ 排序 `id DESC`〔entity 無 created_at〕。**授權**:三條各掛 013 `enforce_mw` + 新 seed migration `m..013_seed_manage_policy`〔getRoleList×{SUPER,ADMIN}、getAllRoles×{SUPER,ADMIN,USER_COMMON}、getUserList 009 已有;**逐條 seed 無 wildcard**〔沿 009 先例,**凌駕本檔 §5.1/§6.3 規劃的「R_SUPER wildcard `p,R_SUPER,*,*` + getAllRoles 用 `*` 主體」**〕;路徑無 /api〕。**lint-safe 映射**:DTO mapping fn 收原始欄位〔非 Model 型〕→ handler/system_manage.rs 零 `entity::` token、守 009 lint。**無新 crate/dep**〔用既有 sea-orm `PaginatorTrait` + 008 + 013〕。**acceptance**:server 96 單測 + entity_access_lint 17 + xdb 9 全綠、47/47 活體 curl/psql〔三 endpoint 分頁/搜尋/超範圍/授權〔Super·Admin 200、User list 403+5003、無 token 3333、getAllRoles 含 User 200〕/無 password/id=string/null 欄〕+ **CDP 經 front-nginx 真實 `/api` 路徑** 端到端〔/manage/user 顯 3 user、/manage/role 顯 3 role、null 欄不 crash R5;順帶 de-risk prod-stack CDP(prod base-web /api 路徑見 §11.11)〕+ 6-lens 對抗審查 confirmed=0。**兩段式 commit**〔rust-api worktree 5 commits `7e5ce0b..81c56ef` push fork〕;subagent-driven-development 各單元 spec+quality 雙審。**設計細節見 [`specs/016-manage-role-user-list/`](../specs/016-manage-role-user-list/)**;follow-up 見本檔 §11.10 wire 細節決策(id 型 accepted)+ §10 Phase 4(menu 3 read endpoint,需 sys_menu)）
   ...
   - **★ user 寫端 CRUD（017-manage-user-write,2026-06-02 merge `617136d` / SHA-pin `7bfb353` / rust-api worktree 18 commits `910a6a2..deb6abee` / base-web `4c33895`;本地 merge --no-ff、017 branch 保留、已推 origin〔merge `617136d`+SHA-pin `7bfb353` 經 git 核實在 origin〕）** — 把 016 唯讀管理頁閉成**完整 CRUD**:落地 **4 條 Super-only systemManage 寫 endpoint** `addUser`/`updateUser`（POST）+ `deleteUser`/`batchDeleteUser`（DELETE 帶 Json body）。
   ...
   - **★ role 寫端 CRUD（018-manage-role-write,2026-06-02 merge `8844105` 回 rev2-admin-root〔--no-ff、保留 018 branch〕+ 已推 origin / SHA-pin `77c1fd6` / rust-api worktree `a95ef69..53420d4` 9 unit push fork + base-web `e735d8d..cdcad05` 3 unit push fork;rust-api + base-web 兩倉）** — 把 016 唯讀 role 列閉成**完整 CRUD**:落地 **4 條 Super-only systemManage 寫 endpoint** `addRole`/`updateRole`（POST）+ `deleteRole`/`batchDeleteRole`（DELETE）,並讓角色 `status` 啟用/停用**真正生效於授權**。
     - **schema 完補（sys_role,§I.6 retrofit）**:migration `m..016_alter_sys_role_business_audit` 給 sys_role **+7 欄** — 業務欄 `role_desc`/`status` + §I.6 審計欄 `created_at`/`created_by`/`updated_at`/`updated_by`/`deleted_by`〔`deleted_at` 006 已有;**無 BIGSERIAL retrofit** —— 006 已 auto_increment、無 017 那種 sequence 缺口〕 + seed `status=1` 回填 `id<=3`;entity +7 欄對齊 sys_user::Model + `AuditSerialize`〔無敏感欄〕。migration `m..017_seed_write_role_policy` casbin **4 行逐條 `p,R_SUPER,<path>,<method>`**〔無 wildcard、沿 009/016 先例〕。
     - **facade 寫 pattern（皆走 011 `mutate_in_txn`、原子 + audit operator 由 015 ctx 帶入）**:`create_role`〔insert + audit `Insert`、**code 唯一前檢**→`DuplicateCode`→2222〕/ `update_role`〔load active→`Ok(false)`〔not-found〕;否則 `col_expr` 業務欄 + **`updated_at`+`updated_by` 成對 DB-side `current_timestamp`**〔D5〕 + 重查 + audit `Update`、**不動 `code`**〔D2〕〕/ `soft_delete`〔**`deleted_at`+`deleted_by` 成對** + audit `SoftDelete`〕;新 `find_active_by_id`/`find_active_enabled`〔`deleted_at IS NULL AND status=1`〕。
     - **★ 有效角色集 ripple（D3+D4,停用即不授權/不可指派）**:`roles_for_user(s)`/`roles_by_codes_query` 由 `find_active` 改吃 **`find_active_enabled`** → 停用/軟刪角色**自動「不授權·不可指派」一處 filter、處處生效**〔enforce / getUserInfo / menu / getAllRoles / replace_roles〕;`getRoleList` 仍用 `find_active`〔顯停用角色、FR-009/FR-012〕。
     - **★★ enforce_mw 改讀 DB-fresh 有效角色（B 決策、user 親決、§11.17 amendment 來源、跨 013）**:`enforce_mw` 由讀 token `claims.roles` 改 `roles_for_user(db, claims.user_id)`〔DB-fresh、走 `find_active_enabled`〕 → 角色停用/軟刪後**下次授權檢查即時失效、舊 token 不重登**〔FR-006;`claims.roles` 對 enforce 變 vestigial、refresh 仍 stateless〕。**此為 018 核心 deliverable**〔§11.17 amendment 之成品落點〕。
     - **★ D5 順帶校正 017**:`update_user` 的 `updated_at` 由 app-side `SystemTime::now()` 改 `col_expr(current_timestamp)` + 重查,審計時間源**全 DB-side**〔對齊 `created_at`/`deleted_at`,呼應 FR-007「時間記錄來自單一一致來源」〕。
     - **種子保護（code-based,analyze I1）**:`is_seed_role_code`〔`R_SUPER`/`R_ADMIN`/`R_USER_COMMON`〕;**update 停用 guard**〔`status != Some(1)` 且種子→2222「不可停用」,含 `status=None` 防 NULL-brick〕 / **delete guard**〔單筆或批次任一種子→2222「不可删除」**整批拒、無部分執行**〕;種子可改 name/desc〔FR-005、US2-6〕。
     - **base-web 接線（MODAL-WIRING ★ + BASE-WEB-WRAPPER,鏡像 017）**:`rev2-system-manage.ts` +4 fetch fn + role-operate-drawer add/edit ternary + index.vue delete/batchDelete 接線〔`roleCode` 不設 readonly、由 server 省欄強制 D2 immutability〕。**無新 crate/dep**。
     - **acceptance（全綠）**:subagent-driven-development 11 unit〔9 rust + 2 base-web〕+ CP1 高風險先驗,各兩階段 review〔spec + quality〕+ opus final holistic = READY TO MERGE〔FR-001..012 + SC-001..011 全可追溯〕。US1-3 curl/psql〔addRole 真值/dup 2222/同名不同碼/種子刪·停用拒/空批 no-op/Admin·User·none 授權/audit Insert·Update·SoftDelete *_by 成對〕+ **status enforce 即時 SC-009**〔自訂 R_ENF 停用→User 舊 token 5003→重啟用 0000〕+ **G1 refresh-then-enforce**〔舊 refresh 換新 access 仍 5003、證 `claims.roles` vestigial〕+ 不可指派〔getAllRoles 排停用、updateUser 指派停用只採有效〕+ 回歸 013 enforce/014 menu/016 list/017 + D5 成對 + 守恆〔server 133 unit + 8 live + lint 17 + xdb 9 + `Migrator::up` 0〕 + migration 016/017 up→down→up 可逆 + prod image build 綠。**CDP browser smoke（T028）defer**〔C-V §7 授權:server 全 curl 驗 + base-web typecheck 乾淨 + 逐字鏡像 017、user 正用瀏覽器;登記 §2.22 follow-up〕。Constitution v1.2.1 §IV 8/8 PASS〔無 amendment、B 記 plan Complexity Tracking〕。**設計細節見 [`specs/018-manage-role-write/`](../specs/018-manage-role-write/)**;follow-up CHECKLIST §2.22;018 branch 保留。
   ...
   - **★ ButtonAuth Rollout — 022「按鈕權限可編輯迴路」rollout 到角色頁/選單頁 + 三頁 reactive 修正（024-button-auth-rollout,2026-06-04 ✅ merged `64d9bec` 回 rev2-admin-root〔--no-ff、保留 024 branch〕+ 已推 origin;rust-api worktree `f7cd042`〔1 commit〕+ base-web `03e20e4..be3e54a`〔3 commit〕,rust-api + base-web 兩倉 + 外層 `tests/024-button-auth-rollout/` CDP harness)** — 把 022 僅覆蓋用戶頁的「按鈕權限可編輯迴路」(modal 勾選→`set_role_button` HARD REPLACE 同步 casbin→頁面 `hasAuth` gating) **rollout 到角色管理頁/選單管理頁**:為 `manage_role`/`manage_menu` seed `role:*`/`menu:*` 按鈕碼進 registry→自動進「按鈕權限」modal 可指派→沿 022 迴路即時生效→role/menu 頁寫按鈕依 `hasAuth(code)` 顯/隱。K1-K4 親決(brainstorm `9e59f09`)。**輕量、decoupled 刻意**:唯一 rust = 1 seed migration(後端機制/modal/wire 零改)。
...
2. **wire shape mapping feature** — output DTO + `From<Entity>` impl + pagination wrapper（**✅ 隨各端點增量落地、跨 feature 範式非單一 feature**:016 立骨架 `PageRes<T>` + UserItem/RoleItem/AllRoleItem;各 feature 隨 endpoint 接 DTO mapping fn〔017 user_item / 018 role_item / 019 menu 讀端 + mapper / 020 menu 寫端 / 022 button / 023 endpoint / 025 re-parent / 029 sessionPolicy〕。**realized 偏離原規劃 `From<Entity>`**:改 lint-safe mapping fn 收原始欄位〔非 Model 型〕→ handler 零 `entity::` token、守 009〕）
3. **alova-only endpoint 處理 feature** — （**✅ 收口,2026-06-06**:017 已實作 4 寫端〔addUser/updateUser/deleteUser/batchDeleteUser,BASE-WEB-WRAPPER〕;**餘 3〔sendCaptcha/verifyCaptcha/getLastTime〕+ /auth/error = ⊘ moot、rust-api 不實作** — grounding 證三者僅服務 base-web `views/alova/scenes/` framework demo 頁〔captcha 非 `pwd-login` 登入流、getLastTime 僅 mock、/auth/error orphan〕,且 dynamic 路由模式(014)`getUserRoutes` 不送 alova demo menu → demo 頁不可達、又走 `service-alova`/ApiFox mock 層非 rust-api → rust-api stub 零產品價值;§11.2「stub」拍板 predate dynamic 不可達之認知,正式收掉、不列 Phase 5。CHECKLIST §5.8/§4 同步）
...
2. **✅ 028-single-session-enforcement 落地(2026-06-06、rust-api `d958c02`、merge `d528e61`)**:per-account 可控 access 端單一-session（policy=開新登入即時踢舊〔`7777`「账号在他处登录」modal、無迴圈〕、policy=關維持 027 多裝置）。獨立 `sid`(Claims required)/ pointer Redis+sys_user 混合(fail-open)/ 4 認證 gate + refresh pointer-first / 登入 revoke 舊鏈 + 一律 set_pointer / policy sys_user 三態 + config 系統預設(028=off dormant)。**base-web 零改、無新對外端點、無 amendment**。**as-built 見 §6.6**;follow-up CHECKLIST §2.33;admin UI 拆 029。
3. **✅ 029-single-session-admin-ui 落地(2026-06-06、rust-api `27c03de` + base-web `75f2a20`、merge `1ce835a`)**:028 policy 的 admin 管理 UI — Super 經 UI runtime 調全站預設(不重部署、下個請求即生效)+ 使用者管理頁設每帳號 policy。新 `system_settings` KV 表(rev2 首張、§I.6 真審計)+ `AppState.session_mode: Arc<RwLock<SessionMode>>` + `settings_watcher`(pub-sub `settings:invalidate`)使 `is_current` 讀 runtime 預設 + 3 Super-only 端點(30→33)+ getUserList additive `sessionPolicy` + casbin/sys_menu seed + base-web 設定頁(MODAL-WIRING (e))/ 使用者頁 policy 欄·modal。**不改 028 enforcement 邏輯**、雙倉、無新 crate。**as-built 見 §6.7**;唯一 amendment v1.6.0 §11.24(ratified `3bd3eda`)。
4. **抽離項 stub feature** — `/auth/error` / `/auth/sendCaptcha` / `/auth/verifyCaptcha`（**⊘ moot、不實作,2026-06-06**:同 Phase 4 #3 alova-only 一批收口 — 僅服務 alova framework demo 頁、dynamic 模式不可達、走 ApiFox mock 非 rust-api → rust-api stub 零價值。詳見 §10 Phase 4 #3 + CHECKLIST §5.8;**Phase 5 唯一剩 cleanup-job**）
5. **✅ 030-cleanup-job 落地(2026-06-06、rust-api worktree `0f16fb8`〔foundation〕→`2f5bbde`〔cleanup-job impl〕→`faa3628`〔jti〕+ 外層 compose `39f68a6`、merge `d3bc391`)**:把 027 FR-011 OUT 的「過期 sys_token 實體清理」落地成 on-demand `cleanup-job` binary,外加修 027 latent 的同秒發證 `token_hash` 撞鍵。**(US1) 清理規則 = 純過期 + 安全邊際**:`DELETE FROM sys_token WHERE expires_at < now()-60s`,**與 status 無關**(active/used/revoked 皆刪)—— 一旦 `expires_at` 過,對應 JWT 必驗不過(`verify` `validate_exp`+`leeway=0`)→ 該列永不可能再被 `rotate()` 以 `token_hash` 查到 → 任何 status 安全刪;比 brainstorm「積極三態」更嚴格(保留 used-未過期 列的整鏈 reuse 偵測)。`60s` margin = 固定常數 `SKEW_MARGIN_SECS`(吸收 `expires_at`↔JWT `exp` 偏移),**不開 CLI 旗標**(FR-002 不可執行期調整)。**dry-run 預設**(`find().count()`、0 改變、`would delete N rows`)/ **`--execute` 才** `delete_many().exec()`(`deleted N rows`)、**冪等**(重跑只清新符合者)。純函式 `purge_cutoff(now,margin)=now-Duration::seconds(margin)` 可注入固定 now 做 red→green 單元測,副作用由 in-crate `#[ignore]` live postgres + acceptance C-V 覆蓋。binary 鏡像 `migration` bin 的極簡 env-load(`DATABASE_URL`→`APP_DATABASE_URL_FILE`→`APP_DATABASE_URL`),**直存 `entity::sys_token`**(009 lint 只掃 `server/src`、清理工具本就該繞 facade)、**無新 crate**(實作既有 stub crate)、**無 lib 抽取**。migration **m030 `idx_sys_token_expires_at`**(純 btree on `expires_at`、非 unique/非 partial、`up`/`down` 可逆、非建表故無 §I.6 審計欄義務)。compose 3 檔加 one-shot service(`profiles:["jobs"]`→一般 `up` 不啟〔FR-007〕 + `restart:"no"` + `cleanup_database_url` secret〔== `database_url`、least-priv PG role **defer**〕 + host cron 外觸、**無 in-stack scheduler**),鏡像 `migrate`;**dispatcher arg trap**:`docker compose run` 附加 arg 取代 service `command` → execute 須走 `--entrypoint /usr/local/bin/cleanup-job … --execute`(否則 `--execute` 被 entrypoint 當 `$1` → usage)。**(US2) jti 修**:`Claims` 加 per-token `pub jti: String`(每次 `sign()` 一個 fresh `uuid::Uuid::new_v4()`、production 唯一寫點),使同秒**輪替**的兩 token JWT byte-distinct → `token_hash = sha256(JWT)` 不撞 UNIQUE。**★ 真正撞鍵路徑 = login→同秒 refresh**(refresh 沿用 login lineage 的 `sid` + 同秒 `iat/exp` → 無 jti 時 byte-identical JWT → `rotate` insert 撞 UNIQUE → `5000`);**非 login×2**(每次 login mint 新 `sid`、本就不撞)—— 故 jti 用 **per-token**(不同於 `sid` 的 per-session/per-lineage)。零 wire 影響(base-web token opaque、不 decode);pre-jti 舊 token 缺 jti → `verify` deserialize 失敗 → 一次性重登(同 028 sid transition);無持久化 Claims-shaped 資料(`sys_token` 只存 `token_hash` digest)故 transition 為暫態。028 C4「≥1s gap」workaround 可拔。**純後端**:base-web 零改、無新對外端點、Constitution 8/8 PASS、無 amendment。**acceptance 全綠**:C1 prod image build(cleanup-job binary 在 runtime stage)+ C2 索引可逆(up→down→up)+ C3 cleanup 對帳(seed 各態列 → dry-run 0 改變〔SC-003〕→ execute 只刪 3 過期超 margin〔SC-001/004〕、within-margin〔30s〕+ live + used-未過期 全存活〔FR-002/003〕→ 冪等 re-run 刪 0〔SC-006〕)+ C4 活體(login×2 平行 + login→同秒 refresh×6 皆 `0000`、`token_hash` count==distinct 全程零撞鍵〔SC-005〕)+ C6(profile:[jobs] gated、一般 up 不啟)。SC-002(store 有上界)= 純過期規則數學必然、設計性質不獨立測(/speckit-analyze E1)。**收尾兩段式 commit + merge `d3bc391`(--no-ff、保留 030 branch、已推 origin)完成**。**as-built 見 [`specs/030-cleanup-job/`](../specs/030-cleanup-job/)**(spec/plan/research/data-model/contracts);follow-up:least-priv cleanup PG role(決策 3 defer)。

### Phase 6 — 觀察性(可選,生產 ready)— ✅ 全完成 (2026-06-08;031-obs-min / 032-obs-full / 033-dashboard-provisioning)
...
2. **✅ 032-obs-full 落地(2026-06-07 實作 → 2026-06-08 merge `a13f85f`、rust-api worktree `server`+`cleanup-job` 兩既有 crate 改 + 外層 compose/deploy/nginx)**:opt-in `profiles:[metrics]` metrics 觀察堆疊(接 031 obs-min 的 log 半邊、補 metrics 半邊),把 **prometheus**(scrape+儲存+PromQL)← rust-api `/metrics` + **postgres_exporter** + **redis_exporter** + **pushgateway**(← cleanup-job push)→ **grafana**(prometheus datasource + baseline alert rule)串成單機性能監控/告警鏈;**同時閉 Phase 3 #5 parked 的 enforce metrics 債**(enforce_mw 自埋 allow/deny counter)。**pin**:`prom/prometheus:v3.12.0`(static scrape 4 target、retention **15d**)/ `prometheuscommunity/postgres-exporter:v0.19.1` / `oliver006/redis_exporter:v1.85.0-alpine` / `prom/pushgateway:v1.11.3` / `grafana/grafana:13.0.2`(reuse 031、profile `["obs"]`→`["obs","metrics"]` 共用 UI);crate `axum-prometheus = "0.7.0"`(最後 axum-0.7 相容、MSRV 1.70、帶 transitive `metrics 0.23`+`metrics-exporter-prometheus 0.15`) + cleanup-job `metrics-exporter-prometheus = { version = "0.15", default-features = false }` + `ureq = { version = "2", default-features = false }`。
   - **rust-api code 改(2 既有 crate、無新 workspace crate)**:**server** `main.rs` `PrometheusMetricLayer::pair()`(內部 `set_global_recorder` 一次/process;**as-built 無 init guard** — `main()` 為唯一呼叫者〔bin-only crate、無測試在程序內建 router,已 grep 證〕,故依 §2 simplicity 不加 OnceLock guard、僅留註解說明)→ 公開 `/metrics` route(掛 auth 外、render handle)+ `.layer()` 全域(取 axum `MatchedPath` → `axum_http_requests_*` 用 matched-route label、低基數 FR-012);**server** `auth/enforce.rs` 在 3 個 enforce outcome(role-loop allow / fall-through deny / fail-closed deny)埋 `metrics::counter!("casbin_enforce_total","decision"=>"allow"|"deny").increment(1)` — **閉 Phase 3 #5 enforce metrics 債**;**cleanup-job** `main.rs` best-effort push 兩 gauge `cleanup_job_last_success_timestamp`/`cleanup_job_rows_deleted` 到 `pushgateway:9091`(`install_recorder()`+`gauge!`+`render()`+ **`ureq` blocking PUT**、warn-on-fail、dry-run 也推 `rows_deleted=0`)。**★ `ureq` 非 `reqwest::blocking`**:reqwest blocking client 在 `#[tokio::main]` runtime 內建構會 panic(nested runtime)、cleanup-job 是 tokio binary 故用無 runtime 依賴的 `ureq`。
   - **★ pin 校正二則(實作期經驗證)**:**(a) `redis_exporter` 必用 `-alpine` 變體** — bare `v1.85.0` tag 是 `FROM scratch`、**無 `/bin/sh`**,而本 feature 用 sh-wrapper(`entrypoint:["/bin/sh","-c"]` 讀 `redis_password` secret 注 `REDIS_PASSWORD`)→ scratch 變體起不來;`v1.85.0-alpine`(同版、含 shell)才可。**(b) cleanup-job `metrics-exporter-prometheus` 用 `default-features = false`** — 預設 features(`push-gateway`/`http-listener`)會拉進 `hyper-rustls`/`aws-lc-rs`/`cmake` 一整套重 transitive stack,但本 feature 只用 `install_recorder()`+`render()`(皆 core/無條件)+ 自寫 `ureq` blocking PUT、**從不用 `with_push_gateway`** → 關預設 features 去掉無謂重依賴。
   - **FR-011 nginx `/api/metrics` 擋塊(★ NEW scope、user 2026-06-07 決策 in-feature 補)**:`deploy/nginx/conf.d/_locations.inc` 加 `location = /api/metrics { return 404; }`。**理由**:front-nginx `location /api/` 會剝 `/api` 前綴,故 `https://<external>/api/metrics` 會打到 rust-api 的 `/metrics`(FR-011/SC-009 禁對外暴露);**research R7/D8 漏抓此路徑**(只驗了「無 `location /metrics`」、沒考慮 `/api/` strip 後可達)。exact-match `location =` 優先於 `/api/` prefix → 對外 404;**內網 prometheus 仍直接 scrape `rust-api:21081/metrics`(不經 nginx)、不受影響**。
   - **secret / 範圍**:**無新 secret** — postgres_exporter `DATA_SOURCE_PASS_FILE=/run/secrets/postgres_password`(env `DATA_SOURCE_USER=soybean`+`DATA_SOURCE_URI` 無 creds)、redis_exporter sh-wrapper 讀 `redis_password`;**無 migration / 無新表**(metrics 在 prometheus、非 DB)、**base-web 零改**、**無新 workspace crate**(deps 加在既有 server+cleanup-job)。least-priv exporter PG role(reuse soybean superuser DSN、internal-only 可接受)**defer follow-up**(合 030 least-priv PG role 軌道)。volume 新 `prometheus_data`(`/prometheus`、retention 15d)、exporter/pushgateway 無持久卷(pushgateway in-memory)。
   - **★ WSL2 gotcha(實作期踩)**:config/provisioning bind-mount 改動(nginx `_locations.inc`、grafana provisioning)套用須 `docker compose ... up -d --force-recreate <svc>`、**非 `restart`** — `restart` 在 `/mnt/d` drvfs 上會撞 bind-mount shadow-path 衝突而失敗,且 single-file/dir mount 在長跑 container 內可能 stale;`--force-recreate` 重建容器才確保讀到新 config。
   - **Constitution / 範圍**:純後端/infra,**Constitution 8/8 PASS、無 amendment**(R8 八問逐項 PASS/N-A;§8.5「obs-full = +prometheus+exporter+pushgateway+grafana alerting」逐字對齊、元件/版本選擇=實作層無拍板撤回)。greenfield(§I.5、未讀/拷 rev1)。**acceptance C1-C10 + SC-005 firing test + 守恆全綠**(C1 `/metrics` axum_http_requests_* matched-route label / C2 enforce allow/deny counter / C3 prometheus `up{rust-api}`=1 / C4 `pg_up`=1+`redis_up`=1 / C5 grafana prometheus datasource 端到端 / C6 cleanup-job push→pushgateway→prometheus〔`honor_labels` job=cleanup_job〕 / C7 3 條 baseline alert rule provisioned + **SC-005 firing 實測**:停 rust-api → `rust-api-target-down` inactive→pending→firing / C8 profile gating〔metrics⊥log〕 / C9 envelope 不變 + 旁路〔prometheus down→app 200;pushgateway down→cleanup-job exit 0〕 / C10 prod `--profile metrics` build〔新 deps 進 release/runtime image〕+ internal-only;守恆:server 225+5 測 + entity_access_lint 17 + cleanup-job 4 + dev/prod compose config parse 全綠)。
   - **設計細節見 [`specs/032-obs-full/`](../specs/032-obs-full/)**(spec/plan/research/data-model/contracts) + Phase 0 brainstorm [`docs/superpowers/032-obs-full.md`](superpowers/032-obs-full.md)。
3. **✅ 033-dashboard-provisioning 落地(2026-06-08、全 outer/deploy、base-web+rust-api 零改、無 worktree)**:接 031 obs-min(log)+ 032 obs-full(metrics),把觀察性從 Explore + alert-only 補成**開 grafana 即見的 6 張現成監控面板**(obs 的「可看」面)。純 grafana config:新 `deploy/grafana-provisioning/dashboards/`(1 provider yaml + 6 dashboard JSON)+ 改 031 `loki.yml` 補顯式 `uid: loki`。
   - **6 板** = **4 greenfield**(自寫、貼 rev2 真實 metric/log、`schemaVersion:39`、uid:`obs-*`):`master-overview`(全 target up + pg/redis up + 總請求率 + deny 率 + cleanup age)/ `rust-api`(請求率 by endpoint·status + 延遲 p50·p95·p99 + 5xx + in-flight + enforce allow·deny)/ `cleanup-job`(上次成功時間 + age + rows + pushgateway up)/ `audit-log`(loki:log 量 by service + rust-api error·warn/by-level + nginx status + enforce-deny logs)+ **2 community pin**:`postgres`(**`postgres_mixin/dashboards/postgres-overview.json @ postgres_exporter v0.19.1`**、82% live-match、3 absent=PG17 移除 bgwriter 子 counter)/ `redis`(**grafana.com dashboard `763 revision 6`** = oliver006/redis_exporter `@v1.85.0` byte-identical、100% match)。
   - **provider**:file provider(`folder: obs-full` reuse 032 alert folder、`options.path` 指 `dashboards/json/` 子目錄、provider yaml 與 JSON 分目錄放、`disableDeletion:true`/`allowUiUpdates:false`)。datasource 引用一律穩定 uid(`prometheus` 032 既有顯式 + `loki` 本刀補顯式);community pin datasource resolution 採 hardcode uid(redis `${DS_PROM}`→`prometheus`+刪 `__inputs`/`__elements`/`__requires`;postgres 24 個 `$datasource` panel ref→`prometheus`+移除 datasource 變數)。
   - **★ as-built live-grounded 修正(implementer 必守、皆 live 取證)**:(a) **research R8 漏判修正** — `loki.yml` 加 `uid: loki` 會讓 grafana 13.0.2 crash-loop(`data source not found`):Loki 在 031 是無 uid 首次 provision、auto-gen uid `P8E80F9AEF21F6940` 已 persist 進 `grafana_data` 卷,改顯式 uid 時 provisioner 找不到該 uid 既有 datasource 而失敗。**修:loki.yml 加 `deleteDatasources:[{name:Loki,orgId:1}]`** 先按名刪舊 row 再以 uid 重建(對 fresh volume no-op、idempotent、純 config、dev 既有卷與任何已跑 031/032 的部署皆自動收斂、零手動步驟;**部署地雷**:已跑過 031/032 的環境落 033 必經此修否則 grafana 起不來)。(b) **research R7 假設修正** — 「rust-api log 全 valid JSON、`| json` 不需 guard」被 live 推翻:本專案 C0 acceptance recipe 用 `docker run rev2-admin-rust-api:dev` 跑 cleanup-job,alloy 依 image 把其純文字 stdout 也標成 `service="rust-api"` 灌進 loki → 板的 `{service="rust-api"} | json`(無 guard)回 **HTTP 400 JSONParserErr**、違反 SC-002。**修:audit-log 板 rust-api `| json` 三 panel(error·warn / by-level / enforce-deny)補 `|~ \`^{\`` 前置過濾**(與 nginx panel 同 idiom);實證未 guard `count_over_time(… | json [30m])`=400、guarded=200 success。對齊 spec edge case「不信抽象假設」。(c) **製程**:community pin 與 greenfield 板皆在 scanned 目錄外 staging 編輯、最後 atomic-mv 進 `dashboards/json/`(file provider `updateIntervalSeconds:30` 會 auto-provision 任何落地 `*.json`、直接 in-place 下載再編輯會被 provider 搶先 provision 半成品而造成 id-collision orphan)。其餘 research 既載 live 修正(job `postgres`/`redis` 非 `*_exporter`、5xx `or vector(0)`、nginx `|~ \`^{\``、cleanup 無 `up{job=cleanup_job}`、cleanup last-success `*1000` s→ms 顯日期)亦全套用。
   - **Constitution / 範圍**:純 grafana config,**base-web + rust-api 零改、無 migration/新 crate/新 secret/新 compose service、無 wire/碼/狀態變**。greenfield app dashboard + upstream exporter dashboard(版本鎖 exporter tag、非 rev1 source、§I.5)、**Constitution 8/8 PASS、無 amendment**。§3「新 workspace crate ⇒ prod build」嚴格不觸發(無 rust 改);但 C5 仍驗 prod `--profile obs --profile metrics` 起 + grafana internal-only。**acceptance C0-C5 + 守恆全綠**(C0 cleanup-job push + 流量 + deny / C1 6 板 provisioned in obs-full / C2 datasource 解析 Loki uid=loki+Prometheus uid=prometheus、6 板 dangling 全 NONE / C3 panel query 經 proxy 回資料〔rust-api req-rate·p99·enforce〔allow+deny〕、infra pg_up·redis_up·pg_activity·redis_memory、cleanup last-success·rows·pushgateway、audit-log log-volume·nginx-status·by-level〕 / C4 provisioning log 乾淨 / C5 prod 6 板 + grafana 無對外 published port〔FR-010〕;守恆:Loki+Prometheus datasource + 3 alert rule + loki Explore via uid:loki + dev/prod compose `--profile obs --profile metrics` config parse 全綠)。
   - **設計細節見 [`specs/033-dashboard-provisioning/`](../specs/033-dashboard-provisioning/)**(spec/plan/research/data-model/contracts) + Phase 0 brainstorm [`docs/superpowers/033-dashboard-provisioning.md`](superpowers/033-dashboard-provisioning.md)。

### Phase 7 — 維護(持續性)

1. **status / gender / 其他 wire 細節對齊 feature** — 走 CDP 全功能巡檢 — **✅ 2026-06-09 完成**(Super 全頁渲染 + status/gender/null 欄/id-string/menu_type wire 細節驗 + RBAC §I.2 端到端〔User 直連 /manage/* → 404〕 + 035 回收桶 restore round-trip 經真實 UI;後續新增 wire 欄〔如真 gender 欄〕再開新巡檢)
...
>
> **✅ 2026-06-09 034-managed-rbac-policy 翻案「拷貝→拷貝+客製≈半重寫」的擔憂、§11.6 後端不動**:上方 2026-05-29 重定位曾預期 [Phase 3](#phase-3--認證--動態選單)#6 受管 RBAC policy 層要為 casbin policy 做 soft-delete → 須 **fork adapter 的 `load_policy`/`remove_*`**(濾 `deleted_at IS NULL` / 設標記)→ adapter 從「拷貝」變「拷貝+客製」≈ 半重寫、且要評估 §11.6 amendment。**034 親驗推翻此前提**:stock `sea-orm-adapter` 嚴格 **column-scoped 到 `ptype,v0..v5`**(`load_policy = Entity::find().all()` 只 select entity 定義的 6 欄;`add_policy` 的 insert 只填那 6 欄)→ 在 `casbin_rule` 加治理欄(`protected`/`created_at`/`created_by`)、另起 `sys_casbin_policy_archive` 表,對 adapter **完全隱形**。**架構 B = archive 表路線**遂以 `casbin_rule` 永遠只裝 live policy(軟刪列搬 archive)維持不變式,治理 facade DB-first 直寫 + 011 audit 同 txn,**完全不碰 adapter load/remove** → §11.6「sea-orm-adapter = 拷貝」前提**不動、後端無 amendment**。被拒的 A 替代(fork adapter 加 `deleted_at` 濾鏡)成本(觸 §11.6 amendment + 永久維護 fork + 「忘濾鏡」DRIFT-3 近親隱患)正是 034 plan Complexity Tracking 拒它、改採 archive 表的理由。**唯一 constitution 相關評估落在前端 US5 回收桶頁(MODAL-WIRING、非本拍板),2026-06-09 user 親決認定落既有 use (e) v1.6.0、亦不需 amendment**(詳 §11.25)。
...
### §11.25 no-amendment 認定 — 034 policy 回收桶頁落 MODAL-WIRING use (e) v1.6.0 既有界(2026-06-09,034,user 親決)

> **背景**:034 `/speckit-plan` Constitution Check item 7(§III ★ MODAL-WIRING)曾標 **⚠ NEEDS AMENDMENT(US5、user 親決)**,理由:US5 base-web policy 回收桶頁(`views/manage/policy-archive/index.vue`)屬「policy 還原維運 UI」、疑似超出現行 (a)-(e) 授權範圍,plan 預留「落 (e) 或新增 use (f) 的 v1.7.0 amendment、實作到 US5 時 user 親決」(spec Assumptions 同註)。
>
> **✅ 親決(2026-06-09,user)= 落既有 use (e) v1.6.0、不新增 amendment**:做到 US5 時對照 §11.24 開的 use (e)「同 `manage/` 範式新管理頁:`views/manage/<page>/index.vue` + 可選 modules、鏡像既有 user/role/menu 頁結構、消費 rev2 端點、route 由 elegant-router 自動生成、選單可見性走 §I.2 Casbin」逐項比對 → policy 回收桶頁**完全落在 (e) 既有界內**,**毋須** v1.7.0、**constitution 維持 v1.6.0**。
>
> **為何落 (e) 而非新 use (f)**:① **鏡像 manage 範式** —— policy-archive 頁就是 index.vue + 列表 + 還原動作,結構鏡像既有 user/role/menu/system-settings 管理頁,非自訂佈局、非 non-manage 頁;② **消費 rev2 端點** —— `getArchivedPolicies` / `restorePolicy` 走 BASE-WEB-WRAPPER(`rev2-policy-archive.ts`)、頁本體僅 (e) 授權的 view 結構;③ **§I.2 Casbin 可見性** —— 回收桶頁可見性走後端 m035 seed(sys_menu 列 + R_SUPER role-menu policy),非前端硬塞、與既有 manage 頁可見性同機制;④ **還原鈕先例** —— 「列上 restore 鈕」此 inline pattern 早於 025 menu 回收桶以 use (d) v1.5.0 授權(§11.23),034 policy 回收桶沿用同一還原-鈕慣例、不構成新 UI 類別。four 點俱足 → (e) 既有界吸收、無新用途。
>
> **改後影響**:**無 constitution 改動、無版本 bump**(對比 §11.23/§11.24 為真 amendment;本條為 amendment-evaluation 之 no-op 認定 of record)。034 US5 base-web touch 全免新授權:回收桶頁本體走 (e)、2 fetch fn 走 BASE-WEB-WRAPPER、`ArchivedPolicy` 型放 `system-manage.d.ts` 走 BASE-WEB-ADAPT。此條為 034 plan 預留 amendment-gate 的審計收口。
>
> **觸發**:034 `/speckit-plan` Constitution Check item 7 預留 gate;實作到 US5 時 user 親決(2026-06-09)。落 034 feature branch(隨 034 merge 回 default)。
