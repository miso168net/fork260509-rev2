...
**下一步**: **020-manage-menu-write 已完整實作 + C-V 驗收 + 本地 Merge `0c73e3b` 回 rev2-admin-root(--no-ff、保留 020 branch)、未推**(待 user 同意 push:rust-api/base-web worktree fork 分支〔§4.1 第一段〕+ 外層 020 branch〔第二段〕)。**017/018/019 早已 merge+已推**。**Phase 4 續做**([DESIGN §10 Phase 4](INTEGRATION-DESIGN.md)):MenuAuth·ButtonAuth 編輯(角色×選單可見性、動 migration 010 casbin menu policy = 受管 policy / menu-auth feature,D1 拍板劃出 020 外)/ 選單 restore·re-parent(FR-011 out、後續)/ alova stub(sendCaptcha/verifyCaptcha/getLastTime,Phase 5)。(**menu 讀端 + getUserRoutes DB-driven ✅ 019、menu 寫端 CRUD ✅ 020**)**Phase 3 續做**([DESIGN §10 Phase 3](INTEGRATION-DESIGN.md)):#3 redis pub-sub(policy/route 失效通知)/ #4 完整 policy 矩陣 + 全路由 endpoint enforce rollout / #5 axum-casbin fuller rewrite(metrics/error/observability)/ #6 受管 RBAC policy 層(casbin_rule soft-delete 可復原 / protected / 走 011 audit / CRUD;**需 fork sea-orm-adapter 的 `load_policy`/`remove_*` + casbin_rule 加 deleted_at → 須評估 §11.6 Amendment**)。Phase 2 餘(非獨立 feature):soft-delete 6-entity rollout(sys_user 完整欄 + id auto_increment **✅ 017 補完**,餘 entity 待建,見 §2.10)/ audit 其他 operation·entity 接線(沿用 011 `mutate_in_txn` pattern)
...
- [ ] **CDP browser smoke defer**(headless 環境無 `:9229` browser、無法自主構造):server 全 curl/psql 驗綠(US1-3 + 種子/父子/停用 guard + batch 原子拒 + D1 payoff + SC-006)+ **front-nginx :21080 `/api` proxy 4 寫端驗**(base-web fetch 實際路徑:種子刪 2222/dup 2222/Admin 5003/none 3333、DB 未變)+ base-web 接線 typecheck 乾淨 + **逐字鏡像 production-proven 017/018**(fetchAdd/Update/Delete/BatchDelete + modal/index 同 pattern)。**補驗**:待 user 開專用 CDP tab 跑(登入 Super→/manage/menu→新增〔列表出現、nav 需 MenuAuth 暫不顯=D3〕→編輯既有〔列表+nav 反映〕→刪自訂〔出列〕→種子刪/父有子刪→2222 toast);連動 [§2.8](#28-feature-004-compose-port-orchestration-follow-up) prod-stack CDP
...
- [ ] **prod-stack CDP via front-nginx `/api`**(承 [§2.8](#28-feature-004-compose-port-orchestration-follow-up)):020 dev acceptance 經 :21081 直連 + :21080 front-nginx `/api` proxy(4 寫端);prod base-web build(`VITE_SERVICE_BASE_URL=/api`)+ prod nginx `/api` strip 端到端仍待(全 Phase 4 共通項)。
