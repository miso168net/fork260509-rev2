...
**下一步**: **014-026 spec-review 完成 + 025-I1 修復驗綠**(報告 [REVIEW-014-026.md](REVIEW-014-026.md):11/13 Ready、0 Critical/specGap、1 Important = 025-I1 已修;type-lie 消費端鐵律固化 [DESIGN §9.1](INTEGRATION-DESIGN.md))。review 衍生 doc-debt 批次已清(§2.31 ✅ 2026-06-05;018/019/020/023/025/026 spec/contracts surgical 修)。**之後候選**([DESIGN §10](INTEGRATION-DESIGN.md)):**Phase 5**(refresh token rotation〔+reuse 偵測/撤銷;§5.3 stale token 大半已成立〕·alova 3 stub·cleanup-job)/ **#6 受管 RBAC**(獨立 deferred 治理軌道、需 fork→§11.6 Amendment、解 casbin 非原子/多-instance 債)/ ButtonAuth「完整版」(aligned visible=clickable、024 decoupled 債對齊 023 端點)。〔#5 axum-casbin 核心已收、metrics 尾巴併 Phase 6,不再列候選〕下個 feature `/speckit-specify` 由 user 手動執行(建 feature branch)。
