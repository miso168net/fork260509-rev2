...
**下一步**: **021 已收尾完成**(merge `827d1e9` --no-ff 回 rev2-admin-root〔保留 021 branch〕+ worktree fork〔rust-api `rev2-admin-rust-api` / base-web `rev2-admin-base-web`〕+ 外層 origin 皆已推 + MILESTONES row 已補)。**017/018/019/020 早已 merge+已推**。**目前無 active feature** —— 下一 feature 待啟動(階段 0 `superpowers:brainstorming` → 階段 1 `/speckit-specify`)。**Phase 4 續做**([DESIGN §10 Phase 4](INTEGRATION-DESIGN.md)):**ButtonAuth 編輯**(角色×按鈕權限、機制不同於 menu 可見性把關、021 劃出獨立 feature)/ 選單 restore·re-parent(FR-011 out、後續)/ alova stub(sendCaptcha/verifyCaptcha/getLastTime,Phase 5)。(**menu arc 已閉:019 讀 → 020 寫 → 021 可見性指派**)**Phase 3 續做**([DESIGN §10 Phase 3](INTEGRATION-DESIGN.md)):#4 完整 policy 矩陣 + 全路由 endpoint enforce rollout / #5 axum-casbin fuller rewrite(metrics/error/observability)/ #6 受管 RBAC policy 層(casbin_rule soft-delete 可復原 / protected / 走 011 audit / CRUD;**需 fork sea-orm-adapter → 須評估 §11.6 Amendment**)。Phase 2 餘(非獨立 feature):soft-delete 6-entity rollout(餘 entity 待建,見 §2.10)/ audit 其他 operation·entity 接線(沿用 011 `mutate_in_txn` pattern)
