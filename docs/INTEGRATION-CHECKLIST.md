...
- **2026-06-05 027-refresh-token-rotation ✅ merge `2d4d8d4`(--no-ff、保留 027 branch、已推 origin;rust-api `93b9be6..1bd0436` 7 commit push fork、base-web 零改)** — refresh 升級 DB 持久化 rotation chain + 盜用偵測 + grace + SHA-256 雜湊:新 `sys_token` 表 + facade(`decide_rotation` 純判定 seam FR-014 全分支 + `rotate` FOR UPDATE 狀態機〔reuse→整族系 revoke + 8888〕+ `create_chain_head`)+ login/refresh 串接。**wire 中性**(`{token,refreshToken}`/Claims/issue_tokens 逐字不變、`8888`/`5000` 絕不 `3333/9999/9998`)。live-DB L1-L6 + curl C1-C5 + 守恆 server 218/lint 17/30 + migration 可逆 + prod build 全綠;subagent-driven A-G 雙審 + final holistic = READY TO MERGE。028 單一-session 拆出立案。詳見 MILESTONES §1 / [DESIGN §6.2·§10 Phase 5](INTEGRATION-DESIGN.md);follow-up §2.32。
> 以下為預計`下一步` (不要再被合到`最新進展`了)

**下一步**: **029-single-session-admin-ui**(028 policy 的 admin 管理 UI:系統預設 runtime store〔rev2 首張 system-settings 表、把 028 config 預設變 runtime 可調〕+ admin 設定頁 + 每帳號 policy UI〔使用者管理頁〕+ get/set endpoint + casbin + base-web,疊在 028 儲存之上、後端先 UI 後)**未起** → 待 **Phase 0 brainstorm**。**其他候選**([DESIGN §10 Phase 5](INTEGRATION-DESIGN.md)):stub(captcha/getLastTime)·cleanup-job(含 sys_token 實體清理 + same-second `jti` fix §2.33)/ **#6 受管 RBAC**(需 fork→§11.6 Amendment)/ ButtonAuth「完整版」(aligned visible=clickable)。
