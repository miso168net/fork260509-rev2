...
- [ ] **CDP browser click-through defer**(沿 016-020、C-V §7):021 menu-auth-modal 端到端瀏覽器點擊(登入 Super→/manage/role→開角色→勾選儲存→重登 nav 反映 + home 下拉)未跑;**curl 雙路徑已證**(rust-api :21081 直連〔T011/T012〕+ front-nginx :21080 /api proxy〔T015〕全 4 端點 e2e + 即時反映 + 自鎖 + redis reload + per-role home),base-web wiring typecheck-verified + 鏡像既有 getPages/getTree pattern。日後補真 CDP click-through。
- [ ] **`find_active`(sys_role.rs)·`roles_for_user`(sys_user_role.rs)stale `#[allow(dead_code)]`**(pre-existing、非 021 引入):二者皆有 live 呼叫端(find_active 經 find_active_by_id/find_active_enabled;roles_for_user 經 enforce_mw/auth/isRouteExist)但 allow 仍掛(Rust 不警告 unused allow、benign);同 §2.24 `normalize_page` 主題,動該檔時順手清。
- [ ] **`add_policies` 非單 txn 部分寫入窗口**(R1、code 已註)：`set_role_menu` 的 `remove_filtered`+`add_policies` 中,adapter `add_policies` 為逐 rule loop(非單 txn),中途失敗→該角色暫態不完整;admin 低頻 + redis reload + 重編修正(已記 `menu_auth.rs` doc、acceptance 接受)。日後若要嚴格原子可評估 adapter 層 txn(觸 §11.6)。
- [ ] **ButtonAuth 編輯仍另開 feature**(承 [§2.24](#224-feature-020-manage-menu-write-follow-up) MenuAuth/ButtonAuth 拆分;021 已落 **MenuAuth**):角色×按鈕權限機制不同(現由登入資訊程式內對映供應、不走可見性把關引擎)→ 獨立後續 feature。
...
- [~] **Casbin redis pub-sub 啟用 ✅ 021 落地(本地、待 push/merge user 同意)** — `casbin:policy:invalidate` channel:`set_role_menu` policy 變更後 PUBLISH(消費 dead_code `AppState.redis` ConnectionManager)+ boot `spawn_policy_watcher` 獨立 pubsub 連線 SUBSCRIBE→`enforcer.write()+load_policy()` reload(+1 dep `tokio-stream` 消費 Stream)。單 instance 自收冪等;多 instance reload 留 follow-up。實現 [DESIGN §10 Phase 3](INTEGRATION-DESIGN.md) #3 / §6.3 v1 即啟用
