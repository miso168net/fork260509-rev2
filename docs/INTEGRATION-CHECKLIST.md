...
**下一步**: 無 active feature(027/028/029 單一-session 三部曲完結)。**Phase 5 候選**([DESIGN §10 Phase 5](INTEGRATION-DESIGN.md)):抽離項 stub(captcha/getLastTime)· cleanup-job(含 sys_token 實體清理 + same-second `jti` fix §2.33)/ **#6 受管 RBAC**(需 fork→§11.6 Amendment)/ ButtonAuth「完整版」(aligned visible=clickable)。新 feature 起手走階段 0 brainstorm → 手動 `/speckit-specify`。
