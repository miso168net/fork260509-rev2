...
- **2026-06-03 constitution v1.3.0 amend(022 ButtonAuth gate、MINOR)**:022 `/speckit-plan` Constitution Check 抓 2 finding〔用戶頁 `hasAuth` gating 超 MODAL-WIRING 邊界 + 救活 toggle-auth 牴觸 §I.2 demo 隱藏〕→ user 親決改 2 條:**§III.2 MODAL-WIRING ★ 邊界 +button 可見性 gating**(提案 DESIGN §11.19)+ **§I.2 開 `function`/`function_toggle-auth` demo→真實 Casbin 選單例外**(DESIGN §11.20)。getUserRoutes 逐字基線將 re-base(含 function/toggle-auth);分流四檔(constitution / DESIGN §11 / CHECKLIST / MILESTONES)

> 以下為預計`下一步` (不要再被合到`最新進展`了)

**下一步**: **022 ButtonAuth 進行中(階段 1 SDD)**:spec-design `fafa3af` → `/speckit-specify` ✅(branch `022-manage-button-auth`、spec.md 16/16 PASS `955d37e`)→ `/speckit-clarify` ✅(0 critical)→ **`/speckit-plan` 進行中** —— Constitution Check 抓 2 finding → **constitution v1.3.0 amend 完成**(§11.19 MODAL-WIRING ★ 邊界 +button gating / §11.20 §I.2 toggle-auth demo→真實選單例外)→ 續產 research/data-model/contracts → `/speckit-tasks` → `/speckit-analyze` → `superpowers:executing-plans`。022 = Casbin `(role,code,'button')` 照抄 021、per-menu 按鈕 seed、pilot gate 用戶頁、救活 toggle-auth(getUserRoutes 基線將 re-base)。可一併處理 [§2.25](#225-feature-021-manage-menu-auth-follow-up) stale allow + 021 menu-auth-modal CDP click-through(同 /manage/role 頁)。**Phase 4 續做**([DESIGN §10 Phase 4](INTEGRATION-DESIGN.md)):**ButtonAuth 編輯**(角色×按鈕權限、機制不同於 menu 可見性把關、021 劃出獨立 feature)/ 選單 restore·re-parent(FR-011 out、後續)/ alova stub(sendCaptcha/verifyCaptcha/getLastTime,Phase 5)。(**menu arc 已閉:019 讀 → 020 寫 → 021 可見性指派**)**Phase 3 續做**([DESIGN §10 Phase 3](INTEGRATION-DESIGN.md)):#4 完整 policy 矩陣 + 全路由 endpoint enforce rollout / #5 axum-casbin fuller rewrite(metrics/error/observability)/ #6 受管 RBAC policy 層(casbin_rule soft-delete 可復原 / protected / 走 011 audit / CRUD;**需 fork sea-orm-adapter → 須評估 §11.6 Amendment**)。Phase 2 餘(非獨立 feature):soft-delete 6-entity rollout(餘 entity 待建,見 §2.10)/ audit 其他 operation·entity 接線(沿用 011 `mutate_in_txn` pattern)
