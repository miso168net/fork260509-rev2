...
**下一步**: **021 已收尾完成**(merge `827d1e9` --no-ff 回 rev2-admin-root〔保留 021 branch〕+ worktree fork〔rust-api `rev2-admin-rust-api` / base-web `rev2-admin-base-web`〕+ 外層 origin 皆已推 + MILESTONES row 已補)。**017/018/019/020 早已 merge+已推**。**下一 feature = 022 ButtonAuth(角色×按鈕權限編輯)** —— **階段 0 brainstorm findings 已 checkpoint**([`docs/superpowers/022-manage-button-auth.md`](superpowers/022-manage-button-auth.md):挖出兩地基問題〔無 button registry + base-web 僅 `function/toggle-auth` demo 用 hasAuth('B_CODE1/2/3')、業務頁不 gate〕,user 親決 spec-design **開新 session** 做〔需深挖 base-web button model + clean context〕)。**新 session 起手**:深挖 base-web mock/button 消費鏈 → 拍板 registry 來源 / getUserInfo.buttons 回歸基線(哪組 code)→ 續 brainstorm → `/speckit-specify`(建 022 branch)。可一併處理 [§2.25](#225-feature-021-manage-menu-auth-follow-up) stale allow + 021 menu-auth-modal CDP click-through(同 /manage/role 頁)。**Phase 4 續做**([DESIGN §10 Phase 4](INTEGRATION-DESIGN.md)):**ButtonAuth 編輯**(角色×按鈕權限、機制不同於 menu 可見性把關、021 劃出獨立 feature)/ 選單 restore·re-parent(FR-011 out、後續)/ alova stub(sendCaptcha/verifyCaptcha/getLastTime,Phase 5)。(**menu arc 已閉:019 讀 → 020 寫 → 021 可見性指派**)**Phase 3 續做**([DESIGN §10 Phase 3](INTEGRATION-DESIGN.md)):#4 完整 policy 矩陣 + 全路由 endpoint enforce rollout / #5 axum-casbin fuller rewrite(metrics/error/observability)/ #6 受管 RBAC policy 層(casbin_rule soft-delete 可復原 / protected / 走 011 audit / CRUD;**需 fork sea-orm-adapter → 須評估 §11.6 Amendment**)。Phase 2 餘(非獨立 feature):soft-delete 6-entity rollout(餘 entity 待建,見 §2.10)/ audit 其他 operation·entity 接線(沿用 011 `mutate_in_txn` pattern)
...
- [x] **Casbin redis pub-sub 啟用 ✅ 021 落地(已 merge `827d1e9` + 已推 origin)** — `casbin:policy:invalidate` channel:`set_role_menu` policy 變更後 PUBLISH(消費 dead_code `AppState.redis` ConnectionManager)+ boot `spawn_policy_watcher` 獨立 pubsub 連線 SUBSCRIBE→`enforcer.write()+load_policy()` reload(+1 dep `tokio-stream` 消費 Stream)。單 instance 自收冪等;**多 instance 真實 reload 未驗、留 follow-up([§2.25](#225-feature-021-manage-menu-auth-follow-up))**。實現 [DESIGN §10 Phase 3](INTEGRATION-DESIGN.md) #3 / §6.3 v1 即啟用
