...
**階段**:**Phase 1(001-006)+ Phase 2(007-012)全完成+已歸檔**。**Phase 3 RBAC 核心完成(行政收口)+ Phase 4 主流業務大部完成(013-026)**:認證地基(013 login/enforce·014 dynamic routes+menu 可見性·015 audit)、user/role/menu CRUD(016 list·017 user·018 role·020 menu·025 restore/re-parent)、menu DB-driven(019 sys_menu)、**menu/button/endpoint 三維權限 runtime 編輯**(021/022/023 + 024 rollout)、enforce_mw DB-fresh 角色(018)、**026 auth DRY refactor**,皆已 merge+推 origin。**Phase 3 兩條尾(非核心 gate)**:#5 axum-casbin 核心已被 013-026 吸收〔唯 metrics 埋點 → Phase 6〕、#6 受管 RBAC = 獨立 deferred 治理軌道〔隨時可做、需 §11.6 fork amendment〕。**Phase 5 進行中**:**027-refresh-token-rotation ✅** + **028-single-session-enforcement ✅**(per-account 可控 access 端單一-session:policy=開新登入即時踢舊〔`7777`「账号在他处登录」〕、關維持 027 多裝置、028 全站預設 off dormant;`Claims` +sid / pointer Redis+sys_user 混合 fail-open / 4 認證 gate + refresh pointer-first / policy 三態 + config;wire 中性、base-web 零改、無新端點/crate/amendment) + **029-single-session-admin-ui ✅**(系統預設 runtime 可調〔rev2 首張 `system_settings` KV + `settings_watcher` pub-sub〕+ 每帳號 policy UI + 設定頁〔MODAL-WIRING ★ (e) v1.6.0〕;不改 028 enforcement、雙倉、無新 crate)。**逐 feature deliverable/as-built 見 [§4 Roadmap](#4-roadmap--phase-狀態) + [DESIGN §10](INTEGRATION-DESIGN.md);merge 史見 [MILESTONES §1](INTEGRATION-MILESTONES.md)。**

**最新進展**(滾動最近 2 條;完整歷史見 [`docs/INTEGRATION-MILESTONES.md`](INTEGRATION-MILESTONES.md)):
- **2026-06-06 029-single-session-admin-ui ✅ merge `1ce835a`(--no-ff、保留 029 branch、已推 origin;rust-api `d958c02..27c03de` 11 commit push fork、base-web `16b675a8..75f2a202` 5 commit push fork)** — 028 policy 升級為 **admin runtime 可調**:rev2 首張 `system_settings` KV 表(§I.6 真審計)+ `AppState.session_mode: Arc<RwLock<SessionMode>>` + `settings_watcher`(pub-sub `settings:invalidate`、鏡像 policy_watcher)使 `is_current` 讀 runtime 預設 + 3 Super-only 端點(30→33)+ getUserList additive `sessionPolicy` + casbin/sys_menu seed(m028/m029)+ base-web 設定頁(MODAL-WIRING ★ (e))/ 使用者頁 policy 欄·modal。**不改 028 enforcement 邏輯、雙倉、無新 crate**。U1/U2/value_in_value_type + live-DB L1-L4 + curl C1-C6+C3b + **CDP isolated-context 雙頁 smoke**(設定頁 switch→DB 翻轉 / 使用者頁 policy 欄·modal)+ 守恆 server 224/lint 17/33 + migration 可逆 + base-web typecheck/prod build 全綠;subagent-driven T002-T021 雙審 + final holistic 三維 = READY TO MERGE。**constitution v1.6.0**(MODAL-WIRING ★ (e)、ratified `3bd3eda`)。詳見 MILESTONES §1 / [DESIGN §6.7·§10 Phase 5](INTEGRATION-DESIGN.md)。
- **2026-06-06 028-single-session-enforcement ✅ merge `d528e61`(--no-ff、保留 028 branch、已推 origin;rust-api `1bd0436..d958c02` 13 commit push fork、base-web 零改)** — per-account 可控 access 端單一-session(policy=開新登入即時踢舊〔舊 session 下個請求 **`7777`「账号在他处登录」** modal→乾淨登出〕、關維持 027 多裝置、**028 預設 off dormant**):`Claims` +`sid`(required) / pointer **Redis+sys_user 混合**(fail-open) / **4 認證 gate** + refresh pointer-first / 登入 revoke 舊鏈 + 一律 set_pointer / policy sys_user 三態 + config。**wire 中性、base-web 零改、無新端點/crate/amendment**。U1/U2/pre-028-reject + live-DB L1-L5 + curl C1-C4 + **CDP isolated-context modal smoke** + 守恆 server 221/lint 17/30 + migration 可逆 + prod build 全綠;subagent-driven T002-T021 雙審 + final holistic = READY TO MERGE。**2 follow-up(§2.33)**:same-second token_hash collision(027 latent、非 028 regr)+ base-web modal HARD-reload boot race。詳見 MILESTONES §1 / [DESIGN §6.6·§10 Phase 5](INTEGRATION-DESIGN.md);follow-up §2.33。
> 以下為預計`下一步` (不要再被合到`最新進展`了)

**下一步**: **cleanup-job(+ jti 修)= 已選定 Phase 5 唯一剩 feature**(stub ⊘moot 後;2026-06-06 user 親選)。**Phase 0 grounding 已收集** → [`docs/superpowers/030-cleanup-job.md`](superpowers/030-cleanup-job.md)(sys_token 刪除安全規則 / jti 範式 / prod-build 已備 / CLI·排程·credential 範式 + **3 待拍板決策**:範圍〔窄 sys_token vs 廣 soft-delete purge〕·排程機制·最小權限 DB 角色)。**下一動作**:階段 0 `superpowers:brainstorming`(讀 030 doc → 敲定 3 決策)→ user 手動 `/speckit-specify`。其餘候選續後:obs(Phase 6)/ #6 受管 RBAC(deferred、需 fork+§11.6 amendment)/ ButtonAuth「完整版」。
