...
- **2026-06-08 032-obs-full ✅ merge `a13f85f`(--no-ff、保留 032 branch、已推 origin;rust-api `2cb1ba7→87e3d8a` 2 commit push fork〔server metrics + cleanup-job push〕、base-web 零改;外層 4 commit〔US1 deploy+nginx / US2 exporters / US3 alerting / docs〕+ SHA pin `05f0bcf`)** — Phase 6 觀察性第二刀(metrics):opt-in `profiles:[metrics]` 堆疊 = prometheus(v3.12.0 retention 15d)← rust-api `/metrics`(axum-prometheus HTTP metrics + `enforce_mw` 自埋 `casbin_enforce_total{decision}` allow/deny counter、**閉 Phase 3 #5 enforce metrics 債**)+ postgres_exporter(v0.19.1)+ redis_exporter(v1.85.0-**alpine**〔bare scratch 無 /bin/sh、sh-wrapper 須 alpine〕)+ pushgateway(v1.11.3、cleanup-job best-effort push)→ grafana(prometheus datasource + 3 baseline alert rule)。**FR-011 nginx `/api/metrics` 擋塊**(R7/D8 漏抓 /api/ strip、user 親決 in-feature 補)。cleanup-job `metrics-exporter-prometheus` default-features=false + ureq(非 reqwest::blocking)。**無新 secret(reuse postgres/redis password)/migration/crate、base-web 零改、Constitution 8/8 PASS、無 amendment**。acceptance C1-C10 + **SC-005 firing 實測**(停 rust-api → rust-api-target-down inactive→pending→firing)+ 守恆全綠(server 225/cleanup-job 4/lint 17);subagent-driven 5 單元雙審 + final holistic = merged。詳見 MILESTONES §1 / [DESIGN §10 Phase 6 #2](INTEGRATION-DESIGN.md)。

> 以下為預計`下一步` (不要再被合到`最新進展`了)

**下一步**: **Phase 6 觀察性三刀全完成**(obs-min log `3383828` + obs-full metrics `a13f85f` + dashboard-provisioning `3d27cd5`、obs 從「能查」進化到「開即見 6 板」)。後續候選(user 擇期啟動):**#6 受管 RBAC**(deferred、需 §11.6 fork amendment)/ **obs prod 對外暴露 + 採集端硬化**(security pass、§2.36)/ **least-priv exporter / cleanup PG role**(§2.35 + 032 follow-up §2.37)。
...
### Phase 6 — 觀察性(可選,生產 ready)(obs-min ✅ 2026-06-07;obs-full ✅ 2026-06-08;dashboard-provisioning ✅ 2026-06-08)
...
- [x] **obs-full feature ✅**(032、merge `a13f85f`)— prometheus(v3.12.0 retention 15d)+ postgres_exporter(v0.19.1)+ redis_exporter(v1.85.0-**alpine**)+ pushgateway(v1.11.3、cleanup-job push)+ grafana prometheus datasource + **3 baseline alert rule**;rust-api `/metrics`(axum-prometheus HTTP metrics + `enforce_mw` allow/deny counter **閉 Phase 3 #5 enforce metrics 債**)+ **FR-011 nginx `/api/metrics` 擋塊**;無新 secret(reuse postgres/redis password)/migration/crate、base-web 零改、Constitution 8/8 PASS、無 amendment;acceptance C1-C10+SC-005 firing+守恆全綠;詳見 [DESIGN §10 Phase 6 #2](INTEGRATION-DESIGN.md) + `specs/032-obs-full/`
- [x] **dashboard-provisioning feature ✅**(033、merge `3d27cd5`)— 6 張 grafana dashboard provisioning(4 greenfield master-overview/rust-api/cleanup-job/audit-log〔schemaVersion 39〕 + 2 community pin postgres `postgres_mixin@v0.19.1`/redis `763 rev6`)+ provider yaml folder obs-full + loki.yml 補 uid:loki(`deleteDatasources` 收斂 persisted auto-gen uid);as-built 修正 R8 loki uid crash-loop + R7 rust-api `| json` guard;純 grafana config、base-web+rust-api 零改、Constitution 8/8 PASS、無 amendment;acceptance C0-C5+守恆全綠;詳見 [DESIGN §10 Phase 6 #3](INTEGRATION-DESIGN.md) + `specs/033-dashboard-provisioning/`
