...
**下一步**: **031-obs-min(Phase 6 obs、log-only)階段 0 brainstorm ✅ 完成** → spec-design [`docs/superpowers/031-obs-min.md`](superpowers/031-obs-min.md)(5 決策:D1 greenfield〔不參照 rev1〕/ D2 完整 log-only〔loki+promtail+grafana + rust-api request_id 注入 log + nginx stdout JSON、無 dashboard〕/ D3 nginx 改 stdout JSON / D4 profiles:[obs] inline / D5 沿用 `ctx_mw` tracing span 注入 trace_id 零新 dep)。**待 user 手動 `/speckit-specify`**(input=該 spec-design、pre-hook 建 `031-obs-min` branch)→ /speckit-plan → tasks → analyze → executing-plans。其餘候選續後:**obs-full**(metrics/prometheus + 3 exporter + axum-casbin metrics 埋點)/ dashboard provisioning / **#6 受管 RBAC**(deferred、需 §11.6 fork amendment)/ **030 follow-up**(§2.35 host 排程未建 / least-priv PG role)。
