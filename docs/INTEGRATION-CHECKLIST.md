...
- **2026-06-09 035-policy-governance-hardening ✅ merge `e841225`+推 origin(--no-ff、保留 035 branch;rust-api worktree `a2a52fb..1fa2ebd` 2 commit push fork〔1dca5b6 US1 / 1fa2ebd US2〕+ 外層 docs 回填 `470b76c` + SHA pin `031d3d8`;base-web 零改)** — 034 治理 facade 收尾硬化 2 US(純 rust-api 4 檔、無 migration/端點/crate/dep、對外 wire/錯誤碼/讀決策逐項不變):US1 restore 審計記 `{role,target,dimension}`(`RestoreOutcome::Restored` 帶 `{v0,v1,v2}` + 新 `restore_audit_payload` 純函式、取代懸空 archive_id、FR-002 還原後可解析)/ US2 no-op reload 跳過(trait `PolicyMutated` 條件化 `mutate_and_reload`、只跳結構性零變更〔Rejected/NoOp/NotFound/menu 查無〕、空-diff Applied 與 menu-found-無-policy 仍 reload〔FR-006〕、`batch_delete_menus` any_mutated、6 caller 零改)。server 225/0 + lint 17/5 + live restore payload roundtrip + prod image build + 讀決策不變(FR-005/SC-003) GREEN;SC-001~004 全 MET;**Constitution v1.6.0 8/8 PASS、無 amendment**。詳見 [DESIGN §10 Phase 3 #6](INTEGRATION-DESIGN.md) + [MILESTONES §1](INTEGRATION-MILESTONES.md)。

> 以下為預計`下一步` (不要再被合到`最新進展`了)

**下一步**: **無啟動中 feature**(035 已收、merge `e841225`)。下一個 feature 待 user 定主題 → Phase 0 `superpowers:brainstorming` → 手動 `/speckit-specify` 從 rev2-admin-root 建 feature branch。候選(擇期):casbin archive 懸空 cruft GC(D8、§2.39)/ obs prod 對外暴露+採集端硬化(§2.36)/ least-priv exporter+cleanup PG role(§2.35+§2.37)/ REVIEW-DATABASE 12-table live 重稽核(§2.39)/ Phase 7 維護項(wire 細節全功能巡檢、upstream rebase、graphify 更新)。
...
- [x] **REVIEW-DATABASE 12-table live 重稽核 → ✅ 2026-06-09 完成**:034+035 merge 後對全新 wipe+seed DB(m001–m035)做完整 11→12 表 live-vs-migration re-audit —— `casbin_rule` +3 治理欄〔protected/created_at/created_by、m031〕/ 新 `sys_casbin_policy_archive` 13 欄+2 功能索引〔m032〕/ `sys_menu` +`protected`〔m034〕全 live-verified **MATCH、零 drift**;seed 計數 casbin 69→72〔US5 +2 endpoint +1 menu policy〕、sys_menu 9→10、protected 19/8;舊 dev-DB 測試殘留 finding 標 **CLEARED**(wipe 已清)。詳見 [REVIEW-DATABASE](REVIEW-DATABASE.md)。
