...
**階段**:**Phase 1 P0 部署基建全數完成(#1~#5)**;**Phase 2 P1 全數完成(007~012)— 007-db-redis-connection(DB/Redis 連線層 + migration proof)、008-response-envelope(統一回應信封 `Res<T>` + `BizCode` 矩陣 + `AppError` + 404 fallback)、009-soft-delete-infra(soft-delete 三重防護:trait / facade / build-time lint + `sys_user` proof)、010-migration-auto-apply(dev/prod stack `up` 自動套 migration + `service_completed_successfully` fail-fast 閘門)、011-audit-log(統一 audit 基礎設施:`sys_operation_log` 表 + `mutate_in_txn` 唯一原子寫入入口 + redact,以 `sys_user soft_delete` 作活體 proof)、012-sub-crate-setup(Casbin RBAC 工具層地基:sea-orm-adapter + xdb 拷貝 rev1、casbin 2.20、casbin_rule migration 005、活體 smoke)皆完整實作+驗收+merge 回 `rev2-admin-root`**。Phase 2 餘(非獨立 feature、隨各 entity / 寫入路徑建立時沿用 pattern):soft-delete 6-entity rollout / audit 其他 operation·entity 接線(沿用 011 `mutate_in_txn` pattern)。**Phase 3 RBAC 起手(013-auth-login-enforce)✅ + 014-dynamic-routes ✅ 皆已落地 + merge**(013=認證地基 login/getUserInfo/JWT/refresh + 首個 Casbin enforce 點 allow+deny + 首條真 base-web↔rev2 CDP wire;014=base-web 翻 dynamic auth route mode + 3 route endpoint〔getConstantRoutes 公開/getUserRoutes·isRouteExist JWT〕+ **業務 menu 可見性走 Casbin enforce 過濾**〔menu-visibility policy 9 rows + 013 enforcer + tree-prune,§I.2〕+ CDP Super-vs-User 側欄 menu deny 端到端);**Phase 3 續做** = enforce 全路由 rollout + 完整 policy 矩陣 / 受管 RBAC policy 層〔casbin_rule soft-delete/protected/audit/CRUD〕/ axum-casbin fuller rewrite / redis pub-sub policy 失效通知(見 §1 下一步)。**Phase 4 主流業務起手(016-manage-role-user-list)✅ 已落地 + merge**(base-web manage 頁 role+user 3 read endpoint:getRoleList〔分頁+搜尋〕/getUserList〔分頁+userRoles+**無 password**、取代 013 stub〕/getAllRoles〔不分頁〕+ sys_role +4 欄/sys_user +6 欄〔VARCHAR status/gender〕+ 3 條 Casbin enforce + **id wire=number**〔constitution v1.1.0〕+ **立「分頁 facade seam + Output DTO + enforce 掛路由」pattern 供 017 menu 沿用**;menu 三件〔getMenuList/v2·getAllPages·getMenuTree、需建選單表〕留 017)。**Phase 4 第二刀(017-manage-menu-list)✅ 已落地 + merge**(menu 三件 + 建 `sys_menu` 21 欄選單表〔扁平+2 JSONB route_ext/buttons、seed 6 節點對齊 014〕+ m..016/017 兩 migration + 3 Casbin enforce〔getMenuList/v2 分頁巢狀樹 / getMenuTree 輕量樹 / **getAllPages 唯一 role-aware**:Super real+demo·Admin 只 real〕;**唯讀、不動 014 getUserRoutes〔雙源並存〕、可見性續住 casbin〔不建 sys_role_menu〕、demo 只進 getAllPages catalog 不 seed 成 menu**;沿用 016 PageResp/normalize_page/facade pattern、getMenuList 取全+記憶體組樹+頂層 slice〔非 016 DB 分頁、D-A〕;menu CRUD / role-menu 授權寫入 / 014 改讀 sys_menu 留後續)。**JWT 機密管理已由 005/007 吸收 ✅**(見 [§4 Roadmap Phase 2](#4-roadmap--phase-狀態))。

**最新進展**(滾動最近 2 條;完整歷史見 [`docs/INTEGRATION-MILESTONES.md`](INTEGRATION-MILESTONES.md)):
- **2026-05-31 018-userroutes-sys-menu 實作+驗收**(rust-api `d4cb09e`+`85f95f1`)— **Phase 4 第三刀:getUserRoutes 改讀 sys_menu(單一真相源、拆 D-D)**。純函式 `build_route_tree(Vec<MenuNode>)→MenuRoute`(parent_id 巢狀 + menu_order 升冪 null-last + None component→"")+ `filter_routes_for_roles` 純化吃 `(routes,roles,enforcer)`(Casbin 邏輯零變更)+ 移除 in-code `business_routes()`;handler `all_active`→map `MenuNode`→build→filter。US2 前置:m..018 seed 修 manage_user-detail component+`{"props":true}`(forward-only、回 base-web 權威)。**Option C**:builder 吃本地 DTO 非 entity::Model(沿 017 慣例避 lint、不動 facade)。三角色 wire 對 014 零差異(struct 未變)、唯讀(FR-008)、server 119 測 + lint 17。詳見 [MILESTONES](INTEGRATION-MILESTONES.md) + specs/018
- **2026-05-31 017-manage-menu-list 完整實作 + 驗收 + merge**(rust-api worktree 7 commits `841b6f5..d66b309` push fork;**兩段式 commit、不動 base-web/014**)— **Phase 4 主流業務第二刀:menu 三件 + 建 `sys_menu` 選單表**。getMenuList/v2(分頁巢狀樹)/ getMenuTree(輕量樹 {id,label,pId,children})/ getAllPages(**唯一 role-aware**:R_SUPER real+demo、R_ADMIN 只 real)。建 sys_menu 21 欄(扁平 + route_ext/buttons 兩 JSONB、partial unique route_name、seed 6 節點對齊 014)+ m..016/017 兩 migration(create+seed / 6 endpoint policy)。**id wire=number**、route_name=casbin v2='menu' 對齊鍵。getMenuList 取全 active+記憶體組樹+頂層 slice(非 016 DB 分頁、D-A);getAllPages 自讀 Claims.roles 做 demo 過濾(double-JWT、D-B);唯讀、不動 014、可見性續住 casbin(不建 sys_role_menu)、demo 只進 catalog 不 seed 成 menu(D-E)。純測 14(build_menu_tree/lite/route_ext/paginate/pages_for_roles/facade) + 三 US 活體(Super⊋Admin 差集38 + enforce) + server **112**/3 + lint 17 + prod build。Constitution **14 PASS、無 violation/Amendment/§I.5 例外**。subagent-driven 13 task、各 spec+quality 雙審 + final review。詳見 MILESTONES + specs/017。follow-up 見 §2.20
**下一步**: **Phase 4 第一刀(016 role+user)+ 第二刀(017 menu 三件 + sys_menu 表)已落地** — manage 頁 6 read endpoint(role/user/menu)全交、分頁 facade + Output DTO + enforce pattern 成熟。**Phase 4 續做**:alova-only endpoint(§11.2)/ menu CRUD + role-menu 授權寫入 + 014 getUserRoutes 改讀 sys_menu(單一真相源統一,[§2.20](#220-feature-017-manage-menu-list-follow-up))/ **Phase 4 A** 資料變動補 operator(createBy/updateBy + sys_operation_log INET retrofit [§2.18](#218-feature-015-audit-middleware-follow-up) + status/gender DB CHECK 約束,D-3)。**Phase 3 續做**([DESIGN §10 Phase 3](INTEGRATION-DESIGN.md)):#3 redis pub-sub(policy/route 失效通知)/ #4 完整 policy 矩陣 + 全路由 endpoint enforce rollout(含 016/017 新端點 enforce 單測補,[§2.19](#219-feature-016-manage-role-user-list-follow-up)/[§2.20](#220-feature-017-manage-menu-list-follow-up))/ #5 axum-casbin fuller rewrite(metrics/error/observability)/ #6 受管 RBAC policy 層(casbin_rule soft-delete 可復原 / protected / 走 011 audit / CRUD;**需 fork sea-orm-adapter 的 `load_policy`/`remove_*` + casbin_rule 加 deleted_at → 須評估 §11.6 Amendment**)。Phase 2 餘(非獨立 feature):soft-delete 6-entity rollout(含 sys_user 完整欄 + `id` auto_increment,見 §2.10)/ audit 其他 operation·entity 接線(沿用 011 `mutate_in_txn` pattern)
...
### 2.20 feature 017-manage-menu-list follow-up
> **D-D(getUserRoutes↔sys_menu 雙源)✅ 2026-05-31 已解** — 018 getUserRoutes 改讀 sys_menu、移除 in-code `business_routes()`。詳見 [MILESTONES §1](INTEGRATION-MILESTONES.md)。

- [x] **單一真相源統一(D-D)**:017 建 sys_menu 表 seed 6 節點對齊 014 `business_routes()`,但 **014 getUserRoutes 仍讀 in-code 路由樹**(雙源並存、皆唯讀 seed-only → 無漂移)。「014 改讀 sys_menu」= 後續 feature(連動 menu CRUD;改後單一真相源)。 ✅ **2026-05-31 已解(018 getUserRoutes 改讀 sys_menu、移除 in-code business_routes)**
- [ ] **menu CRUD / role-menu 授權寫入 / demo-as-menu / operator / 按鈕權限(D-E、FR-011 scope-out)**:017 唯讀;menu 新增/修改/刪除、角色-選單授權 modal 儲存、createBy/updateBy(Phase 4 A)、按鈕權限授予/強制(需 sys_menu_button)皆留後續。
- [ ] **getMenuList 非 DB 分頁(D-A)**:取全 active + 記憶體組樹 + 頂層 slice;規模假設 = menu 有界小(導覽設定)。若 menu 量大需改 DB 分頁(recursive CTE、破 facade seam)再評估。
- [ ] **getAllPages double-JWT-parse(D-B)**:enforce_mw 閘 + handler 自讀 Claims.roles 各 parse 一次 = §2.17 DRY debt;待 enforce 全路由 rollout 抽 `verify_and_load_roles` helper 時一併。
- [ ] **DEMO_PAGES 手動同步(D-C)**:`DEMO_PAGES`(38 條)對齊 base-web `routes.ts` 的 `view.X` 頁級 route key、§I.5 未 grep rev1;base-web 增刪 demo 頁需同步此 in-code const(無自動偵測)。
- [ ] **enforce 單測未覆蓋 3 新端點**(同 §2.19):getMenuList/v2·getMenuTree·getAllPages 靠 m..017 seed + 活體 acceptance 驗、無 unit-level enforce 決策測 → 連動 §4 全路由 enforce rollout 補。
- [ ] **無 CDP smoke(純後端 read)**:curl 直送驗 wire shape ≠ base-web manage/menu tree-table 實際渲染 + 角色-選單授權 modal tree-select 對齊;留 **prod-stack CDP 巡檢**(連動 [§2.8](#28-feature-004-compose-port-orchestration-follow-up)/[§2.16](#216-feature-013-auth-login-enforce-follow-up)/[§2.18](#218-feature-015-audit-middleware-follow-up) cluster)。
- [ ] **`build_menu_tree_lite` 無 sort 依賴 facade 排序**(review note):lite 版不重排、靠 `all_active` 的 `ORDER BY menu_order ASC` 保序;若未來有別的 caller 餵未排序輸入則 children 序失準(現唯一 caller 是 getMenuTree、安全)。
...
- [x] **MenuType enum**:1=directory / 2=menu(非舊推測「1=group / 2=page」)(§4.2.1)— ✅ 017 sys_menu seed + getMenuList/v2 活體(manage menuType='1' 目錄、葉 '2' 選單;iconType '1'/'2' 同);constitution §I.3 鎖定
- [x] **Status nullable**:`CommonRecord.status: EnableStatus \| null` rust-api 須支援(§4.2.2)— ✅ 016/017 送非 null `EnableStatus`('1'/'2') 子集滿足(menu 永有 status default '1'、同 RoleItem/UserItem 既驗法)
