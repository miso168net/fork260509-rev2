...
- **2026-05-31 constitution v1.2.0 修憲**(`e21a642`)— 新增 **§I.6 業務表審計欄標準(SCHEMA-AUDIT-COLUMNS)**:業務主表建表 MUST 含 6 審計欄(created/updated/deleted × at/by)、`*_by`=`Option<i64>` operator(對齊 sys_operation_log/sys_login_attempt);append-only 審計表 + 硬刪 join 表豁免;forward-only、既有 sys_user/sys_role/sys_menu 的 `*_by` 缺口折 Phase 4 A。+ §IV Compliance Check #8、回填 DESIGN §11.14 + §10 Phase 4 A。詳見 [MILESTONES](INTEGRATION-MILESTONES.md) + [constitution §I.6](../.specify/memory/constitution.md)
...
- [ ] **單一真相源統一(D-D)**:017 建 sys_menu 表 seed 6 節點對齊 014 `business_routes()`,但 **014 getUserRoutes 仍讀 in-code 路由樹**(雙源並存、皆唯讀 seed-only → 無漂移)。「014 改讀 sys_menu」= 後續 feature(連動 menu CRUD;改後單一真相源)。
