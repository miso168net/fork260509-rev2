...
- [~] manage list endpoints feature(6 read endpoint,對齊 mock)— **016 第一刀已交**(user/role 三條:getUserList/getRoleList 分頁 + getAllRoles 全量,2026-06-01 / SHA pin `81c56ef`、merge SHA 見 [MILESTONES](INTEGRATION-MILESTONES.md));**menu 三條(getMenuList/v2、getAllPages、getMenuTree)留後續**(需 sys_menu 表)
- [~] wire shape mapping feature(Output DTO + `From<Entity>` + pagination wrapper)— **016 已立骨架**(PageRes<T> 分頁外殼 + UserItem/RoleItem/AllRoleItem DTO + lint-safe primitive 映射 seam);其餘 entity 隨各 endpoint 接線
