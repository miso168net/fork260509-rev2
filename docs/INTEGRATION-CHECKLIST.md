...
**階段**:**Phase 1 部署基建 全數完成(001~006)**;**Phase 2 後端基礎設施 全數完成(007~012)**;**Phase 3 RBAC 起手(013-auth-login-enforce)✅ + 014-dynamic-routes ✅ + 015-audit-middleware ✅ 皆已落地 + merge**(013=認證地基 login/getUserInfo/JWT/refresh + 首個 Casbin enforce 點 allow+deny + 首條真 base-web↔rev2 CDP wire;014=base-web 翻 dynamic auth route mode + 3 route endpoint〔getConstantRoutes 公開/getUserRoutes·isRouteExist JWT〕+ **業務 menu 可見性走 Casbin enforce 過濾**〔menu-visibility policy 9 rows + 013 enforcer + tree-prune,§I.2〕+ CDP Super-vs-User 側欄 menu deny 端到端;015=請求層 request-context middleware + 兩 append-only 審計表〔sys_access_log/sys_login_attempt〕+ 首個 xdb 消費者 + 真值 INET client_ip);**Phase 3 續做** = enforce 全路由 rollout + 完整 policy 矩陣 / 受管 RBAC policy 層〔casbin_rule soft-delete/protected/audit/CRUD〕/ axum-casbin fuller rewrite / redis pub-sub policy 失效通知(見 §1 下一步)。**JWT 機密管理已由 005/007 吸收 ✅**(見 [§4 Roadmap Phase 2](#4-roadmap--phase-狀態))。**Phase 4 主流業務起手(016-manage-role-user-list)✅**:3 條唯讀 systemManage endpoint(getUserList/getRoleList 分頁 + getAllRoles 全量)落地 + merge,**管理頁第一次看到真實 user/role 資料**(取代 013 stub)。**Phase 4 user 寫端 CRUD(017-manage-user-write)✅**:4 條 Super-only 寫 endpoint(addUser/updateUser/deleteUser/batchDeleteUser)+ sys_user schema 完補(業務欄 + **§I.6 審計欄 retrofit DONE** + id BIGSERIAL)+ 停用拒登 1000 + base-web MODAL-WIRING,**管理頁現可完整 CRUD users**(**已 merge+已推**)。**Phase 4 role 寫端 CRUD(018-manage-role-write)✅(已 merge+已推)**:4 條 Super-only 寫 endpoint(addRole/updateRole/deleteRole/batchDeleteRole)+ sys_role §I.6 retrofit(migration 016 +7 欄)+ casbin seed 017 + **有效角色集 `find_active_enabled`(deleted_at IS NULL AND status=1)統一**(停用/軟刪角色不授權·不可指派)+ **★ enforce_mw 改讀 DB-fresh 有效角色(B 決策)→ 停用/刪角色下次授權即時失效**(舊 token 不重登;順帶 D5 校正 017 update_user 審計時間源全 DB-side)+ 種子保護(code-based、不可刪/停用、可改 name·desc)+ base-web MODAL-WIRING,**管理頁現可完整 CRUD roles + status 真正生效於授權**。manage 另 3 條(getMenuList/v2、getAllPages、getMenuTree、需 sys_menu 表)續做。**Phase 4 menu DB-driven(019-manage-menu-list)✅(已 merge `a07ff13`+已推 origin)**:建 `sys_menu` 業務表(§I.6 凍結後**首張新建業務表**、create 即帶 6 審計欄、forward-only)為選單定義單一真相,**getUserRoutes 改讀 sys_menu(輸出逐字不變=回歸鐵律 D5)+ 管理頁三讀端(getMenuList/v2·getMenuTree·getAllPages,Super-only)共讀同源**;可見性仍走 Casbin enforce(§I.2 不變、010 policy 不動)。**server-only**(base-web menu fns 已存在 upstream、getUserRoutes wire 透明 → base-web 未動)。**Phase 4 menu 寫端 CRUD(020-manage-menu-write)✅(merge `0c73e3b`、保留 020 branch、**已推 origin**)**:4 條 Super-only 寫端(addMenu/updateMenu/deleteMenu/batchDeleteMenu)寫 019 `sys_menu`、閉成完整選單管理 CRUD = **D1 統一源 payoff**(編輯既有可見選單即時反映 getUserRoutes、`get_user_routes` code 不動);routeName·menuType immutable 雙鎖 + 種子/父子 guard + batch 兩段原子拒 + §I.6 成對審計;**不建表/欄**(用 019 sys_menu)、**D3 不碰 casbin 可見性**(010 不動);**rust-api + base-web 兩倉**(facade 寫端 + 4 handler + migration write policy + MODAL-WIRING)。MenuAuth·ButtonAuth 編輯(後續、動 010 policy)+ alova stub(Phase 5)續做(見 [§4 Phase 4](#4-roadmap--phase-狀態))。**Phase 4 MenuAuth(021-manage-menu-auth)✅(已 merge `827d1e9`+已推 origin)**:角色×選單可見性 **runtime 編輯**(casbin `remove_filtered_policy`+`add_policies` stock adapter HARD REPLACE、不 fork、即時反映無重啟)+ **per-role home**(sys_role +home、getUserRoutes 取第一 active 角色 id ASC)+ **redis pub-sub 失效通知**(`casbin:policy:invalidate`、**順帶實現 Phase 3 #3**)。閉合 menu arc(019 讀→020 寫→**021 指派**)、解 020「新選單指派前不顯 nav」缺口;自鎖 guard(R_SUPER 須留 manage_menu)。**+1 dep tokio-stream**(消費 redis pubsub Stream、親決)。**Phase 4 ButtonAuth(022-manage-button-auth)✅(已 merge `e103de0`+已推 origin)**:角色×按鈕權限 **runtime 編輯**(鏡像 021、Casbin `(role,code,'button')` HARD REPLACE 不 fork、無自鎖、即時反映)+ **per-menu 按鈕來源**(sys_menu.buttons 聚合 = registry、key-on-code)+ **pilot 用戶頁 hasAuth gating**(端到端生效:Admin 見編輯·不見刪除/新增、Super 全)+ **救活 toggle-auth demo**(seed 入 sys_menu + menu policy 三角色、保 B_CODE1/2/3 可驗);getUserInfo.buttons 改讀 Casbin、buttons.rs 退場;**getUserRoutes 逐字基線 re-base**(三角色 +function/toggle-auth、唯一 intended 例外);CDP 4/4(toggle-auth 3/2/1 + 用戶頁 gating + button-modal registry + 順補 021 menu-modal click-through)+ 守恆(server 189/lint 17)全綠;單一 migration 022、**無新 crate/dep**;Constitution v1.3.0 §IV 8/8 PASS。**menu+button auth arc 完整閉合**。**Phase 3 #4 EndpointAuth(023-manage-endpoint-auth)✅(已 merge `ef5ebe0`+已推 origin)**:角色×API-endpoint 權限 **runtime 編輯**(鏡像 022、Casbin `(role,path,method)` 逐 method HARD REPLACE 保 menu/button 正交、無新 crate/dep/fork)+ **root-mode**(R_SUPER 不可編輯→自鎖歸零)+ **D1 build-time 靜態守衛**(main.rs↔migration↔ENDPOINT_REGISTRY 三方一致、關 path-typo/漏 seed 死路由破口、negative fixture 證咬合)+ 接口权限 modal(root-mode disabled-for-Super)+ §11.22 過時 `p,R_SUPER,*,*` wildcard→per-endpoint 矩陣 reconcile(menu-read 三列 Super-only、FR-007 單一真相)。**reframe**:全路由 enforce 字面早完成(25/25 零破口)、本波交付 runtime 可編輯+治理+守衛。migration 023 只 seed 3 治理端點。Constitution v1.4.0 §IV 8/8 PASS。**menu+button+endpoint auth 三本柱完整閉合**。**Phase 4 ButtonAuth Rollout(024-button-auth-rollout)✅(已 merge `64d9bec`+已推 origin)**:把 022 用戶頁按鈕權限可編輯迴路 **rollout 到角色頁/選單頁** + 三頁 reactive 修正(§2.26 閉合)。唯一 rust=1 seed migration(role:*/menu:* registry + R_SUPER 6 button policy、後端零改)→ `aggregate_active_buttons` 自動納碼;base-web 三頁 hasAuth gating(menu 加子選單 menuType 疊加)+ `watch(userInfo.buttons)→reloadColumns` reactive。decoupled 刻意(button 可見≠端點可呼叫)+ 跨維度 as-built(非-Super 無 role/menu 選單可見性→gating 只在能檢視頁角色可觀察)。acceptance 全綠(確定性 7 維度 + D3 + 3 對抗式 refuted + CDP 5/5)、無新 crate/dep/fork/endpoint/表/單元測試、Constitution v1.4.0 §IV 8/8 PASS(**無 amendment**)。

**最新進展**(滾動最近 2 條;完整歷史見 [`docs/INTEGRATION-MILESTONES.md`](INTEGRATION-MILESTONES.md)):
- **2026-06-04 024-button-auth-rollout 完整實作 + holistic acceptance/CDP 驗收 + Merge `64d9bec` 回 rev2-admin-root(--no-ff、保留 024 branch)+ 已推 origin**(rust-api `ab7ad2f..f7cd042` 1 commit push fork / base-web `95167f55..be3e54a` 3 commit push fork)— **ButtonAuth Rollout:022 用戶頁按鈕權限可編輯迴路 rollout 到角色/選單頁 + 三頁 reactive 修正(§2.26 閉合)**。唯一 rust=1 seed migration `m20260529_000024`(role:*/menu:* registry + R_SUPER 6 button policy、down by-code 可逆、後端零改)→ `aggregate_active_buttons`(聚合全 active sys_menu.buttons)自動納碼、getAllButtons 12;base-web 三頁 hasAuth gating(menu 加子選單 `menuType==='1'&&hasAuth('menu:add')` 疊加保留資料條件)+ `watch(userInfo.buttons)→reloadColumns` reactive(鏡像既有 locale watch、三頁 byte-consistent)。**decoupled 刻意**(button 可見≠端點可呼叫、FR-008)+ **跨維度 as-built**(預設只 R_SUPER 有 manage_role/menu 選單可見性→非-Super 導向 404、role/menu 頁 gating 只在能檢視頁角色可觀察、literal US1 須先 021 授選單)。subagent-driven T002-T005 逐單元 spec+quality 雙審 + final holistic = READY TO MERGE。**acceptance 全綠**:確定性 7 維度 + D3 round-trip 即時(redis pub-sub 無重啟)+ **3 對抗式 skeptic refuted**(可逆 57→63→57/022 零回歸 純 additive/decoupled 保留 實證)+ **CDP 5/5 isolated-context**(Super 三寫鈕 + Super /manage/menu 全顯 + modal 6 新碼可勾〔virtual-scroll〕+ 撤 Super role:delete→删除消失 逐碼隱藏 + literal US1 授 Admin menu+button→見編輯)+ 守恆(server 195/lint 17/Migrator::up 0/typecheck 0)+ migration 可逆 16→10→16 + 022 矩陣 10 不變(FR-007)。**無新 crate/dep/fork/endpoint/表/單元測試**;Constitution v1.4.0 §IV 8/8 PASS(MODAL-WIRING v1.3.0 (b) 既有邊界內、無 amendment)。follow-up 見 §2.28
- **2026-06-04 023-manage-endpoint-auth 完整實作 + C-V/CDP 驗收 + Merge `ef5ebe0` 回 rev2-admin-root(--no-ff、保留 023 branch)+ 已推 origin**(rust-api worktree `22aa0b5..ab7ad2f` push fork〔migration 023 / endpoint_auth+6 單測 / 3 handler+route / D1 lint〕/ base-web `bd210d2..95167f55` push fork〔wire/modal/drawer〕)— **EndpointAuth 角色×API-endpoint 權限 runtime 編輯**:reframe(grounding 揭露全路由 enforce 字面早完成 25/25 零破口)→ 交付 **runtime 可編輯 + 治理 + 防呆守衛**。新 `auth/endpoint_auth.rs`(鏡像 022、唯一 divergence=endpoint 列 v2=HTTP method 故**逐 method `remove_filtered` HARD REPLACE 保 menu/button 正交**、絕不裸空-v2)+ 3 Super-only handler(getAllEndpoints 回 28 const registry infallible / getRoleEndpoints 預載〔R_SUPER→全 registry〕/ updateRoleEndpoints HARD REPLACE + 011 audit + 共用 021 watcher)+ **root-mode**(拒編輯 R_SUPER→2222、自鎖歸零)+ **D1 build-time 靜態守衛**(text-parse main.rs↔migration↔ENDPOINT_REGISTRY 三方一致、關 path-typo/漏 seed 死路由破口、negative fixture 證咬合)+ **接口权限 modal**(鏡像 button-auth、composite key、root-mode disabled-for-Super)+ §11.22 wildcard 矩陣 reconcile(per-endpoint、menu-read 三列 Super-only)。migration 023 只 seed 3 治理端點、**無新 crate/dep/fork/表**。subagent-driven 12 單元逐一 spec+quality 雙審 + 最終 holistic = READY TO MERGE。**acceptance 全綠**:contracts §1-7 curl/psql/redis/audit + CDP isolated-context(① 非-Super 可編輯 ② root-mode disabled)+ migration 可逆 + 守恆(server 195 unit+D1 5+lint 17、零 entity::、Migrator::up 0)+ 回歸 013/016/019/021/022 不破、既有矩陣不變(FR-010)。Constitution v1.4.0 §IV 8/8 PASS。follow-up 見 §2.27
> 以下為預計`下一步` (不要再被合到`最新進展`了)

**下一步**: **024 ButtonAuth Rollout ✅ 完成**(merge `64d9bec`、--no-ff、保留 024 branch、已推 origin;角色/選單頁納入按鈕權限可編輯迴路 + 三頁 reactive 修正〔§2.26 閉合〕)。**之後候選**([DESIGN §10](INTEGRATION-DESIGN.md)):Phase 3 #5 axum-casbin fuller rewrite / #6 受管 RBAC policy 層(需 fork→§11.6 Amendment)/ Phase 4 餘(選單 restore·re-parent、alova 3 stub Phase 5)/ Phase 2 餘(soft-delete 6-entity rollout、audit 其他)/ ButtonAuth「完整版」(aligned visible=clickable、024 decoupled 債對齊 023 端點)
...
- [ ] **spec doc 行號 off-by-one(cosmetic)**:tasks/research 引用 DESIGN「L749-752」,§11.22 reconcile 後實際 §6.3 body 在 L750;內容正確、僅行號漂移(同既往 spec-doc 回填債 pattern,可選補)。
...
- [x] **policy seed / 全路由 rollout + 矩陣治理 feature(#4)** ✅ (2026-06-04 merge `ef5ebe0`、023-manage-endpoint-auth 收口)— **013 第一刀** migration 009 seed 示範路由 × {R_SUPER,R_ADMIN};**016 第二刀** migration 013 補 getRoleList/getAllRoles;**017-022 每個寫/讀/auth feature 各隨 migration 逐條 seed Super-only endpoint policy(無 wildcard)**;**023 收口**:grounding 證全路由 enforce 字面早完成(25/25 零破口)→ 不補端點、改交付 **runtime 可編輯 endpoint policy**(逐 method HARD REPLACE)+ **D1 build-time 守衛**(main.rs↔migration↔ENDPOINT_REGISTRY 三方一致、關 path-typo/漏 seed 死路由破口)+ 接口权限 modal(root-mode)+ **§11.22 wildcard→per-endpoint 矩陣 reconcile**(menu-read 三列 Super-only、FR-007 單一真相)。詳見 [DESIGN §10 Phase 3 #4](INTEGRATION-DESIGN.md) + `specs/023-manage-endpoint-auth/`;follow-up [§2.27](#227-feature-023-manage-endpoint-auth-follow-up)
...
- [x] **ButtonAuth Rollout feature(024)✅ 已 merge `64d9bec`(--no-ff、保留 024 branch)+ 已推 origin** — 把 022「按鈕權限可編輯迴路」**rollout 到角色頁/選單頁** + 三頁 reactive 修正(§2.26 閉合)。**唯一 rust=1 seed migration `m20260529_000024`**(後端機制/modal/wire 零改):(a) UPDATE manage_role/manage_menu.buttons registry(role:*/menu:* 各 3 碼)+ (b) casbin button policy 6 列只 R_SUPER(K4);down by-code 精準可逆(不踩 022 的 10 列)、up→down→up 16→10→16。後端零改:`aggregate_active_buttons`(聚合全 active sys_menu.buttons)自動納碼 → modal registry 12 碼、`set_role_button` HARD REPLACE code-agnostic、getUserInfo.buttons 自動含。base-web 三頁(MODAL-WIRING ★ v1.3.0 (b) 既有邊界內、無 amendment):role/menu/user index.vue hasAuth gating(menu 加子選單 menuType 疊加)+ `watch(userInfo.buttons)→reloadColumns` reactive(三頁 byte-consistent、§2.26 閉合)。**decoupled 刻意**(button 可見≠端點可呼叫 FR-008)+ 跨維度 as-built(非-Super 無 role/menu 選單可見性、gating 只在能檢視頁角色可觀察)。acceptance 全綠(確定性 7 維度 + D3 round-trip + 3 對抗式 refuted + CDP 5/5、`tests/024-button-auth-rollout/`)。**無新 crate/dep/fork/endpoint/表/單元測試**;Constitution v1.4.0 §IV 8/8 PASS。詳見 [DESIGN §10 Phase 4 ButtonAuth Rollout as-built](INTEGRATION-DESIGN.md) + `specs/024-button-auth-rollout/`;follow-up [§2.28](#228-feature-024-button-auth-rollout-follow-up)
- [~] wire shape mapping feature(Output DTO + `From<Entity>` + pagination wrapper)— **016 已立骨架**(PageRes<T> 分頁外殼 + UserItem/RoleItem/AllRoleItem DTO + lint-safe primitive 映射 seam);**017 user_item 改吃 sys_user 真值**(gender/status i16→string、時間 rfc3339、operator i64→string,缺值仍 null)+ **018 role_item 改吃 sys_role 真值**(role_desc/status i16→string、create/update time rfc3339、operator i64→string,缺值仍 null)+ **018 role 寫端 DTO**(RoleCreateReq / RoleUpdateReq〔省 roleCode,D2 immutable〕)+ **019 menu 讀端 DTO/mapper**(MenuItem〔flat 分頁、parentId top→"0"〕 + MenuTreeNode〔id/pId number、root pId=0〕 + MenuNode 樹中介型 + `menu_node_to_route`〔MenuNode→MenuRoute、props 由路徑衍生〕 + `parent_id_to_wire` 純 helper、reuse 既有 MenuRoute/RouteMeta struct 保 wire 序)+ **020 menu 寫端 DTO**(MenuCreateReq〔parentId 用 `de_parent_id` 彈性反序列化 number\|string\|null→Option<i64>、`order` wire key〕 / MenuUpdateReq〔含 id:String、**省 routeName/menuType/parentId**,D2+FR-011 immutable〕 + CreateMenuData/UpdateMenuData facade 型)+ **022 button DTO**(`ButtonItem{code,desc}` 讀端 registry + `RoleButtonReq{roleId,codes:string[]}` 寫端 + 私有 `aggregate_active_buttons` 聚合 helper〔sys_menu.buttons flatten→dedup-by-code 字典序、getAllButtons + updateRoleButton F1 驗證共用〕、key-on-code、getUserInfo.buttons 仍 `Vec<String>` 不變)+ **023 endpoint DTO**(`Endpoint{method,path}` 讀/寫共用〔serde camelCase、method/path 單字無 rename 差〕 + `RoleEndpointReq{roleId,endpoints:Endpoint[]}` 寫端〔`de_role_id` 彈性 roleId〕;`ENDPOINT_REGISTRY` 28-const〔(method,path) tuple〕為 getAllEndpoints 來源、casbin 列 `[role,path,method]`、modal composite key `"method path"` 僅 NTree 內部);其餘 entity 隨各 endpoint 接線
...
- [x] `BASE-WEB-WRAPPER` ✅ (2026-06-02,017+018+020;2026-06-04 +023):`src/service/api/rev2-system-manage.ts` — 017 加 user 4 fn(fetchAddUser/Update/Delete/BatchDelete)+ **018 加 role 4 fn(fetchAddRole/UpdateRole/DeleteRole/BatchDeleteRole)** + **020 加 menu 4 fn(fetchAddMenu/UpdateMenu/DeleteMenu/BatchDeleteMenu)** + **022 加 button 3 fn(fetchGetAllButtons/GetRoleButton/UpdateRoleButton)** + **023 加 endpoint 3 fn(fetchGetAllEndpoints〔`Endpoint[]`〕/fetchGetRoleEndpoints〔`Endpoint[]`〕/fetchUpdateRoleEndpoints〔`null`〕)**、`request<null|T>`、index.ts `export *`
...
- [x] `MODAL-WIRING` ★ (v1.4.0 邊界擴:同模式新權限 modal+trigger)✅ (2026-06-04,023):**新 `endpoint-auth-modal.vue`**(鏡像 button-auth-modal、NTree composite key `"method path"`、`watch(visible)` 重載、root-mode 對 R_SUPER 全勾+disabled)+ **role-operate-drawer 新 trigger NButton**(v-if=isEdit、`:role-code` 傳 root-mode);constitution v1.4.0 §III.2 (c) 子句授權(amendment `d700434`、DESIGN §11.21);CDP isolated-context ① 可編輯 ② root-mode disabled 雙證
...
- **MODAL-WIRING ★**(DESIGN §7.4;**邊界 v1.4.0 擴**):允許動 `views/manage/**` 內 `// request` 一行(含 `modules/*-operate-{modal,drawer}.vue` create/update **+** `index.vue` 的 delete/batchDelete handler)+ **(v1.3.0)業務頁操作鈕 `hasAuth(<button_code>)` 可見性 gating**(`index.vue` v-if/JSX 條件渲染 + 共用元件如 `table-header-operation.vue` 附加 prop、安全 default;DESIGN §11.19)+ **(v1.4.0 (c))同模式新權限 modal + drawer trigger**(嚴格對齊既有 MenuAuth/ButtonAuth 範式〔NTree + 3 fetch fn + `watch(visible)`〕的「角色×某權限維度」runtime 編輯介面,不擴張到任意新 UI;DESIGN §11.21、amendment `d700434`),僅限「接 wrapper call / button gating / 同模式權限 modal」邊界(017 user CRUD 起用、022 用戶頁 + **024 角色/選單/用戶三頁 button gating**、023 endpoint-auth modal)
