...
| `d50df8b` | 2026-05-30 | Merge feature 015-audit-middleware 回 rev2-admin-root(--no-ff,feature branch 保留)— **Phase 3 audit-middleware 前半:請求層審計(request-context middleware + access-log)+ 解 §2.14 INET 真值機制 + §2.15 xdb runtime 首消費**。**全域 request-context middleware**(`server/src/audit_ctx.rs::audit_mw`、`from_fn_with_state` 掛全 Router〔fallback 後、with_state 前、最外層〕、`main.rs` serve 改 `into_make_service_with_connect_info::<SocketAddr>` 取直連 peer、boot `searcher_init(env XDB_FILEPATH)`〔非 None、避容器 cwd=/app panic〕):每請求建 `RequestContext{operator_id:Option<i64>/client_ip:IpAddr/x_forwarded_for:Option<String>/region:Option<String>/trace_id:String}` 入 extensions;client_ip=`ConnectInfo` peer〔永有值〕、x_forwarded_for=`X-Forwarded-For` raw 逐字〔無→None、**不做信任代理解析** D5〕、region=`xdb::search_by_ip(client_ip).ok()`〔best-effort、私有 IP→`内网IP`〕、trace_id=入站 `X-Request-Id`〔trim 截斷≤64〕else `uuid::Uuid::new_v4()`〔D9〕;operator_id=試 bearer→`jwt::verify`〔access secret+JWT_AUD〕→`claims.user_id`、失敗→None。**兩審計表**(migration `m..011`/`m..012`、經 010 自動套、append-only 無 deleted_at):`sys_access_log`〔已認證請求:operator_id NOT NULL/method/path/http_status/client_ip INET/x_forwarded_for/region/trace_id〕+ `sys_login_attempt`〔登入嘗試:attempted_user_name/success/operator_id NULL/來源欄 + **2 lockout index** `(attempted_user_name,created_at)`/`(client_ip,created_at)`〕。**「只記已認證」閘門**(research R2:middleware `next.run` 後 **iff operator_id Some** 才寫 access-log〔帶最終 response status〕→ 天然涵蓋 public/health/login/過期 token **不寫**)+ **登入嘗試由 login handler 自記**(D4:`handler/auth.rs::login` 加 `Extension<RequestContext>`、每出口 `record_login_attempt`〔成功路徑唯一 success=true+operator_id Some(user.id)、6 early-return 皆 false+None〕、attempted_user_name 截斷≤64)。**best-effort**(D6/FR-007:`best_effort_audit(Result<(),DbErr>, what)` wrapper 吞 Err 只 `tracing::warn!`、型別保證不傳播〔middleware 回 Response/handler 回 Res、皆非 Result〕、收斂兩呼叫點;**非** 011 `mutate_in_txn` 原子)。facade `sys_access_log`/`sys_login_attempt::write`〔唯一構造 ActiveModel、單筆 INSERT、守 009 lint〕。**§2.14 INET 真值機制解**:workspace sea-orm += `with-ipnetwork`〔拉 `ipnetwork 0.20`〕+ entity `client_ip: IpNetwork`〔`sea_orm::entity::prelude` re-export〕+ facade `IpNetwork::from(IpAddr)` → emit `'x.x.x.x/32'` typed INET〔SQL-build 單測證、不再 PG 42804〕;**sys_operation_log 不 retrofit-於 015**〔Deviation D-2、折 Phase 4 A、保 011 現狀〕。**§2.15 xdb runtime 首消費者**:`XDB_FILEPATH` env〔dev `/app/xdb/resources/ip2region.xdb` bind-mount、prod `/app/resources/ip2region.xdb`〕+ **prod runtime Dockerfile COPY `ip2region.xdb`**〔~11MB、T017 驗 image 內存在〕。**acceptance 三 US 全綠**:US1 curl 登入成敗各 1 筆 sys_login_attempt〔success/attempted_user_name/operator_id/INET client_ip〕+ US2 getUserInfo(Bearer)→1 筆 sys_access_log·公開 getConstantRoutes/health/無 Bearer→**不寫**·login 自身→不寫 access-log + US3 帶 `X-Forwarded-For: 1.2.4.8, 10.0.0.1`→x_forwarded_for 逐字保存·client_ip=直連 peer〔**非** XFF 解析〕·region 非空;SC-004 best-effort 純單測。守 008 envelope/013 login wire 不變/009 entity-access lint〔17、audit_ctx/handler 不碰 entity::、新表只經新 facade〕/007 FR-009〔server 不自動 migrate、grep 0 命中〕。**無新 workspace crate**;新 dep `uuid`(v4、已 transitive)+ `with-ipnetwork`;**prod runtime image build + xdb 打包驗綠**〔§2.13 跨 feature 守則〕。既有不破(server 56+3→**69+3**〔+13:client_ip v4/v6·xff verbatim/absent·trace_id 5·SQL-build 2·best-effort 2〕+ xdb 9 + lint 17)。**兩段式 commit**(rust-api worktree 8 commits `825f52f..858f318` push fork rev2-admin-rust-api + 外層 Dockerfile/compose XDB_FILEPATH + docs 回填 + SHA pin `858f318`〔commit `0378b4b`〕;**不動 base-web**);subagent-driven-development 17 task/6 phase、各 unit spec+quality 雙審 + opus final holistic review = READY TO MERGE;Constitution 7+7=14 ✅ + Deviation Log D-1(Dockerfile/compose 為外層非 worktree、commit 歸屬校正)/ D-2(sys_operation_log INET retrofit 折 Phase 4 A);015 branch 保留;follow-up 見 CHECKLIST §2.18 |
| `6398906` | 2026-05-30 | Merge feature 016-manage-role-user-list 回 rev2-admin-root(--no-ff,feature branch 保留)— **Phase 4 主流業務第一刀:base-web manage 頁 role+user 3 read endpoint + sys_role/sys_user manage 顯示欄補齊 + 3 條 Casbin enforce(立分頁清單 pattern 供 017 menu 沿用)**。**3 endpoint**:GET /systemManage/getRoleList(分頁+搜尋 roleName/roleCode〔LIKE〕·status〔eq〕、回 `{records,current,size,total}`、只 active)/ GET /systemManage/getUserList(分頁+搜尋 userName/nickName/userPhone/userEmail〔LIKE〕·userGender/status〔eq〕、每列 userRoles〔`roles_for_user` join、role code 陣列〕、**取代 013 getUserList stub**、**無 password**)/ GET /systemManage/getAllRoles(不分頁 bare array `{id,roleName,roleCode}[]`、只 active)。**id wire=number**(constitution v1.1.0 §I.3/§11.10 amend、`9f1452d`〔constitution〕/`ace455f`〔回填 DESIGN〕;i64 直序列化、覆寫先前 string 拍板;`getUserInfo.userId` 維持 string 不受影響)。**schema 補欄**:`sys_role` +4(description/status/created_at/updated_at)、`sys_user` +6(status/gender/phone/email/created_at/updated_at)、**status/gender VARCHAR '1'/'2'**(對齊 base-web EnableStatus/UserGender、非 PG enum);3 migration `m..013_alter_sys_role`/`m..014_alter_sys_user`(NOT NULL 欄帶 DEFAULT '1'/now() 護既有 3 seed、各欄獨立 alter_table Postgres-safe)/ `m..015_seed_manage_policy`(getRoleList/getAllRoles 各 R_SUPER/R_ADMIN 共 4 條、getUserList 沿 009、`ON CONFLICT DO NOTHING`、經 010 自動套)。**facade pattern(立供 017)**:`*_filtered(filter)->Select<Entity>` WHERE-build seam(`.contains` LIKE/`.eq` 等值、空白 `.trim()` 略過〔FR-009〕、全參數化)+ `list_paged` count(非投影 filtered query)+ order_by_desc(created_at)+limit/offset(`current.saturating_sub(1)*size`)+ `all_active`(getAllRoles)。**password 投影排除**(`sys_user::user_list_select` `select_only()` 顯式列舉 9 欄〔不含 Password〕→ `UserListRow` FromQueryResult〔無 password 欄〕→ `UserItem` DTO〔無 password 欄〕,SQL-build 單測 `!sql.contains("password")` + 活體 grep 雙證)。`PageResp<T>{records,current,size,total}`(serde camelCase)+ Output DTO(camelCase、createBy/updateBy null〔operator 折 Phase 4 A〕)+ `normalize_page`(current 缺/0→1、size 缺→10、>100→100 clamp、純單測)。**handler 因 009 entity-access lint 不可 `use entity::`/`impl From<Entity>` → inline field-access map**(facade 返 row、handler 映 DTO)。**acceptance 三 US 全綠**:getRoleList 分頁+`roleName=管理`〔中文 seed〕命中 2·`roleCode=R_ADMIN`·status 篩選·`size=999`→clamp 100 + getUserList userRoles〔Super→[R_SUPER]/Admin→[R_ADMIN]/User→[R_USER_COMMON]〕+**整 response 無 password**+nickName 模糊 + getAllRoles bare array 3 條 + **enforce User 403+5003 / Admin·Super 200 / 無 token 3333**(SC-001..007)。守 008 envelope/009 entity-access lint〔17、handler 不碰 entity::〕/007 FR-009〔server 不自動 migrate、0 命中〕/soft-delete〔只 active〕/013 enforce。**無新 crate/dep/表**(sea-orm paginate/contains 內建、只 alter+seed);prod runtime image build 193MB 驗綠。既有不破(server 69+3→**92+3**〔+23:normalize_page 5·role filter SQL-build 7·user filter+no-password SQL-build 11〕+ xdb 9 + lint 17)。**兩段式 commit**(rust-api worktree 7 commits `1febd20..97c602f` push fork rev2-admin-rust-api + 外層 DESIGN 回填 `6c8a921` + SHA pin `97c602f`〔commit `a797770`〕;**不動 base-web、無外層 deploy 改動**);subagent-driven-development 14 task/6 phase、各 unit spec+quality 雙審 + final holistic review = READY TO MERGE;Constitution 7+7=14 全 PASS〔13 ✅ + 1 ⚠️ §I.5 user 授權例外〕 + Deviation Log D-1(id wire string→number amend v1.1.0)/ D-2(schema 補欄交叉參照 rev1 rust-api、隔離抽純欄位事實、破 §I.5 user 授權)/ D-3(operator created_by/updated_by + status/gender DB CHECK 約束折 Phase 4 A);016 branch 保留;follow-up 見 CHECKLIST §2.19 |
