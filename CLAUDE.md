...
**Phase**: 階段 2 TDD 實作 ✅ 完成 + merge (2026-05-30、merge `6398906` / SHA pin `97c602f`) — **Phase 4 第一刀「manage-role-user-list」**:3 read endpoint〔getRoleList 分頁+搜尋 / getUserList 分頁+userRoles join+**無 password**、取代 013 stub / getAllRoles 不分頁 bare array〕+ sys_role +4 欄/sys_user +6 欄〔VARCHAR status/gender '1'/'2'、3 migration:2 alter+1 seed policy〕+ 3 條 enforce_mw〔m..015 seed getRoleList/getAllRoles、getUserList 沿 009〕+ **id wire=number**〔constitution v1.1.0、`9f1452d`/`ace455f`〕。立「分頁 facade seam(`*_filtered`+`list_paged`+`select_only` password 投影)+ Output DTO(camelCase、id number)+ `normalize_page` clamp + enforce 掛路由」pattern 供 017。三 US 活體(分頁/搜尋/size clamp 100/userRoles/無 password/enforce 403+5003·200·3333)+ server 69→92/3 + lint 17 + prod build 193MB + Constitution 14 PASS(13✅+1⚠️ §I.5)。subagent-driven 14 task/6 phase、各 spec+quality 雙審 + final holistic review。**不動 base-web、無新 crate/dep/表、無外層 deploy 改動**。Deviation D-1(id amend)/D-2(rev1 schema 交叉參照破 §I.5、user 授權)/D-3(operator+DB 約束折 Phase 4 A)。rust-api worktree 7 commits `1febd20..97c602f`、016 branch 保留。
**下一步**: **017 menu 三件**(getMenuList/v2·getAllPages·getMenuTree、需建選單表、沿用 016 分頁 pattern)/ **Phase 4 A** 資料變動補 operator(createBy/updateBy + sys_operation_log INET retrofit)/ **Phase 3 續做**(全路由 enforce 矩陣〔含 016 兩端點 enforce 單測補〕/ redis pub-sub / axum-casbin fuller rewrite / 受管 policy 層)。follow-up 見 [CHECKLIST §2.19](docs/INTEGRATION-CHECKLIST.md)。
<!-- SPECKIT END -->
