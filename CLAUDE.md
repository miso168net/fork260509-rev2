...
<!-- SPECKIT START -->
**Active Spec**: [`specs/022-manage-button-auth/spec.md`](specs/022-manage-button-auth/spec.md)
**Active Plan**: [`specs/022-manage-button-auth/plan.md`](specs/022-manage-button-auth/plan.md)
**Phase**: **022 ButtonAuth — ✅ 完整收尾(merge `e103de0` 回 rev2-admin-root〔--no-ff、保留 022 branch〕+ 已推 origin + finalize)**。**角色×按鈕權限 runtime 編輯**(鏡像 021:Casbin `(role,code,'button')` HARD REPLACE via **stock adapter 不 fork**、`auth/button_auth.rs` **無自鎖**、`getUserInfo.buttons` 改讀 Casbin 字典序、buttons.rs 退場)+ **per-menu 按鈕來源**(`sys_menu.buttons` 聚合 registry、**key-on-code**)+ **pilot 用戶管理頁 `hasAuth` gating**(端到端真生效;Admin 見編輯不見刪除/新增)+ **救活 toggle-auth demo**(seed function/toggle-auth + 三角色 menu policy、保 B_CODE1/2/3)。3 Super-only handler + **endpoint policy seed(FINDING A、tasks.md 漏列、鏡像 021 補)**;單一 migration 022、**無新 crate/dep**。**Constitution v1.3.0 §IV 8/8 PASS**(2 amendment `7a3ebd1`:MODAL-WIRING +button gating〔§11.19〕/ §I.2 toggle-auth 例外〔§11.20〕)。**getUserRoutes 逐字基線 re-base**(三角色 +function/toggle-auth、FR-010 唯一 intended 例外);getUserInfo.buttons B_CODE 子序逐字保留。subagent-driven T002-T014 + 兩階段 review + opus final = Ready;**CDP 4/4**(toggle-auth 3/2/1 + 用戶頁 gating + button-modal registry + 順補 021 menu-modal)+ 守恆(server 189/lint 17)全綠。**menu+button auth arc 完整閉合**。
**下一步**: 022 收尾完成。候選:**Phase 3 RBAC 深化**(完整 policy 矩陣 + 全路由 enforce rollout / axum-casbin fuller rewrite / 受管 RBAC policy 層〔需 fork sea-orm-adapter、評估 §11.6 Amendment〕)/ **Phase 4 餘**(其餘業務頁 button gating〔FR-009 out〕/ 選單 restore·re-parent / alova stub Phase 5)。詳見 [DESIGN §10](docs/INTEGRATION-DESIGN.md)。
<!-- SPECKIT END -->
