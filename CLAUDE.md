...
**Phase**: 階段 1 SDD 設計鏈 — specify ✅(16/16、0 提問) / clarify ✅(11 類全 Clear/N-A/延 plan、0 提問) / plan ✅(Constitution 7+7=14 ✅;research R1-R8〔§I.5 未 grep rev1、wire 權威=base-web〕;**有 data-model**〔sys_role(009 soft-delete)/sys_user_role/sys_user.nick_name + casbin policy seed(stock) + 4 wire DTO(camelCase) + JWT Claims + casbin RBAC model + AppState.enforcer + button 矩陣〕;C-V §0-§6 + CDP 登入 smoke + quickstart)。**Phase 3 RBAC 起手 feature「auth-login-enforce」**:登入 + getUserInfo + JWT(HS256、新 dep jsonwebtoken)+ **第一個 Casbin enforce 點(最小機制證明、rev2 自家 axum middleware = §11.6 重寫第一刀 + 012 stock adapter + RBAC model、一條示範路由 allow+deny)**。wire 對齊 base-web mock(LoginToken/UserInfo、camelCase、userId string、Super/Admin/User→User01 alias);error code 對齊 mock §4.11(login 1000/token 3333/refresh 8888/deny 5xxx);Res<T> err 泛型化(§2.11)。**最高風險 = wire DTO camelCase 3 端對齊 + CDP smoke 的 base-web .env→rev2 設定相依(R8)**。scope:完整 policy 矩陣/全路由 enforce/login audit(§2.14/§2.15)/casbin_rule soft-delete/refresh 持久化/dynamic routes 皆**後續階段**。**動 rust-api worktree + base-web .env(BASE-WEB-ADAPT)→ 兩段式 commit**
**下一步**: `/speckit-tasks` →(`/speckit-analyze`)→ `superpowers:executing-plans`。feature branch `013-auth-login-enforce`(pre-hook 已建);spec docs 落此 branch、rust-api 改動走兩段式 commit。完成後 merge 回 `rev2-admin-root`
<!-- SPECKIT END -->
