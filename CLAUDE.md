...
<!-- SPECKIT START -->
**Active Spec**: [`specs/028-single-session-enforcement/spec.md`](specs/028-single-session-enforcement/spec.md)（✅ 已落地）
**Active Plan**: [`specs/028-single-session-enforcement/plan.md`](specs/028-single-session-enforcement/plan.md)（✅ 已落地）
**Phase**: **028 single-session-enforcement — ✅ 全完成 merge `d528e61`**（--no-ff、保留 028 branch、已推 origin;rust-api `1bd0436..d958c02` 13 commit push fork、base-web 零改）。把 027 刻意維持的 access stateless 多裝置升級為 **per-account 可控的 access 端單一-session**(policy=開新登入即時踢舊〔舊 session 下個請求 **`7777`**「账号在他处登录」→ 乾淨登出〕、policy=關維持 027 多裝置、**028 全站預設 off dormant**):`Claims` +`sid`(required) + `sys_user` +2 系統欄(migration 027) + config 系統預設 + `auth/session.rs`(`resolve_policy` 純 seam + pointer **Redis+sys_user 混合 fail-open** + `is_current`) + **4 認證 gate** + refresh pointer-first + login revoke 舊鏈+一律 set_pointer。**wire 中性、base-web 零改、無新對外端點/crate/amendment**。acceptance:U1/U2/pre-028-reject + live L1-L5 + curl C1-C4 + CDP modal smoke + 守恆 server **221**/lint **17**/**30** + migration 可逆 + prod build 全綠;Constitution v1.5.0 §IV **8/8 PASS**;subagent-driven T002-T021 雙審 + final holistic = READY TO MERGE。真正 per-device OUT(FR-013)。as-built [DESIGN §6.6·§10 Phase 5](docs/INTEGRATION-DESIGN.md);follow-up [CHECKLIST §2.33](docs/INTEGRATION-CHECKLIST.md)（same-second jti + base-web modal boot race，皆非 028 後端缺陷）。
**下一步**: **029-single-session-admin-ui**(028 policy 的 admin 管理 UI:系統預設 runtime store〔rev2 首張 system-settings 表、把 028 config 預設變 runtime 可調〕+ 每帳號 policy UI〔使用者管理頁〕+ get/set endpoint + casbin + base-web,疊在 028 儲存之上)**未起** → 待 Phase 0 brainstorm。
<!-- SPECKIT END -->
