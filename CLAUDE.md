...
<!-- SPECKIT START -->
**Active Spec**: [`specs/020-manage-menu-write/spec.md`](specs/020-manage-menu-write/spec.md)
**Active Plan**: [`specs/020-manage-menu-write/plan.md`](specs/020-manage-menu-write/plan.md)
**Phase**: **020 階段 1 SDD 設計鏈進行中**(brainstorm `91432b0`〔已推〕+ specify `400b2f6` + clarify〔0 題、spec 無修、D1-D5 已釘〕+ plan/research〔5-cluster grep `wf_5cd259ad`〕/data-model/contracts/quickstart 已產,待 `/speckit-tasks`)。020 = Phase 4 menu **寫端 CRUD**:4 條 Super-only 寫端(addMenu/updateMenu/deleteMenu/batchDeleteMenu)寫 019 `sys_menu`,閉成完整選單管理 CRUD = **D1 統一源 payoff**(編輯既有可見選單→getUserRoutes 即時反映,code 不動)。**rust-api + base-web 兩倉**(非 server-only;鏡像 017/018)。親決:D1 範圍=menu 定義 CRUD〔MenuAuth 角色×選單可見性編輯=獨立後續 feature〕/ D2 **routeName·menuType editable 時 immutable** / D3 **020 不碰 casbin**〔新選單可見性等 MenuAuth、migration 010 不動〕/ D4 **6 種子選單保護**〔不可刪/停用、可改其他欄〕/ D5 **父有 active 子則擋刪**〔2222;batch 任一種子或具子整批拒〕。**不建表/不 alter**(用 019 sys_menu,§I.6 審計欄已備);parentId 彈性反序列化(number\|string\|null→Option<i64>,grep R1 證寫端混型)。Constitution v1.2.1 §IV 8/8 PASS(#8 N/A 不建表;無 amendment,parentId 彈性 + getUserRoutes code-不動 記 Complexity Tracking)。**無新 crate/dep**。
**下一步**: `/speckit-tasks`(產 tasks.md,rust-api+base-web:facade 寫端〔create/update/soft_delete + guard seam〕→ handler 4 寫端+種子/父子 guard → migration 020 write policy + 4 route → base-web wrapper 4 fn + MODAL-WIRING 接 5 placeholder → Polish/C-V)→ `/speckit-analyze` → 階段 2 `superpowers:executing-plans`。outer 在 `020-manage-menu-write` feature branch(brainstorm `91432b0`〔已推〕/ specify `400b2f6` / plan 待 commit,**plan 本地未推**);rust-api/base-web worktree 未動(實作階段才改)。
<!-- SPECKIT END -->
