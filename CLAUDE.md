...
<!-- SPECKIT START -->
**Active Spec**: [`specs/027-refresh-token-rotation/spec.md`](specs/027-refresh-token-rotation/spec.md)
**Active Plan**: [`specs/027-refresh-token-rotation/plan.md`](specs/027-refresh-token-rotation/plan.md)
**Phase**: **027 refresh-token-rotation — ✅ 實作+全驗收+merge 完成**(merge `2d4d8d4`、--no-ff、保留 027 branch、已推 origin;rust-api worktree `93b9be6..1bd0436` 7 commit push fork `rev2-admin-rust-api`、**base-web 零改**;外層 docs `2bf468c` + SHA pin `cc19f80`)。把 013 最小無狀態 refresh 升級為 **DB 持久化 rotation chain + 盜用偵測 + grace 寬限窗口 + SHA-256 雜湊**:新 `sys_token` 表(migration 026,session infra 表、§I.6 PASS-by-scope)+ entity + 唯一寫入 facade(`decide_rotation` 純判定 seam FR-014 全分支 + `rotate` 狀態機〔`db.begin()` 純 txn + 單列 FOR UPDATE、active→used→revoked、reuse→整族系 revoke〕+ `create_chain_head`)+ `login`(015 審計後串)/`refresh_token`(走 rotate)兩 handler。**wire 中性**(`LoginToken`/Claims/issue_tokens 逐字不變、失敗碼 `8888`/`5000` 絕不 `3333/9999/9998`)、**無新 workspace crate**(sha2/chrono 加 server dep、皆 lock 既有 transitive、非 fork)、**無 amendment**。subagent-driven-development A-G 雙審 + final holistic = READY TO MERGE;acceptance live-DB L1-L6 + curl C1-C5 + 守恆 server **218**/lint 17/30 + migration up→down→up 可逆 + prod build 全綠。Constitution v1.5.0 §IV **8/8 PASS**。詳見 [DESIGN §6.2·§10 Phase 5](docs/INTEGRATION-DESIGN.md) / [MILESTONES §1](docs/INTEGRATION-MILESTONES.md);follow-up CHECKLIST §2.32。
**下一步**: **028-single-session-enforcement** 已立案(027 拆出、登記 DESIGN §10 Phase 5 + CHECKLIST §2.32;access 端 stateful 即時踢/current-session pointer、027 落地後可做)。下個 feature `/speckit-specify` 由 user 手動執行(建 feature branch)。
<!-- SPECKIT END -->
