...
**Phase**: **019 階段 1 SDD 設計鏈進行中**(brainstorm `d1a4883`〔已推 rev2-admin-root〕+ specify `c20d247` + clarify `b0e4843`〔Q1 讀端 Super-only〕+ plan/research〔5-cluster grep `wf_ac073965`〕/data-model/contracts/quickstart 已產,待 `/speckit-tasks`)。019 = Phase 4 menu 改 **DB-driven**:建 `sys_menu` 業務表為選單定義單一真相,**runtime getUserRoutes 與管理頁 getMenuList 共讀同源**,可見性仍走 Casbin enforce(§I.2 不變)。本波 = **讀端 + runtime 來源遷移**(寫端 CRUD → 020,D2)。**server-only**(base-web menu fns 已存在 upstream、getUserRoutes wire 逐字不變 → 不動 base-web)。親決:D1 DB-driven 統一源 / D2 讀端+runtime(寫端→020)/ D3 buttons 存 sys_menu JSONB 欄 / D4 可見性 Casbin policy(010)不變·MenuAuth 編輯→後續 / D5 **getUserRoutes 輸出逐字不變(回歸鐵律)**+ constantRoutes/isRouteExist(enforce-based)不動。sys_menu = §I.6 凍結後**首張新建業務表**(create 即帶 6 審計欄、forward-only);3 讀端 Super-only。Constitution v1.2.1 §IV 8/8 PASS(無 amendment,getUserRoutes 源遷移記 Complexity Tracking)。**無新 crate/dep**。
**下一步**: `/speckit-tasks`(產 tasks.md,server-only:schema→facade+樹組裝→getUserRoutes 遷移→3 讀端→Polish)→ /speckit-analyze → 階段 2 `superpowers:executing-plans`。outer 在 `019-manage-menu-list` feature branch(specify `c20d247` / clarify `b0e4843` / plan 待 commit,**全本地未推**);rust-api worktree 未動(實作階段才改),**base-web 本波不動**(server-only)。
<!-- SPECKIT END -->
