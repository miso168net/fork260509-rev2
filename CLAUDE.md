...
**Phase**: **034 managed-rbac-policy(受管 RBAC policy 治理層、Phase 3 #6)— ✅ 已實作+雙審(本機、awaiting merge)**。架構 **B(archive 表、免 fork)**:casbin_rule 永遠只裝 live policy、軟刪列搬 `sys_casbin_policy_archive`;親驗 stock adapter 只認 `ptype,v0..v5` → **soft-delete 不需 fork adapter、後端不觸 §11.6、無 backend amendment**(推翻原 defer 主因)。寫側 DB-first(facade sea-orm 直寫 casbin_rule + 011 audit 同 `mutate_in_txn` → commit → enforcer `load_policy()` → publish),旁路 adapter auto_save = 修 D2 非原子。**5 US 全交**:US1 原子 facade+audit / US2 data-driven protected(修 D13、退役 `is_seed_menu` 4 caller)/ US3 menu↔policy 同步(修 DRIFT-2/3/4、同名重建零繼承)/ US4 收斂 021/022/023 為 `set_role_dimension`(wire 逐位元組不變)/ US5 base-web 回收桶 UI + 2 端點 + D10。**5 migration m031-m035**。**Constitution US1-US4 全 PASS;US5 回收桶頁經 user 親決(2026-06-09)落 MODAL-WIRING use (e) v1.6.0 既有界、不需 v1.7.0 amendment**(DESIGN §11.25)。server 245/0 + lint 17/5 + live 17 + prod build GREEN + CDP smoke 過。設計見 [`docs/superpowers/034-managed-rbac-policy.md`](docs/superpowers/034-managed-rbac-policy.md) + [`specs/034-managed-rbac-policy/`](specs/034-managed-rbac-policy/) + [DESIGN §10 Phase 3 #6](docs/INTEGRATION-DESIGN.md)。
**下一步**: **034 收尾 merge**(T054 `superpowers:finishing-a-development-branch` → `git merge --no-ff` 回 `rev2-admin-root`、保留 034 branch;merge 後 MILESTONES append row + outer SHA pin)。worktree commits:rust-api `87e3d8a..a2a52fb` push fork、base-web `75f2a20..a5783df` push fork;034 spec/plan/docs 回填在 feature branch `034-managed-rbac-policy`。
<!-- SPECKIT END -->
