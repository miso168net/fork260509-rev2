...
<!-- SPECKIT START -->
**Active Spec**: [`specs/013-auth-login-enforce/spec.md`](specs/013-auth-login-enforce/spec.md)
**Active Plan**: [`specs/013-auth-login-enforce/plan.md`](specs/013-auth-login-enforce/plan.md)
**Phase**: 階段 2 TDD 實作 **✅ 全完成 + merge**(subagent-driven-development、16/16 task、各 unit spec+quality 雙審 + opus final holistic review = **Ready to merge**)。**Phase 3 RBAC 起手 feature「auth-login-enforce」落地**:登入 + getUserInfo + JWT(HS256、新 dep jsonwebtoken 9.3.1〔MSRV 1.86 故非 10.x〕)+ **首個 Casbin enforce 點**(rev2 自家 axum middleware = §11.6 重寫第一刀 + 012 stock SeaOrmAdapter + RBAC model;示範路由 `/systemManage/getUserList` allow〔Super/Admin〕+ deny〔User 403+5003〕)+ 最小無狀態 refresh(8888、絕不 3333/9999/9998)。sys_role/sys_user_role + sys_user.nick_name(User→User01)+ migration 006-009(經 010 自動套)+ facade(守 009 lint)+ Res<T> 泛型(§2.11)+ BizCode 5003。**acceptance 三 US 全綠**:curl 活體 + **首條真 base-web↔rev2 CDP 瀏覽器登入 smoke**(token iss=rev2-admin、§I.1 里程碑)。Constitution 7+7=14 ✅ + Deviation Log D-001~003。**兩段式 commit**(rust-api worktree 13 commits `60e675e..bbabdbc` push fork、base-web `.env.test`→rev2 push fork、外層 SHA pin)
**下一步**: 013 已 merge 回 `rev2-admin-root`(--no-ff、feature branch 保留)。Phase 3 續做:#2 dynamic routes(getUserRoutes/getConstantRoutes、menu 過濾)/ #4 完整 policy 矩陣 + 全路由 enforce rollout / #5 axum-casbin fuller rewrite(metrics/observability)/ #6 受管 RBAC policy 層(casbin_rule soft-delete)/ audit-middleware(operator_ip/xdb、§2.14/§2.15)。013 follow-up nits 見 [CHECKLIST §2.16](docs/INTEGRATION-CHECKLIST.md)
<!-- SPECKIT END -->
