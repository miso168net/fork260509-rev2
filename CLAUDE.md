...
<!-- SPECKIT START -->
**Active Spec**: [`specs/014-dynamic-routes/spec.md`](specs/014-dynamic-routes/spec.md)
**Active Plan**: [`specs/014-dynamic-routes/plan.md`](specs/014-dynamic-routes/plan.md)
**Phase**: 階段 2 TDD 實作 **✅ 全完成 + merge**(subagent-driven-development、13 task、各 unit spec+quality 雙審 + opus final holistic review = READY TO MERGE;SHA pin `8965c8a`、merge `9d06347`)。**Phase 3 #2 feature「dynamic-routes」落地**:base-web 翻 dynamic auth route mode + 3 route endpoint(getConstantRoutes 公開 / getUserRoutes·isRouteExist JWT、**過濾在 handler 內、不掛 enforce middleware**)+ **業務 menu 可見性走 Casbin enforce 過濾**(menu-visibility policy `p,role,route_name,menu` migration 010 seed **9 rows**〔D3 矩陣 Super5+Admin3+User1、父層 manage tree-prune 不 seed、經 010 自動套〕+ 013 enforcer `enforce((role,route_name,"menu"))` + tree-prune,§I.2 核心原則落地)。route 定義程式內(`server/src/route/menu.rs`,component 對齊 base-web elegant-router `$` 複合格式、§I.5 未 grep rev1)+ enforce 決策/tree-prune **test-first 6 新單測**(MemoryAdapter)。**acceptance 全綠**:三 endpoint curl(三角色不同 menu + home + 3333 + isRouteExist **User manage_role deny**)+ **CDP dynamic-mode 瀏覽器 smoke(Super 側欄「系统管理」vs User 只「首页」= menu deny 端到端)**。**無新 dep/表/crate**;守 007 FR-009 + 009 lint(17)+ 008;prod build 驗綠;既有不破(server 56+3〔+6〕+ xdb 9)。Constitution 7+7=14 ✅ + Deviation D-001(§I.2 re-scope)/D-002(menu row 文件計數 8→9 修正)。**兩段式 commit**(rust-api worktree 7 commits `f65105e..c4b1d7e` push fork、base-web `.env` dynamic `41f44b18` push fork、外層 SHA pin)
**下一步**: 014 已 merge 回 `rev2-admin-root`(--no-ff、feature branch 保留)。**Phase 3 續做**:#3 redis pub-sub(policy/route 失效通知)/ #4 完整 policy 矩陣 + 全路由 endpoint enforce rollout / #5 axum-casbin fuller rewrite(metrics/observability)/ #6 受管 RBAC policy 層(casbin_rule soft-delete/protected/audit/CRUD、須評估 §11.6 Amendment)/ audit-middleware(operator_ip/xdb、§2.14/§2.15)。013/014 已完成(見 [CHECKLIST §1](docs/INTEGRATION-CHECKLIST.md))
<!-- SPECKIT END -->
