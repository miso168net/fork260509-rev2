...
**Phase**: **019 階段 2 實作完成 + C-V 驗收全綠(本地未推未 merge,待 finishing)**(SDD 設計鏈 specify `c20d247`/clarify `b0e4843`/plan `b8ae37a`/tasks `0f99d44`/analyze `dffaa73` → subagent-driven 4 單元〔A entity+migration018 seed / B facade+純函式 assemble_menu_tree / C getUserRoutes 改讀 sys_menu 逐字回歸 / D 三讀端+Super-only policy019〕各兩階段 review〔spec+quality〕+ opus final = READY TO MERGE;rust-api worktree `20022fb..3ef3369` 5 commit + 外層 SHA-pin `239dcfd`)。019 = Phase 4 menu 改 **DB-driven**:建 `sys_menu` 業務表為選單定義單一真相,**runtime getUserRoutes 與管理頁 getMenuList 共讀同源**,可見性仍走 Casbin enforce(§I.2 不變)。本波 = **讀端 + runtime 來源遷移**(寫端 CRUD → 020,D2)。**server-only**(base-web menu fns 已存在 upstream、getUserRoutes wire 逐字不變 → 不動 base-web)。親決:D1 DB-driven 統一源 / D2 讀端+runtime(寫端→020)/ D3 buttons 存 sys_menu JSONB 欄 / D4 可見性 Casbin policy(010)不變·MenuAuth 編輯→後續 / D5 **getUserRoutes 輸出逐字不變(回歸鐵律)**+ constantRoutes/isRouteExist(enforce-based)不動。sys_menu = §I.6 凍結後**首張新建業務表**(create 即帶 6 審計欄、forward-only);3 讀端 Super-only。Constitution v1.2.1 §IV 8/8 PASS(無 amendment,getUserRoutes 源遷移記 Complexity Tracking)。**無新 crate/dep**。
**下一步**: **019 已完整實作 + C-V 驗收全綠**(getUserRoutes 三角色逐字 diff=0 + getConstantRoutes 不變 + 三讀端形狀/Super-only〔Admin 5003·none 3333〕 + 真 CDP /manage/menu 顯 6 筆真值〔父級ID top=0·隱藏=是·icon·排序〕+ migration 018+019 up→down→up 可逆〔010 menu policy 9 不動〕 + prod image build 綠 + 守恆〔server 150 單測 + lint 17 + handler 零 entity::〕)→ `superpowers:finishing-a-development-branch`:本地 `git merge --no-ff` 019 → `rev2-admin-root`(保留 branch),**push 待 user 同意**。017/018 亦本地 merge/未 merge 未推、可同批推送。**寫端 menu CRUD → 020**(D2);**base-web 本波未動**(server-only)。
<!-- SPECKIT END -->
