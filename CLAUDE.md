...
**Phase**: **021 ✅ 完成 + 已 merge `827d1e9`(--no-ff、保留 021 branch)+ 已推 origin**(SDD 設計鏈 brainstorm `903e5a2`/specify `f07e130`/plan `971d59e`/tasks `203a9f3`/analyze `cfbadaf`;階段 2 `superpowers:executing-plans`→`subagent-driven-development` 16 task〔T002-T016〕逐單元 fresh implementer + per-task 兩階段 review〔spec compliance 對 spec.md + code quality〕+ 最終 holistic review,FR-001..010 + SC-001..007 全 trace)。021 = MenuAuth **角色×選單可見性 runtime 編輯**(casbin `remove_filtered_policy(0,[role,"","menu"])`+`add_policies` via **stock adapter 不 fork**、HARD REPLACE)+ **per-role home**(sys_role +home、getUserRoutes 取第一 active 角色 by id ASC)+ **redis pub-sub 失效通知**(消費 dead_code `AppState.redis`、`casbin:policy:invalidate` publish+subscriber reload、**實現 Phase 3 #3**);閉合 menu arc(019 讀→020 寫→**021 可見性指派**)、解 020「新選單指派前不顯 nav」缺口。worktree:**rust-api `bbdadcb`**(T002-T008 + T016 doc)/ **base-web `cbbf1ea`**(T009-T010)。acceptance 全綠:即時反映(無重啟)/redis reload log/自鎖 guard(R_SUPER 須留 manage_menu→2222)/per-role home/**三角色 getUserRoutes 逐字==基線**(回歸鐵律)/migration 021 up→down→up 可逆/prod image build 綠/front-nginx :21080 /api 4 端點。**+1 direct dep `tokio-stream`**(消費 redis pubsub Stream、已在 Cargo.lock transitive、實作期親決;偏離原規劃「無新 dep」、記 plan Complexity Tracking)。Constitution **v1.2.2** §IV 8/8 PASS(不建表 ALTER +home、不 fork、無 amendment)。
**下一步**: **021 已收尾**(merge `827d1e9` --no-ff 回 rev2-admin-root〔保留 021 branch〕、worktree fork〔rust-api `rev2-admin-rust-api` / base-web `rev2-admin-base-web`〕+ 外層 origin 皆已推、MILESTONES row 已補)。**無 active feature** —— 下一 feature 待啟動(`superpowers:brainstorming` → `/speckit-specify`)。**021 follow-up**(CHECKLIST §2.25):home select page-key vs route-name 落差(base-web upstream form)/ CDP browser click-through(curl 雙路徑已證、defer)/ `find_active`·`roles_for_user` stale `#[allow(dead_code)]` 清理(pre-existing)/ add_policies 非單 txn 部分寫入窗口(code 已註)/ **ButtonAuth 編輯**(機制不同、獨立後續 feature)。
<!-- SPECKIT END -->
