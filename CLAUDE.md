...
**Phase**: **025 menu-restore-reparent(Phase 4 餘)— SDD 設計鏈進行中(brainstorm `4403425`/specify〔`0cb3eea`〕/clarify〔0 ambiguity〕/plan ✅ + Constitution v1.5.0 (d) amendment `2b05a5e` ratified→§IV 8/8 PASS / tasks ✅〔T001-T010〕/ analyze ✅〔0 CRIT/HIGH/MED、覆蓋 11/11 FR + 6/6 SC、3 LOW remediated `bcad139`〕— **SDD 設計鏈完成、待實作**)**。補完 020 FR-011 OUT scope:選單 **restore**(復原誤刪軟刪選單)+ **re-parent**(變更自訂選單上層父)。親決 brainstorm S1/R1-R3。後端鏡像 soft_delete/update_menu:facade +restore/list_deleted/find_deleted/would_create_cycle(純函式)+ updateMenu 加回 parent_id;2 新 Super-only endpoint(getDeletedMenus/restoreMenu、migration 025、**D1 lint 三方 count 28→30**)、re-parent 騎 updateMenu。**grounding 更正**:de_parent_id 只接 MenuCreateReq(非 update)→ re-parent 須加 MenuUpdateReq.parent_id;`AuditOperation::Restore` 已存在(audit.rs 零改);restore 用 find_deleted_by_id;restore/re-parent 零 casbin 觸碰(可見性自動套回)。**無新表/crate/dep/fork/§11.6 amendment**。**Constitution v1.5.0 §IV 8/8 PASS**(MODAL-WIRING ★ v1.5.0 (d) amendment `2b05a5e` user 親決+ratified〔分流四檔:constitution §III.2 / DESIGN §11.23 / MILESTONES `77986e6` / CHECKLIST §1 隨收尾〕,授權 `views/manage/menu/**` 選單復原/re-parent 維運控制 R1/R2/R3)。可順帶清 §2.23/§2.24。
**下一步**: `superpowers:executing-plans`→`subagent-driven-development` + TDD(讀 [tasks.md](specs/025-menu-restore-reparent/tasks.md)、MVP=US1 restore:Foundational T002 facade restore/list_deleted/`would_create_cycle`〔TDD〕+ immutability 重寫 → US1 T003-T005 restore handler/endpoint 3-way 28→30/base-web restore UI 端到端 → US2/US3/Polish;每單元 fresh implementer + spec compliance〔對 spec.md〕+ code quality 雙審;**push/merge 凍結至 finishing**)。
<!-- SPECKIT END -->
