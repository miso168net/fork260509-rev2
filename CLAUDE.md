...
**Phase**: **029 single-session-admin-ui — SDD 設計鏈進行中**(specify ✅ `86c82ec` / clarify ✅〔0 歧義、brainstorm 全決〕/ plan ✅ + **constitution amend v1.6.0** `3bd3eda`〔MODAL-WIRING ★ (e) 同 manage 範式新管理頁〕);TDD 實作未起。把 028 的「單一-session policy 後端/config 設」升級為 **admin UI 可調**:系統預設 runtime 可調 + 每帳號 policy UI + 通用「系統設定」KV 地基(single-session 是第一個設定)。**029 = 後端控制面 + base-web 頁**:新 `system_settings` KV 表(+§I.6 審計欄、migration m…028)+ `AppState.session_mode: Arc<RwLock<SessionMode>>` + `settings_watcher`(pub-sub `settings:invalidate`、鏡像 policy_watcher)使 `is_current` 讀 runtime 預設 + facade(system_settings + `sys_user.update_session_policy`〔+ `DEL sess:{uid}` 即時生效〕)+ 3 Super-only 端點(30→33)+ `getUserList` 加 `sessionPolicy`(additive)+ sys_menu/casbin seed + **base-web 新設定頁**(MODAL-WIRING (e))+ 使用者頁 policy 欄/action。**不改 028 enforcement 邏輯**(只換系統預設來源)、**雙倉**、**無新 crate/dep**。Constitution v1.6.0 §IV **8/8 PASS**(gate 7 = (e) amendment ratified)。詳見 [`specs/029-single-session-admin-ui/`](specs/029-single-session-admin-ui/)。
**下一步**: `/speckit-tasks`(產 tasks.md)→ `/speckit-analyze` → `superpowers:executing-plans` 實作(各 speckit 步由 user 手動執行)。
<!-- SPECKIT END -->
