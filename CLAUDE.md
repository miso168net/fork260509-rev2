...
**Phase**: **023 EndpointAuth(Phase 3 #4)— SDD 設計鏈完成(brainstorm `44ce6b8`/specify/clarify/plan/tasks〔`09cb134`〕/analyze〔0 CRITICAL,remediation 套用〕)、待實作**。**角色×API-endpoint 權限 runtime 編輯**(鏡像 021/022:Casbin `(role,path,method)` HARD REPLACE via **stock adapter 不 fork**、新 `auth/endpoint_auth.rs` **逐 method `remove_filtered` 避免誤刪 menu/button 列**、無自鎖)+ **root-mode**(R_SUPER 永遠全通、`update_role_endpoints` 拒編輯 Super → 整站自鎖歸零)+ **D1 build-time 靜態守衛**(parse main.rs+migration、斷言每 enforce_mw route 有 seed policy + ENDPOINT_REGISTRY const 一致 → 關 path-typo 死路由破口)+ **接口权限 modal**(新、鏡像 button-auth-modal、root-mode disabled-for-Super)。reconcile 過時 `R_SUPER,*,*` wildcard 文字(matcher exact-equality、wildcard 從未實作)。單一 migration 023 只 seed 3 治理端點列、**無新 crate/dep/fork**。**reframe**:全路由 enforce 字面已完成(25/25 零破口)、本波交付 runtime 可編輯 + 治理 + 守衛。**Constitution v1.4.0 §IV 8/8 PASS**(amendment `d700434`+MILESTONES `6fb4725`:§III.2 MODAL-WIRING ★ 邊界 +同模式新權限 modal+trigger〔§11.21〕)。**D1 守衛改 build-time 靜態 lint**(research R5、非 brainstorm 假設 live-DB)。
**下一步**: `superpowers:executing-plans`→`subagent-driven-development` 實作(MVP=US1 curl round-trip)。**/speckit-analyze remediation 已套用**:FR-007 wildcard/menu-read reconcile + DESIGN §11.22(故 tasks T010 降為 verify)、G1 audit C-V(contracts §4f)、research/data-model §11.21→§11.22。詳見 [DESIGN §10 Phase 3 #4](docs/INTEGRATION-DESIGN.md)。
<!-- SPECKIT END -->
