...
**Phase**: **020 階段 2 TDD 實作 + 驗收 + 本地 merge 完成 ✅**(全 17 task,subagent-driven-development 每 task 兩階段 review〔spec+quality〕+ opus final holistic = READY TO MERGE;merge `0c73e3b` 回 rev2-admin-root〔--no-ff、保留 020 branch〕、**已推 origin**)。020 = Phase 4 menu **寫端 CRUD**:4 條 Super-only 寫端(addMenu/updateMenu/deleteMenu/batchDeleteMenu)寫 019 `sys_menu` = **D1 統一源 payoff**(編輯既有可見選單→getUserRoutes 即時反映,code 不動)。**rust-api + base-web 兩倉**。親決 D1-D5 全落地:D2 routeName·menuType immutable 雙鎖(DTO 省欄 + SQL col_expr 不含)/ D3 不碰 casbin(010 不動、runtime 零 casbin_rule 寫入)/ D4 6 種子保護(不可刪/停用)/ D5 父有 active 子擋刪 + batch 兩段原子拒;§I.6 成對審計;parentId 彈性反序列化(number\|string\|null→Option<i64>)。acceptance 全綠(US1-3 curl/psql + D1 payoff + SC-006 + 三角色 getUserRoutes 逐字回歸 + migration 可逆 + prod build + front-nginx /api proxy);CDP browser smoke defer(headless 無 :9229,curl + /api proxy + 鏡像 017/018 覆蓋,CHECKLIST §2.24)。Constitution v1.2.1 §IV 8/8 PASS(無 amendment)。**無新 crate/dep、不建表**。
**下一步**: 020 已完整實作 + 驗收 + merge + **已推 origin**(rust-api/base-web worktree fork + 外層 rev2-admin-root/020 branch 皆已推、§4.1 兩段式)。下一 feature(見 [CHECKLIST §1 下一步](docs/INTEGRATION-CHECKLIST.md)):MenuAuth·ButtonAuth 編輯(角色×選單可見性、動 010 casbin menu policy)/ 選單 restore·re-parent / alova stub(Phase 5)/ Phase 3 enforce 全路由 rollout。
<!-- SPECKIT END -->
