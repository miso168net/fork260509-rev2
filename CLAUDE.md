...
**Phase**: **025 menu-restore-reparent(Phase 4 餘)— SDD 設計鏈進行中(brainstorm `4403425`/specify〔`0cb3eea`〕/clarify〔0 ambiguity〕/plan ✅、待 amendment 親決→tasks/實作)**。補完 020 FR-011 OUT scope:選單 **restore**(復原誤刪軟刪選單)+ **re-parent**(變更自訂選單上層父)。親決 brainstorm S1/R1-R3。後端鏡像 soft_delete/update_menu:facade +restore/list_deleted/find_deleted/would_create_cycle(純函式)+ updateMenu 加回 parent_id;2 新 Super-only endpoint(getDeletedMenus/restoreMenu、migration 025、**D1 lint 三方 count 28→30**)、re-parent 騎 updateMenu。**grounding 更正**:de_parent_id 只接 MenuCreateReq(非 update)→ re-parent 須加 MenuUpdateReq.parent_id;`AuditOperation::Restore` 已存在(audit.rs 零改);restore 用 find_deleted_by_id;restore/re-parent 零 casbin 觸碰(可見性自動套回)。**無新表/crate/dep/fork/§11.6 amendment**。**⚠️ Constitution §IV 6/8 + FINDING #2/#7**:base-web parentId NTreeSelect(新 form 控件)+ 已刪 toggle/restore **超出 MODAL-WIRING ★ (a)(b)(c) 邊界** → **需 v1.5.0 (d) amendment(待 user 親決、§V.2、先例 022/023)**;親決+ratify 後 8/8 PASS。
**下一步**: **user 親決 MODAL-WIRING v1.5.0 (d) amendment**(逐行列、分流四檔 constitution §III.2/DESIGN §11.23/CHECKLIST §1/MILESTONES)→ ratify → `/speckit-tasks` → `/speckit-analyze` → `superpowers:executing-plans`(MVP=US1 restore)。詳見 [plan.md](specs/025-menu-restore-reparent/plan.md) Constitution Check + spec-design [`docs/superpowers/025-menu-restore-reparent.md`](docs/superpowers/025-menu-restore-reparent.md)。
<!-- SPECKIT END -->
