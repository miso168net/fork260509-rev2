...
<!-- SPECKIT START -->
**Active Spec**: [`specs/028-single-session-enforcement/spec.md`](specs/028-single-session-enforcement/spec.md)
**Active Plan**: [`specs/028-single-session-enforcement/plan.md`](specs/028-single-session-enforcement/plan.md)
**Phase**: **028 single-session-enforcement — SDD 設計鏈進行中**(specify ✅ `b27a8de` / clarify ✅〔0 高影響歧義、brainstorm 已全決〕/ plan ✅);TDD 實作未起。把 027 刻意維持的 access stateless 多裝置並存升級為 **per-account 可控的 access 端單一-session**:政策解析=開的帳號新登入即時踢舊(舊 session 下個請求回 **`7777`**「账号在他处登录」→ 乾淨登出)、解析=關維持 027 多裝置。**028 = 後端引擎 + policy 儲存**:`Claims` 加 `sid`(獨立 uuid、required)+ `sys_user` 加 2 系統欄(`current_session_id`/`session_policy`、migration 027)+ config 系統預設(**028=off、dormant 上線**)+ `auth/session.rs`(`resolve_policy` 純 seam + pointer **Redis〔讀〕+ sys_user〔持久真相〕混合** + `is_current`)+ session 檢查注入 **4 認證 gate**(enforce_mw + getUserInfo/getUserRoutes/isRouteExist、不含 ctx_mw)+ refresh 驗 pointer + 繼承 sid + login revoke 舊鏈(僅 resolved=on)。**wire 中性**(登入/換新回應逐字不變、`Claims.sid` internal、踢用既有 `7777` 絕不 `3333/9999/9998`)、**base-web 零改**、**無新對外端點**、**無新 workspace crate**(redis/uuid/chrono 既有)、**無 amendment**(§11.17 018 stateful 先例;sys_user 系統欄 §I.6 PASS-by-scope)。Constitution v1.5.0 §IV **8/8 PASS**(plan + post-design re-check)。**管理 UI 拆 029**;真正 per-device「每裝置一個 session」OUT(FR-013)。詳見 [`specs/028-single-session-enforcement/`](specs/028-single-session-enforcement/)。
**下一步**: `/speckit-tasks`(產 tasks.md)→ `/speckit-analyze` → `superpowers:executing-plans` 實作(各 speckit 步由 user 手動執行)。**029-single-session-admin-ui** 已登記(DESIGN §10 Phase 5 + CHECKLIST §2.32/§4;028 落地後做)。
<!-- SPECKIT END -->
