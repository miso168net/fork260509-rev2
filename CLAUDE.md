...
**Phase**: 階段 1 SDD 設計鏈 — specify ✅(2 US/FR-001..008/SC-001..004、0 NEEDS CLARIFICATION) / clarify ✅(10 類掃描、0 提問) / plan ✅(Constitution **v1.2.0** 8/8 PASS、**無 violation/Amendment/§I.5 例外**;research R1-R8 三端對齊;data-model sys_menu 讀欄位+seed 修正+build_route_tree 契約;C-V §0-§5 重用 014 CDP)。**Phase 4「getUserRoutes 改讀 sys_menu」**:`GET /route/getUserRoutes` 業務路由樹來源由 014 in-code `business_routes()` → 讀 `sys_menu` 表(單一真相源、拆 D-D)。+ 純函式 `build_route_tree(Vec<Model>)→MenuRoute`〔parent_id 巢狀 + menu_order 升冪 null-last〕、`filter_routes_for_roles` 純化重構吃 tree〔Casbin 過濾不變〕、**移除 `business_routes()`**。grep base-web routes.ts 定讞 017 seed bug:`manage_user-detail` component=NULL+漏 props → 一條 forward-only seed 修正 migration〔m..018 UPDATE component='view.manage_user-detail'+route_ext '{"props":true}'〕。**唯讀;不動 getConstantRoutes/isRouteExist;可見性續住 casbin;props 存 route_ext JSONB**。無新 crate/dep、不動 base-web、無外層 deploy 改動。Deviation D-A(build_route_tree 不抽共用、與 017 build_menu_tree 各一份)。
**下一步**: `/speckit-tasks` →(`/speckit-analyze`)→ `superpowers:executing-plans`。feature branch `018-userroutes-sys-menu`(pre-hook 已建);spec docs + rust-api SHA pin 落此 branch、走兩段式 commit、完成後 merge 回 `rev2-admin-root`。017 已完成+merge(見 [CHECKLIST §1](docs/INTEGRATION-CHECKLIST.md))。
<!-- SPECKIT END -->
