...
**Phase**: **026 auth-dry-refactor — 實作完成、acceptance 全綠、待 finishing merge**(SDD 設計鏈 brainstorm→specify→clarify→plan→tasks 全 ✅;**TDD 實作 T001-T008 ✅** via `superpowers:subagent-driven-development` 各單元 fresh implementer + spec compliance〔對 spec.md〕+ code quality 雙審)。承 Phase 3 #5 縮水後實質剩餘 = auth 層 **DRY refactor**:抽 **`verify_bearer`**(5 callsite bearer→jwt::verify→claims:enforce_mw/get_user_info/get_user_routes/is_route_exist/ctx_mw)+ **`issue_tokens`**(2 callsite access+refresh:login/refresh_token)+ fail-closed/advisory/best-effort 三策略文件化;**roles 段刻意留 inline**(變體+錯誤映射差、CP 值低 → 落地為窄 `verify_bearer`、非設想的 `verify_and_load_*`)、metrics defer Phase 6、token rotation/stale token defer Phase 5。**純 refactor 行為零變**:既有 200 單測前後逐字不變 + 2 新 helper 單測 → server **206** + entity_access_lint 17 + endpoint_coverage_lint 30 + curl 等價 (a)-(g) **14/14 == baseline**;唯一可觀察差異 = verify-failed debug log 4→1(+ctx_mw 原 silent 經 helper)+ refresh sign-error 2→1(親決、wire 中性)。**rust-api 單倉、無新端點/表/crate/dep/migration、不動 base-web/casbin policy、無 amendment**(Constitution v1.5.0 §IV 8/8 PASS)。
**下一步**: `superpowers:finishing-a-development-branch` 多段式收尾 —— bump rust-api SHA pin(worktree 5 commit `a49e1de..` → outer)+ §1 最新進展/MILESTONES 回填(merge SHA)+ `git merge --no-ff` 回 `rev2-admin-root`(保留 026 branch)+ push(**user 同意後**)。push/merge 凍結至此階段。
<!-- SPECKIT END -->
