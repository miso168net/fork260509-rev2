...
<!-- SPECKIT START -->
**Active Spec**: [`specs/035-policy-governance-hardening/spec.md`](specs/035-policy-governance-hardening/spec.md)
**Active Plan**: [`specs/035-policy-governance-hardening/plan.md`](specs/035-policy-governance-hardening/plan.md)
**Phase**: **034 ✅ merge `8e95fa1` + 推 origin(Phase 3 全收、保留 034 branch);035-policy-governance-hardening specify→clarify(0 問)→plan 完成(Constitution 8/8 PASS、無 amendment、無 migration/端點/crate)**。034(受管 RBAC policy 治理層、Phase 3 #6)= 架構 B(archive 表、免 fork adapter、§11.6 後端不觸、無 backend amendment)、5 US/5 migration m031-m035、各單元 spec+code-quality 雙審 + 最終整體 review(16 FR/7 SC 全 MET)、US5 回收桶頁 user 親決落 MODAL-WIRING use (e) v1.6.0(不需 amendment);詳見 [DESIGN §10 Phase 3 #6](docs/INTEGRATION-DESIGN.md) + [MILESTONES §1](docs/INTEGRATION-MILESTONES.md)。**035** = 034 治理 facade 收尾硬化 2 US（純 rust-api、無 migration/端點、base-web 零改）:**US1** restore audit 豐富化（`RestoreOutcome::Restored` 帶 `{v0,v1,v2}` → restore_policy 審計記 `{role,target,dimension}`、非懸空 archive_id）/ **US2** no-op reload 跳過（trait `PolicyMutated`、只跳明確 no-op〔Rejected/restore NoOp/NotFound/menu 查無〕、6 caller 零改）。Constitution 預期全 PASS、無 amendment。設計見 [`docs/superpowers/035-policy-governance-hardening.md`](docs/superpowers/035-policy-governance-hardening.md)。
**下一步**: **user 手動 `/speckit-tasks`**(產 tasks.md)→ `/speckit-analyze`(跨檔一致性)→ `superpowers:executing-plans`(subagent-driven、逐 US 雙審)。035 plan + research/data-model/contracts/quickstart 在 feature branch `035-policy-governance-hardening`(本機、未 push);US1 先(改 RestoreOutcome shape)、US2 後。
<!-- SPECKIT END -->
