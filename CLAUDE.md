...
**Phase**: **026 auth-dry-refactor — SDD 設計鏈進行中(brainstorm `08310d6` Approach A 親決 / specify ✅〔branch、spec + checklist〕/ clarify ✅〔0 critical ambiguity、spec 未變更〕/ plan ✅〔§IV 8/8 trivially PASS、無 amendment;research+data-model+contracts+quickstart〕/ tasks ✅〔T001-T008、US 組織、dependency-ordered〕— 待 analyze)**。承 Phase 3 #5 縮水後實質剩餘 = auth 層 **DRY refactor**(§2.16/17/18/19):抽 **`verify_bearer`**(5 callsites bearer→jwt::verify→claims:enforce_mw/get_user_info/get_user_routes/is_route_exist/ctx_mw)+ **`issue_tokens`**(2 callsites access+refresh:login/refresh_token)+ §2.18 fail-closed/open 文件化;**roles 段刻意留 inline**(變體+錯誤映射差、CP 值低)、metrics defer Phase 6、token rotation/stale token defer Phase 5。**純 refactor 行為零變**(既有測試前後逐字不變 + 兩 helper 新單測驗;唯一可觀察差異=4 條 verify-failed debug log 合併為 1 條,親決)。**rust-api 單倉、無新端點/表/crate/dep/migration、不動 base-web/casbin policy、無 amendment**(Constitution §IV 8/8 trivially PASS)。先前兩個 stale backlog(soft-delete 6-entity rollout 幻影、§2.16 nick_name)已校正/標 ✅。
**下一步**: 手動執行 `/speckit-analyze`(spec/plan/tasks 跨檔一致性),再 `superpowers:executing-plans`+subagent-driven 實作(MVP=US1 verify_bearer:T002 TDD + T003 5 callsite → US2 issue_tokens → US3 文件 → Polish;refactor 主驗=既有測試前後不變 + 2 helper 新單測 + curl 等價 smoke)。`/speckit-analyze` 由 user 手動跑;push/merge 凍結至 finishing。
<!-- SPECKIT END -->
