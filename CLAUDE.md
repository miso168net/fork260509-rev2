...
**Phase**: **022 ButtonAuth — SDD 設計鏈進行中**(brainstorm `fafa3af`/specify `955d37e`〔spec 16/16 PASS〕/clarify〔0 critical〕/plan 本輪:research+data-model+contracts+quickstart)。**角色×按鈕權限 runtime 編輯**(鏡像 021:Casbin `(role,code,'button')` HARD REPLACE via **stock adapter 不 fork**、新 `auth/button_auth.rs` **無自鎖**、`getUserInfo.buttons` 改讀 Casbin 字典序)+ **per-menu 按鈕來源**(`sys_menu.buttons` 聚合 = registry、**key-on-code**)+ **pilot 用戶管理頁 `hasAuth` gating**(端到端真生效;Admin 見編輯不見刪除/新增)+ **救活 toggle-auth demo**(seed function/toggle-auth 入 sys_menu + 三角色 menu policy、保 B_CODE1/2/3 可驗)。單一 migration 022 seed、**無新 crate/dep**。**Constitution v1.3.0**(2 amendment `7a3ebd1`+MILESTONES `27410bf`:§III.2 MODAL-WIRING ★ 邊界 +button gating〔DESIGN §11.19〕+ §I.2 `function`/`toggle-auth` demo→真實 Casbin 選單例外〔§11.20〕;§IV **8/8 PASS**)。唯一 intended 回歸例外 = **getUserRoutes 逐字基線 re-base**(三角色 +function/toggle-auth);getUserInfo.buttons 既有 B_CODE 子序逐字保留。
**下一步**: plan artifacts 已產(research/data-model/contracts/quickstart)→ **`/speckit-tasks`** → `/speckit-analyze` → `superpowers:executing-plans`→`subagent-driven-development` 實作。可一併清 021 §2.25:stale `#[allow(dead_code)]`(`find_active`/`roles_for_user`/`normalize_page`,022 動該檔順手清)+ 021 menu-auth-modal CDP click-through(同 /manage/role 頁同 harness)。
<!-- SPECKIT END -->
