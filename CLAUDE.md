...
**Phase**: **032 obs-full(Phase 6 obs、metrics)— 設計中**(specify ✅〔16/16〕+ clarify ✅〔0 critical〕+ plan ✅〔**Constitution 8/8 PASS**、純 infra/後端、無新 crate/migration〕;tasks/analyze 待)。接續 031 obs-min log 半邊、補 metrics 半邊 + 閉 Phase 3 #5 parked 的 enforce metrics 債。opt-in `profiles:[metrics]` 堆疊 = **prometheus**(`v3.12.0` static-scrape、retention 15d)← rust-api `/metrics`(`axum-prometheus 0.7.0`〔最後 axum-0.7 相容、MSRV 1.70〕HTTP metrics + `enforce_mw` 自埋 allow/deny counter)+ **postgres_exporter**(`v0.19.1`)+ **redis_exporter**(`v1.85.0`)+ **pushgateway**(`v1.11.3`、cleanup-job 推 job metric)→ **grafana**(reuse `13.0.2`、加 prometheus datasource + baseline alert rule)。**動 2 既有 crate(server+cleanup-job)、無新 workspace crate、無 migration/新表、無新 secret(reuse postgres/redis password)、base-web 零改**。greenfield、Constitution 8/8 PASS、無 amendment。least-priv exporter PG role defer follow-up(reuse soybean)。設計見 [`docs/superpowers/032-obs-full.md`](docs/superpowers/032-obs-full.md) + [`specs/032-obs-full/`](specs/032-obs-full/)。
**下一步**: `/speckit-tasks`(dependency-ordered)→ `/speckit-analyze` → `superpowers:executing-plans` 實作(收尾 §4.1 兩段式 commit:server+cleanup-job worktree)。
<!-- SPECKIT END -->
