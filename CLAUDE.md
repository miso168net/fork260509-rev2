...
**Active Plan**: (待 `/speckit-plan` 產出)
**Phase**: **021 階段 1 SDD 設計鏈進行中**(brainstorm `903e5a2`〔已推〕+ specify〔spec.md + quality checklist、M1-M5 已釘、**無 NEEDS CLARIFICATION**〕已產,待 `/speckit-clarify`〔可選〕或 `/speckit-plan`)。021 = MenuAuth **角色×選單可見性 runtime 編輯**(動既有 Casbin menu-visibility policy)+ **per-role home** + **Casbin redis pub-sub 失效通知**(消費閒置 `AppState.redis`、實現 Phase 3 #3/W-F11);閉合 menu arc(019 讀→020 寫→021 可見性指派)、解 020「新選單指派前不顯 nav」缺口。接 base-web 角色管理頁 `menu-auth-modal` 4 placeholder;**ButtonAuth 另開**。親決:M1 範圍=MenuAuth(menu 可見性+home)/ M2 policy 機制=**方案1 混合**(casbin 原生 MgmtApi remove_filtered+add_policies via **stock adapter 不 fork** + 011 audit、**無 §11.6 amendment**;實查 adapter.rs 寫方法齊)/ M3 redis=方案B(§6.3 v1 即啟用、publish `casbin:policy:invalidate`+subscriber reload)/ M4 per-role home(sys_role +home 欄、getUserRoutes per-role、多角色取第一 active by id ASC、未編輯逐字基線)/ M5 自鎖=handler code guard(候選 Super 種子可見性保護)。Constitution **v1.2.2** §IV 8/8 PASS 預判(不建表、不 fork、無 amendment)。
**下一步**: `/speckit-clarify`(可選、spec 無 NEEDS CLARIFICATION 可跳)或 `/speckit-plan`(Phase 0 research grep:casbin MgmtApi 簽章/auto_save/filtered-policy field index、base-web `menu-auth-modal` 送出形〔checks=menu id、home 值型、getUserRoutes.home 期望〕、getUserRoutes home 衍生點〔route/menu.rs 寫死 "home"〕、redis-rs pubsub 獨立連線、sys_menu id↔route_name 對映、多角色 home 取序)→ `/speckit-tasks` → `/speckit-analyze` → 階段 2 `superpowers:executing-plans`。outer 在 `021-manage-menu-auth` feature branch(brainstorm `903e5a2`〔已推 rev2-admin-root〕/ specify 待 commit、**本地未推**);rust-api/base-web worktree 未動(實作階段才改)。
<!-- SPECKIT END -->
