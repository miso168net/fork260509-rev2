...
**Phase**: **033 dashboard-provisioning(Phase 6 obs、dashboards)— 設計中**(specify ✅〔16/16〕+ clarify ✅〔0 critical〕+ plan ✅〔**Constitution 8/8 PASS**、純 grafana config、無 rust/base-web/migration/crate/secret〕;tasks/analyze 待)。接續 031 log + 032 metrics、補 dashboard「可看」面(obs 從 Explore+alert-only → 開 grafana 即見 6 板)。**6 板** = master overview / rust-api / cleanup-job / audit-log(greenfield 自寫、貼 rev2 真 metric/log)+ postgres / redis(pin 官方 exporter dashboard〔`postgres_mixin@v0.19.1` + grafana.com `763 rev6`〕、upstream 非 rev1)。新 `deploy/grafana-provisioning/dashboards/`(provider yaml + 6 JSON `schemaVersion 39`、folder obs-full)+ 改 031 `loki.yml` 補 `uid:loki`。全 live-grounded(3 research agent:provisioning 格式 + exporter dashboard pin/metric 驗 + panel query)。**base-web+rust-api 零改、無 migration/新 crate/新 secret/新 compose service、greenfield(§I.5)、Constitution 8/8 PASS、無 amendment**。設計見 [`docs/superpowers/033-dashboard-provisioning.md`](docs/superpowers/033-dashboard-provisioning.md) + [`specs/033-dashboard-provisioning/`](specs/033-dashboard-provisioning/)。
**下一步**: `/speckit-tasks`(dependency-ordered)→ `/speckit-analyze` → `superpowers:executing-plans` 實作(純 outer/deploy、無 worktree;收尾 finishing-a-development-branch)。
<!-- SPECKIT END -->
