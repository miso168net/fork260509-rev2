...
<!-- SPECKIT START -->
**Active Spec**: [`specs/021-manage-menu-auth/spec.md`](specs/021-manage-menu-auth/spec.md)
**Active Plan**: [`specs/021-manage-menu-auth/plan.md`](specs/021-manage-menu-auth/plan.md)
**Phase**: **021 階段 1 SDD 設計鏈進行中**(brainstorm `903e5a2`〔已推〕+ specify `f07e130`〔spec + checklist、M1-M5 已釘、**無 NEEDS CLARIFICATION**〕+ clarify〔spec 無修、跳過〕+ plan/research〔7-cluster grep `wf_b7e25ff4`〕/data-model/contracts/quickstart 已產,待 `/speckit-tasks`)。021 = MenuAuth **角色×選單可見性 runtime 編輯**(動既有 Casbin menu-visibility policy `(role_code,route_name,'menu')`)+ **per-role home** + **Casbin redis pub-sub 失效通知**(消費閒置 `AppState.redis`、實現 Phase 3 #3);閉合 menu arc(019 讀→020 寫→021 可見性指派)、解 020「新選單指派前不顯 nav」缺口。接 base-web `menu-auth-modal` 4 placeholder;**ButtonAuth 另開**。親決:M1 範圍=MenuAuth(menu 可見性+home)/ M2=**方案1 混合**(casbin MgmtApi remove_filtered+add_policies via **stock adapter 不 fork** + 011 audit、**無 §11.6 amendment**)/ M3 redis=方案B(§6.3 v1 即啟用、publish `casbin:policy:invalidate`+subscriber reload)/ M4 per-role home(sys_role +home 欄、getUserRoutes per-role、多角色取第一 active by id ASC、未編輯逐字基線)/ M5 自鎖=handler code guard(R_SUPER 須留 manage_menu 可見性)。**研究關鍵釘死**:home 寫死於 `handler/route.rs:86`、`roles_for_user` 現**無 ORDER BY**(須加 role_id ASC)、`remove_filtered_policy(0,[role,"","menu"])` 空=wildcard 只清 menu 列、casbin 2.20 auto_save 確認、redis 1.2.1 需獨立 pubsub 連線、policy v0=role **code** 非 id(roleId 須查 sys_role.code)。Constitution **v1.2.2** §IV 8/8 PASS(不建表 ALTER +home、不 fork、無 amendment;runtime policy 編輯 + redis 啟用 + home per-role + roles 加序 記 Complexity Tracking)。**無新 crate/dep**。
**下一步**: `/speckit-tasks`(產 tasks.md,rust-api+base-web:migration 021〔ALTER +home + seed 4 write policy〕→ facade〔id↔route_name 對映 + update_role_home + roles_for_user 加序〕→ menu-auth policy 寫路徑〔set_role_menu + 自鎖 guard〕+ redis policy_watcher → handler 4 端 + getUserRoutes home → 4 route → base-web wrapper 4 fn + MODAL-WIRING 接 4 placeholder → Polish/C-V)→ `/speckit-analyze` → 階段 2 `superpowers:executing-plans`。outer 在 `021-manage-menu-auth` feature branch(brainstorm `903e5a2`〔已推〕/ specify `f07e130` / plan 待 commit,**plan 本地未推**);rust-api/base-web worktree 未動(實作階段才改)。
<!-- SPECKIT END -->
