...
<!-- SPECKIT START -->
**Active Spec**: （無啟動中 feature；上一個 035-policy-governance-hardening 已 merge `e841225`，spec 見 [`specs/035-policy-governance-hardening/spec.md`](specs/035-policy-governance-hardening/spec.md)）
**Active Plan**: （無啟動中 feature；035 plan 見 [`specs/035-policy-governance-hardening/plan.md`](specs/035-policy-governance-hardening/plan.md)）
**Phase**: **035-policy-governance-hardening ✅ merge `e841225` --no-ff 回 rev2-admin-root + 推 origin(保留 035 branch)**。034 治理 facade 收尾硬化 2 US（純 rust-api 4 檔 199+/18-、無 migration/端點/crate/dep、base-web 零改、對外 wire/錯誤碼/讀〔enforce〕決策逐項不變）:**US1** restore 審計記 `{role,target,dimension}`（`RestoreOutcome::Restored` 帶 `{v0,v1,v2}` + 新 `pub(crate) restore_audit_payload` 純函式、取代懸空 archive_id、身分在 archive 刪除前帶出 → FR-002 還原後可解析）/ **US2** no-op reload 跳過（trait `PolicyMutated` 條件化 `mutate_and_reload`〔`if result.mutated()`〕、只跳結構性零變更〔Rejected/restore NoOp/NotFound/menu 查無〕、空-diff Applied 與 menu-found-無-policy 仍 reload〔FR-006〕、6 caller 零改、`batch_delete_menus` any_mutated 追蹤）。server 225/0 + lint 17/5 + live restore payload roundtrip + prod image build + 讀決策不變(FR-005/SC-003) GREEN;SC-001~004 全 MET;**Constitution v1.6.0 8/8 PASS、無 amendment**。詳見 [DESIGN §10 Phase 3 #6](docs/INTEGRATION-DESIGN.md) + [MILESTONES §1](docs/INTEGRATION-MILESTONES.md)。
**下一步**: **無啟動中 feature**。下一個 feature 待 user 定主題 → Phase 0 `superpowers:brainstorming` → 手動 `/speckit-specify`(建 feature branch)。候選見 CHECKLIST §1「下一步」/ §2 backlog（casbin archive 懸空 cruft GC〔D8、§2.39〕/ obs prod 對外暴露+採集端硬化〔§2.36〕/ least-priv exporter+cleanup PG role〔§2.35+§2.37〕/ REVIEW-DATABASE 12-table live 重稽核〔§2.39〕/ Phase 7 維護項〔wire 細節全功能巡檢、upstream rebase、graphify 更新〕）。
<!-- SPECKIT END -->
