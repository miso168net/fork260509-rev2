...
**Phase**: **026 auth-dry-refactor ✅ 完整完成 — Merge `4553892` 回 rev2-admin-root(--no-ff、保留 026 branch)+ 已推 origin**(rust-api worktree `95771ff..93b9be6` 5 commit push fork `rev2-admin-rust-api` + 外層 SHA pin `f8e94ff`;**base-web 不動**)。SDD 設計鏈全 ✅ + **TDD 實作 T001-T008 ✅** via `superpowers:subagent-driven-development`(各單元 fresh implementer + spec compliance〔對 spec.md〕+ code quality 雙審 + final holistic review)。承 Phase 3 #5 縮水 = auth 層 **DRY refactor**:`verify_bearer`(5 callsite bearer→jwt::verify→claims:enforce_mw/get_user_info/get_user_routes/is_route_exist/ctx_mw、各保留失敗映射)+ `issue_tokens`(2 callsite access+refresh:login/refresh_token、各保留 5000)+ fail-closed/advisory/best-effort 三策略文件化;**roles 段刻意留 inline**(CP 值低 → 落地窄 `verify_bearer`、非 `verify_and_load_*`)、metrics defer Phase 6、token rotation/stale token defer Phase 5。**純 refactor 行為零變**:既有 200 單測前後逐字不變 + 2 新 helper 單測 → server **206** + entity_access_lint 17 + endpoint_coverage_lint 30 + curl 等價 (a)-(g) **14/14 == baseline**;唯一可觀察差異 = verify-failed debug log 4→1(+ctx_mw 原 silent 經 helper)+ refresh sign-error 2→1(親決、wire 中性)。**rust-api 單倉、無新端點/表/crate/dep/migration、不動 base-web/casbin policy、無 amendment**(Constitution v1.5.0 §IV 8/8 PASS)。
**下一步**: 026 收尾完成。之後候選([DESIGN §10](INTEGRATION-DESIGN.md)):Phase 3 #5 axum-casbin **fuller rewrite 餘部**(metrics/error/observability + 全路由 rollout,defer Phase 6)/ #6 受管 RBAC policy 層(需 fork→§11.6 Amendment)/ Phase 5(refresh token rotation〔捆綁 stale token〕·alova 3 stub·cleanup-job)/ ButtonAuth「完整版」。下個 feature `/speckit-specify` 由 user 手動執行(建 feature branch)。
<!-- SPECKIT END -->
