...
<!-- SPECKIT START -->
**Active Spec**: [`specs/034-managed-rbac-policy/spec.md`](specs/034-managed-rbac-policy/spec.md)
**Active Plan**: [`specs/034-managed-rbac-policy/plan.md`](specs/034-managed-rbac-policy/plan.md)
**Phase**: **034 managed-rbac-policy(受管 RBAC policy 治理層、Phase 3 #6)— plan 完成(specify→clarify→plan)**。架構 **B(archive 表、免 fork)**:casbin_rule 永遠只裝 live policy、軟刪列搬 `sys_casbin_policy_archive`;親驗 stock adapter 只認 `ptype,v0..v5` → **soft-delete 不需 fork adapter、後端不觸 §11.6、無 backend amendment**(推翻原 defer 主因)。寫側 DB-first(facade sea-orm 直寫 casbin_rule + 011 audit 同 `mutate_in_txn` → commit → enforcer `load_policy()` → publish),旁路 adapter auto_save = 修 D2 非原子。**5 US**:US1 原子 facade+audit / US2 data-driven protected(casbin_rule + sys_menu、修 D13、退役 `is_seed_menu` 4 caller)/ US3 收斂 021/022/023 為 `set_role_dimension`(wire 逐位元組不變)/ US4 menu↔policy 同步(修 DRIFT-2/3/4)/ US5 base-web 回收桶 UI + 2 端點 + D10。4 migration m031-m034。**Constitution US1-US4 全 PASS;唯 US5 回收桶頁需 MODAL-WIRING v1.7.0 amendment(§V.2 user 親決、做到 US5 提案)**。2 Clarification:protected seed 固定不可改 / 選單連帶項排除 policy 回收桶。Phase 0 6-cluster grep 校正命名(`set_role_endpoint` 單數 / `load_policy` 非 reload_policy / 型放 `system-manage.d.ts` 非 rev2-extra)。設計見 [`docs/superpowers/034-managed-rbac-policy.md`](docs/superpowers/034-managed-rbac-policy.md) + [`specs/034-managed-rbac-policy/`](specs/034-managed-rbac-policy/)。
**下一步**: `/speckit-tasks`(產 tasks.md)→ `/speckit-analyze` → `superpowers:executing-plans`(subagent-driven、逐 US 雙審)。034 spec/plan/research/data-model/contracts 在 feature branch `034-managed-rbac-policy`(本機未 commit、未 push)。前序 033 obs 三刀已 merge,rev2-admin-root 已 push 至 `d3a0bdf`(含 REVIEW-DATABASE `12b3894` + 034 brainstorm `d3a0bdf`)。
<!-- SPECKIT END -->
