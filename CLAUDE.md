...
**Phase**: **027 refresh-token-rotation — SDD 設計鏈進行中(specify ✅ `4d4839d` / clarify ✅ `dba573d` / plan ✅);TDD 實作未起**。把 013 最小無狀態 refresh 升級為 **DB 持久化 rotation chain + 盜用偵測 + grace 寬限窗口 + SHA-256 雜湊**:新 `sys_token` 表(migration 026,session infra 表)+ entity + 唯一寫入 facade(`decide_rotation` 純函式 seam + `rotate` 狀態機〔FOR UPDATE 序列化、active→used→revoked〕+ `create_chain_head`)+ 改 `login`/`refresh_token` 兩 handler。**wire 中性**(`LoginToken`/access/Claims 逐字不變、失敗碼 `8888`/`5000` 絕不 `3333/9999/9998`)、**base-web 零改**、**無新 workspace crate**(sha2 加 server dep、uuid 已在)、**無 amendment / 無 fork**。Constitution v1.5.0 §IV **8/8 PASS**(#8:`sys_token` = session/token 基礎設施表、非業務主表 → §I.6 六審計欄 scope 解讀不適用;可選 PATCH amendment explicit 化、推薦不 amend)。clarify 拍板:盜用作廢 = 單一族系(token family)/ session 存活 = sliding / 盜用事件 = warn 日誌(持久化審計 defer Phase 6)/ **「一帳號單一登入 + 即時踢」拆獨立 feature 028-single-session-enforcement**(機制 M3 current-session pointer、027 落地後做)。
**下一步**: `/speckit-tasks`(產 tasks.md)→ `/speckit-analyze` → `superpowers:executing-plans` 實作(各 speckit 步由 user 手動執行)。**028-single-session-enforcement** 已立案(027 收尾時登記 DESIGN §10 + Follow-up Backlog)。
<!-- SPECKIT END -->
