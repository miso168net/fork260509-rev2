...
<!-- SPECKIT START -->
**Active Spec**: [`specs/033-dashboard-provisioning/spec.md`](specs/033-dashboard-provisioning/spec.md)
**Active Plan**: [`specs/033-dashboard-provisioning/plan.md`](specs/033-dashboard-provisioning/plan.md)
**Phase**: **033 dashboard-provisioning(Phase 6 obs、dashboards)— ✅ 完成+merged `3d27cd5`(--no-ff、保留 033 branch、本機未 push origin)**(specify→plan→tasks→analyze→subagent-driven 6 板逐單元雙審→acceptance C0-C5+守恆全綠→final review 0 issue;Constitution 8/8 PASS、無 amendment)。**as-built live 修正**:R8 loki uid 需 `deleteDatasources`(否則 crash-loop、persisted auto-gen uid、部署地雷)、R7 rust-api `| json` 需 `|~ \`^{\`` guard(cleanup-job docker-run 經 alloy image 污染 service=rust-api)。接續 031 log + 032 metrics、補 dashboard「可看」面(obs 從 Explore+alert-only → 開 grafana 即見 6 板)。**6 板** = master overview / rust-api / cleanup-job / audit-log(greenfield 自寫、貼 rev2 真 metric/log)+ postgres / redis(pin 官方 exporter dashboard〔`postgres_mixin@v0.19.1` + grafana.com `763 rev6`〕、upstream 非 rev1)。新 `deploy/grafana-provisioning/dashboards/`(provider yaml + 6 JSON `schemaVersion 39`、folder obs-full)+ 改 031 `loki.yml` 補 `uid:loki`。全 live-grounded(3 research agent:provisioning 格式 + exporter dashboard pin/metric 驗 + panel query)。**base-web+rust-api 零改、無 migration/新 crate/新 secret/新 compose service、greenfield(§I.5)、Constitution 8/8 PASS、無 amendment**。設計見 [`docs/superpowers/033-dashboard-provisioning.md`](docs/superpowers/033-dashboard-provisioning.md) + [`specs/033-dashboard-provisioning/`](specs/033-dashboard-provisioning/)。
**下一步**: **Phase 6 觀察性三刀全完成**(obs-min `3383828` + obs-full `a13f85f` + dashboards `3d27cd5`、obs 從「能查」進化到「開即見 6 板」)。033 本機已 merge、**尚未 push origin(user 擇期)**。後續候選:#6 受管 RBAC(deferred、需 §11.6 fork amendment)/ obs prod 對外暴露+硬化(§2.36)/ least-priv exporter·cleanup PG role(§2.35)。
<!-- SPECKIT END -->
