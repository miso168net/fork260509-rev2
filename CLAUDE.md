...
**Phase**: 階段 1 SDD 設計鏈 — specify ✅(16/16、0 提問) / clarify ✅(11 類全 Clear/N-A/延 plan、0 提問) / plan ✅(Constitution 7+7=14 ✅;**Deviation D-001 = §I.2 re-scope**〔初版 brainstorm「程式內 map 過濾」被 Constitution Check item 3 抓違反 §I.2「menu 走 Casbin enforce」→ user 拍板改 menu-visibility policy seed + 013 enforcer 過濾,route 定義仍程式內寫死、D3 不變〕;research R1-R7〔grep base-web 真實 route shape、component `$` 複合格式、constant 5 條〕;data-model〔MenuRoute/UserRoute camelCase + 程式內 route 樹 + menu policy 8 rows seed + enforce 約定 obj=route_name/act=menu〕;C-V §0-§5 + CDP dynamic-mode smoke)。**Phase 3 #2 feature「dynamic-routes」**:base-web 翻 dynamic auth route mode,3 endpoint(getConstantRoutes 公開 / getUserRoutes JWT / isRouteExist JWT),**業務 menu 可見性走 Casbin enforce 過濾(§I.2 核心原則落地、重用 013 enforcer)**;route 程式內寫死、menu policy migration 010 seed(經 010 自動套);D3 三階梯(Super 全 / Admin home+manage〔user,user-detail〕/ User 只 home)。**無新 dep/表/crate**。**dynamic mode 副作用:demo menu 不送 → §11.5 隱藏 demo moot**。**動 rust-api worktree + base-web `.env`(VITE_AUTH_ROUTE_MODE=dynamic、BASE-WEB-ADAPT)→ 兩段式 commit**。**最高風險 = route 物件 component/meta shape 對齊 base-web elegant/routes.ts(CDP smoke 抓)**
**下一步**: `/speckit-tasks` →(`/speckit-analyze`)→ `superpowers:executing-plans`。feature branch `014-dynamic-routes`(pre-hook 已建);spec docs 落此 branch、rust-api 改動走兩段式 commit。完成後 merge 回 `rev2-admin-root`。013 已完成(見 [CHECKLIST §1](docs/INTEGRATION-CHECKLIST.md));Phase 3 餘:#3 redis pub-sub / #4 全路由 endpoint enforce 矩陣 / #5 axum-casbin fuller rewrite / #6 受管 policy 層 / audit-middleware
<!-- SPECKIT END -->
